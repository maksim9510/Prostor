...
AlexKucera reported on Discord (2026-04-16) that after ``prostor update`` pulled
...
Opt-in — not part of default CI:
    PROSTOR_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.prostor/.env).
"""
...
pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set PROSTOR_LIVE_TESTS=1"),
