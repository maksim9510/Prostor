...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    prostor_state = ModuleType("prostor_state")
    prostor_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "prostor_state", prostor_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Prostor"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Prostor"
