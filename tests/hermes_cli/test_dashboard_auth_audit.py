...
Profile-aware location: ``$PROSTOR_HOME/logs/dashboard-auth.log``.
...
def profile_home(tmp_path, monkeypatch):
    """Redirect $PROSTOR_HOME and ~ to a tmp dir for the duration of the test."""
...
def test_audit_write_failure_does_not_raise(monkeypatch, tmp_path):
    ...
    # Point PROSTOR_HOME at a file (not a dir) so mkdir/open will fail.
    broken = tmp_path / "not-a-dir"
    broken.write_text("blocking file")
    monkeypatch.setenv("PROSTOR_HOME", str(broken))
    # Should NOT raise.
