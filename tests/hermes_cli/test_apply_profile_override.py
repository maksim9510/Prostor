"""Regression tests for _apply_profile_override PROSTOR_HOME guard (issue #22502).

When PROSTOR_HOME is set to the prostor root (e.g. systemd hardcodes
PROSTOR_HOME=/root/.prostor), _apply_profile_override must still read
active_profile and update PROSTOR_HOME to the profile directory.

When PROSTOR_HOME is already a profile directory (.../profiles/<name>),
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    the active_profile check, while PROSTOR_HOME already pointing to a
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        PROSTOR_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets PROSTOR_HOME to the prostor root
        ...
        assert result is not None, "PROSTOR_HOME must be set after profile redirect"
        assert "profiles" in result, (
            f"Expected PROSTOR_HOME to point into profiles/ dir, got: {result!r}"
        )
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        user_home = tmp_path / "home" / "prostor"
        profile_dir = user_home / ".prostor" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".prostor").mkdir(parents=True, exist_ok=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SUDO_USER", "prostor")
        monkeypatch.delenv("PROSTOR_HOME", raising=False)
        monkeypatch.setattr(os, "geteuid", lambda: 0, raising=False)
        monkeypatch.setattr(sys, "argv", ["prostor", "-p", "elias", "gateway", "install", "--system"])
    ...
    def test_subcommand_profile_flag_is_not_consumed(self, tmp_path, monkeypatch):
        ...
        When that argv is passed through `prostor mcp add --args`, the early
    ...
    def test_profile_after_chat_subcommand_is_still_consumed(self, tmp_path, monkeypatch):
        """Profile flags historically work after normal Prostor subcommands."""
        result = _run_apply_profile_override(
            tmp_path,
            monkeypatch,
            hermes_home=None,
        ...
        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["prostor", "chat", "-q", "hello"]
    ...
    def test_top_level_profile_after_value_flag_is_consumed(self, tmp_path, monkeypatch):
        ...
        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["prostor", "-m", "gpt-5", "chat"]
    ...
    def test_top_level_profile_after_continue_flag_is_consumed(self, tmp_path, monkeypatch):
        ...
        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["prostor", "--continue"]
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``prostor gateway run`` (no ``-p``) to mean "the root PROSTOR_HOME
    profile". The run-script exports ``PROSTOR_S6_SUPERVISED_CHILD=1``.
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        """PROSTOR_S6_SUPERVISED_CHILD + active_profile=briefer must NOT redirect.

        Reproduces the Docker/profile scoping bug: the supervised default
        gateway is launched as bare ``prostor gateway run`` with
        PROSTOR_HOME=/opt/data (the container root, whose parent is NOT
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `prostor gateway run` still honors
