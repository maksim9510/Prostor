...
class TestUnifiedDashboardRouting:
    ...
    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        ...
        machine root /opt/data — NOT drop PROSTOR_HOME.

        Dropping it makes the child fall back to $HOME/.prostor
        (= /opt/data/.prostor), an empty auto-seeded home, so the dashboard
    ...
    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (PROSTOR_DESKTOP=1) must NOT
