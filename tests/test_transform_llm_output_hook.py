...
def _make_enabled_plugin(prostor_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <prostor_home>/plugins/<name> and opt it in."""
    plugin_dir = prostor_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PROSTOR_HOME", str(tmp_path / "prostor_empty"))
