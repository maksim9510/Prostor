...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"PROSTOR_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"PROSTOR_MAX_ITERATIONS": "not-a-number"})
...
class TestHistoryDisplay:
    ...
    def test_history_shows_recent_sessions_when_current_chat_is_empty(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running Prostor Agent",
                "preview": "check running gateways for prostor agent",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running Prostor Agent" in output
    ...
    def test_resume_updates_prostor_session_id_env_and_context(self, tmp_path):
        ...
        os.environ["PROSTOR_SESSION_ID"] = "current_session"
        _VAR_MAP["PROSTOR_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["PROSTOR_SESSION_ID"] == "target_session"
            assert get_session_env("PROSTOR_SESSION_ID") == "target_session"
        finally:
            cli._session_db.close()
            os.environ.pop("PROSTOR_SESSION_ID", None)
            _VAR_MAP["PROSTOR_SESSION_ID"].set(_UNSET)
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Running Prostor Agent")

        mock_resume.assert_called_once_with(
            "/resume Checking Running Prostor Agent"
        )
