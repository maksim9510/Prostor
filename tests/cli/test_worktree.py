...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("prostor/prostor-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned prostor/* and pr-* branches."""

    def test_prunes_orphaned_prostor_branch(self, git_repo):
        """prostor/prostor-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "prostor/prostor-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "prostor/prostor-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "prostor/prostor-deadbeef" in result.stdout
        ...
        assert "prostor/prostor-deadbeef" in orphaned
        ...
        assert "prostor/prostor-deadbeef" not in result.stdout
