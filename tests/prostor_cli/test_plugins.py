...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<prostor_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <PROSTOR_HOME>/config.yaml.
...
class TestPluginDiscovery:
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PROSTOR_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".prostor" / "plugins"
...
class TestPluginDebugLogging:
    """PROSTOR_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
