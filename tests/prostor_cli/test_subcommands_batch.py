...
# ── deprecated `prostor login` fails gracefully, not with argparse error ────
#
# `prostor login` is a removed command; its handler (`login_command` in
# `prostor_cli/auth.py`) prints a deprecation notice pointing at `prostor auth` /
# `prostor model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `prostor --help`.
