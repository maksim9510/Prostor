...
``prostor_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``prostor --help``, ``prostor version``,
``prostor logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``prostor --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from prostor_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["prostor", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["prostor", "-w", "--tui"], None),
        (["prostor", "version"], "version"),
        (["prostor", "--tui", "chat"], "chat"),
        (["prostor", "-w", "logs"], "logs"),
        (["prostor", "chat", "hello world"], "chat"),
        (["prostor", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["prostor", "-m", "gpt5", "chat"], "chat"),
        (["prostor", "--model", "gpt5", "chat", "hi"], "chat"),
        (["prostor", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["prostor", "-z", "hello world"], None),
        (["prostor", "-z", "hello", "chat"], "chat"),
        (["prostor", "--model=gpt5", "chat"], "chat"),     # inline form
        (["prostor", "--", "chat"], "chat"),               # -- terminator
        (["prostor", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["prostor", "some-plugin-cmd"], "some-plugin-cmd"),
        (["prostor", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["prostor"],                          # bare → chat
        ["prostor", "--help"],                # top-level help
        ["prostor", "-h"],
        ["prostor", "version"],               # known built-in
        ["prostor", "logs"],
        ["prostor", "gateway", "run"],
        ["prostor", "--tui"],
        ["prostor", "-w", "--tui"],
        ["prostor", "chat", "hi"],
        ["prostor", "help"],                  # accepted built-in-ish
        ["prostor", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["prostor", "meet", "join"],          # potential google_meet plugin
        ["prostor", "honcho", "status"],      # potential memory plugin
        ["prostor", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``prostor help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"prostor_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
