...
subparser, placing the flag BEFORE the subcommand (e.g. 'prostor --yolo chat')
...
def _build_parser():
    """Build the prostor argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets PROSTOR_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `prostor gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `prostor <argv>` must exit 0 (help) rather than
