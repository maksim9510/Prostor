...
def prostor_home(tmp_path, monkeypatch):
    ...
    # /etc/prostor on the dev/CI box can't influence the test.
    monkeypatch.setenv("PROSTOR_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_prostor_dotenv(prostor_home=str(home))
