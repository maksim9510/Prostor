"""Tests for prostor_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $PROSTOR_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Prostor-managed node dir.
"""
...
def _make_prostor_node(prostor_home: Path) -> Path:
    """Create a fake $PROSTOR_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = prostor_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_prostor_node(fake_home):
    prostor_home = fake_home / ".prostor"
    node_bin = _make_prostor_node(prostor_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(prostor_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    prostor_home = fake_home / ".prostor"
    _make_prostor_node(prostor_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(prostor_home) == []


def test_removes_dangling_symlink_into_prostor_node(fake_home):
    """A link into the Prostor node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Prostor links that exist; ignores the ones that don't."""
    prostor_home = fake_home / ".prostor"
    node_bin = _make_prostor_node(prostor_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Prostor-managed; node is a real user binary.
