...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "prostor  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.prostor.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.prostor.gateway.plist",
        "launchctl stop ai.prostor.gateway",
        "systemctl restart prostor-gateway",
        "systemctl stop prostor-gateway.service",
        "systemctl start prostor-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill prostor gateway process",
        "pkill -f prostor.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "prostor cron list",
        "prostor update",
        "prostor config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_prostor_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade prostor then run prostor gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.prostor.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart prostor-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-prostor systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart prostor-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart prostor-gateway"]
