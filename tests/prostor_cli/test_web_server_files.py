...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("PROSTOR_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("PROSTOR_DASHBOARD_FILES_ROOT", raising=False)
...
def test_gated_local_mode_still_defaults_to_home(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("PROSTOR_HOME", str(home / ".prostor"))
