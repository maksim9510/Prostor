...
Regression coverage for [#26923](https://github.com/NousResearch/prostor-agent/issues/26923):
...
def test_xai_loopback_login_manual_paste_skips_http_server(monkeypatch):
    ...
    def _fake_prompt(_redirect_uri):
        # Prostor generates state internally; we won't know it ahead of
        # time, so capture the state Prostor baked into the authorize
...
def test_xai_wait_for_callback_accepts_ready_stdin_code(monkeypatch):
    """Users can paste the Grok Build code while Prostor is still waiting."""
    class _StubServer:
