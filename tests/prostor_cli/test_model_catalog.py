"""Tests for prostor_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate PROSTOR_HOME + reset any module-level catalog cache per test."""
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from prostor_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in prostor_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``prostor model`` CLI — not the live
        ...
        # ``PROSTOR_HOME / auth.json`` then resolves to the same path the
        ...
        importlib.reload(model_catalog)
        try:
            from prostor_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["PROSTOR_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("prostor_cli.models.check_nous_free_tier", return_value=False), patch(
                "prostor_cli.models.union_with_portal_free_recommendations",
