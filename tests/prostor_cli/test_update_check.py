"""Tests for the update check mechanism in prostor_cli.banner."""
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "prostor_cli" / "banner.py"
    ...
    monkeypatch.setenv("PROSTOR_HOME", str(tmp_path))
    monkeypatch.delenv("PROSTOR_REVISION", raising=False)
    with patch("prostor_cli.banner.subprocess.run") as mock_run, \
         patch("prostor_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PROSTOR_HOME", str(tmp_path))
    with patch("prostor_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:NousResearch/prostor-agent.git\n")
    ...
    with patch("prostor_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PROSTOR_HOME", str(tmp_path))
    with patch("prostor_cli.banner.subprocess.run") as mock_run:
        with patch("prostor_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when PROSTOR_HOME has no git repo."""
    import prostor_cli.banner as banner
    ...
    # Point PROSTOR_HOME at a temp dir with no prostor-agent/.git
    monkeypatch.setenv("PROSTOR_HOME", str(tmp_path))
    with patch("prostor_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    PROSTOR_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `prostor update` correctly refusing to run there. The guard
    ...
    with patch("prostor_cli.config.detect_install_method", return_value="docker"), \
         patch("prostor_cli.banner.subprocess.run") as mock_run, \
         patch("prostor_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("prostor_cli.config.detect_install_method", return_value="pip"), \
         patch("prostor_cli.banner.subprocess.run") as mock_run, \
         patch("prostor_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from prostor_cli.main import _invalidate_update_cache

    # Build a fake ~/.prostor with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"PROSTOR_HOME": str(default_home)}):
