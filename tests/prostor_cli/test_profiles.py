...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["prostor", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Prostor install itself or with
...
class TestGetProfileDir:
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".prostor" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".prostor" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.prostor must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".prostor"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes prostor-agent, .worktrees,
        ...
        (default_home / "prostor-agent" / ".git").mkdir(parents=True)
        (default_home / "prostor-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "prostor-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "prostor-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `prostor profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `prostor update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `prostor update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".prostor" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".prostor" / ".env").write_text("KEY=root\n")
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".prostor" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom PROSTOR_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("PROSTOR_HOME", str(custom))
        # With Docker-aware roots, a custom PROSTOR_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("prostor")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        ...
        assert "exec /opt/prostor/bin/prostor -p mybot" in content
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "prostor -p mybot" in content
        assert "%*" in content
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "prostor -p redqueen" in content
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".prostor" / "profiles" / "oldname"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "prostor.ssi_health": {"workspace": "prostor", "enabled": True}
            }
        }))
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("prostor-agent", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/prostor-agent", "default/.worktrees", "default/profiles",
...
class TestInternalHelpers:
    ...
    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (PROSTOR_HOME outside ~/.prostor), profiles go under PROSTOR_HOME."""
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (PROSTOR_HOME under ~/.prostor), profiles root is still ~/.prostor/profiles."""
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under PROSTOR_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("PROSTOR_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".prostor" / "profiles" / "mybot"
        assert result == expected
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        ...
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("PROSTOR_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
