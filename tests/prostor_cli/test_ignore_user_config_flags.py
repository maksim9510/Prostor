"""Tests for --ignore-user-config and --ignore-rules flags on `prostor chat`.
...
files. In Prostor the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.prostor/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("PROSTOR_IGNORE_USER_CONFIG", "PROSTOR_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("PROSTOR_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestCmdChatWiring:
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("PROSTOR_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("PROSTOR_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("PROSTOR_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("PROSTOR_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("PROSTOR_IGNORE_USER_CONFIG") == "1"
        assert "PROSTOR_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "PROSTOR_IGNORE_USER_CONFIG" not in os.environ
        assert "PROSTOR_IGNORE_RULES" not in os.environ
