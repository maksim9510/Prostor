...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("PROSTOR_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "PROSTOR_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="prostor-task", assignee="daily")
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Prostor-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``PROSTOR_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("PROSTOR_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Prostor' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Prostor' DB/metadata/logs on task completion.
    """
...
# spawned with `prostor -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's PROSTOR_HOME.
# ---------------------------------------------------------------------------
...
class TestSharedBoardPaths:
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.prostor/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.prostor/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.prostor/kanban.db.
        default_home = tmp_path / ".prostor"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # PROSTOR_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `prostor -p coder`).
    ...
    def test_explicit_override_via_prostor_kanban_home(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("PROSTOR_HOME", str(profile_home))
        monkeypatch.setenv("PROSTOR_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("PROSTOR_HOME", str(default_home))
        monkeypatch.setenv("PROSTOR_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # PROSTOR_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile PROSTOR_HOME and reads.
        monkeypatch.setenv("PROSTOR_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_prostor_kanban_db_pin_beats_kanban_home(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("PROSTOR_HOME", str(default_home))
        monkeypatch.setenv("PROSTOR_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("PROSTOR_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows PROSTOR_KANBAN_HOME -- the pins
        # are independent.
    ...
    def test_prostor_kanban_workspaces_root_pin_beats_kanban_home(
        ...
        monkeypatch.setenv("PROSTOR_HOME", str(default_home))
        monkeypatch.setenv("PROSTOR_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("PROSTOR_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows PROSTOR_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("PROSTOR_HOME", str(default_home))
        monkeypatch.setenv("PROSTOR_KANBAN_DB", "   ")
        monkeypatch.setenv("PROSTOR_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject PROSTOR_KANBAN_DB
        # and PROSTOR_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites PROSTOR_HOME.
        default_home = tmp_path / ".prostor"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["PROSTOR_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["PROSTOR_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["PROSTOR_KANBAN_TASK"] == "t_dispatch_env"
        assert env["PROSTOR_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'prostor kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# $PATH that doesn't include the venv's bin/, so a bare `["prostor", ...]`
...
def test_resolve_prostor_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "prostor.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("PROSTOR_BIN", raising=False)
...
def test_resolve_prostor_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    `python -m prostor` which fails with `No module named prostor` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_prostor_argv_module_actually_runs():
    ...
    assert "Prostor Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
