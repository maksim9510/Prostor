...
When ``PROSTOR_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.prostor/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.prostor            (has its own auth.json fixture)
    * Profile     -> tmp_path/.prostor/profiles/coder   (active, PROSTOR_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Prostor is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from prostor_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    prostor_home = tmp_path / "classic"
    prostor_home.mkdir()
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))

    _write(prostor_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (PROSTOR_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.prostor as the "real home" — which is NOT equal
    # to the PROSTOR_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    prostor_home = tmp_path / "classic"
    ...
    _write(prostor_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from prostor_cli.auth import read_credential_pool, write_credential_pool
