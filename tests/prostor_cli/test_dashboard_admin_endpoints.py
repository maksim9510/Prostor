    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("prostor-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # prostor-index source advertises availability + featured search.
            idx = _Src("prostor-index")
        ...
        assert {"official", "github", "prostor-index"} <= ids
...
class TestUpdateCheckEndpoint:
    """``GET /api/prostor/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/prostor/update`` runs ``prostor update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/prostor/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
