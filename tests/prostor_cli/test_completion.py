"""Tests for prostor_cli/completion.py — shell completion script generation."""
...
def _make_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(prog="prostor")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_prostor_completion()" in out
        assert "complete -F _prostor_completion prostor" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _prostor prostor' in out
        assert '_prostor "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_prostor_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[prostor]}} == _prostor ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c prostor -f" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_prostor_profiles()" in out
        assert 'profiles_dir="$HOME/.prostor/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'prostor profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _prostor_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_prostor_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _prostor_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_prostor_profiles()" in out
        assert "$HOME/.prostor/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__prostor_profiles" in out
        assert "$HOME/.prostor/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__prostor_profiles" in out
        count = out.count("(__prostor_profiles)")
