"""Unit tests for prostor_cli/dingtalk_auth.py (QR device-flow registration)."""
...
class TestApiPost:
    ...
    def test_raises_on_nonzero_errcode(self):
        ...
        with patch("prostor_cli.dingtalk_auth.requests.post", return_value=mock_resp):
            with pytest.raises(RegistrationError, match=r"boom \(errcode=42\)"):
                _api_post("/app/registration/init", {"source": "prostor"})

    def test_returns_data_on_success(self):
        from prostor_cli.dingtalk_auth import _api_post
        ...
        with patch("prostor_cli.dingtalk_auth.requests.post", return_value=mock_resp):
            result = _api_post("/app/registration/init", {"source": "prostor"})
...
class TestBeginRegistration:
    ...
    def test_chains_init_then_begin(self):
        from prostor_cli.dingtalk_auth import begin_registration
        ...
        with patch("prostor_cli.dingtalk_auth._api_post", side_effect=responses):
...
class TestWaitForSuccess:
    ...
    def test_returns_credentials_on_success(self):
        from prostor_cli.dingtalk_auth import wait_for_registration_success
        ...
        with patch("prostor_cli.dingtalk_auth.poll_registration", side_effect=responses), \
             patch("prostor_cli.dingtalk_auth.time.sleep"):
...
class TestRenderQR:
    ...
    def test_returns_false_when_qrcode_missing(self, monkeypatch):
        from prostor_cli import dingtalk_auth
    ...
    def test_prints_when_qrcode_available(self, capsys):
        ...
        from prostor_cli.dingtalk_auth import render_qr_to_terminal
