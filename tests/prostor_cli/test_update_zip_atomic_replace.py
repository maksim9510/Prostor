...
which broke ``prostor --tui`` because ``ui-tui/`` had vanished.
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.prostor-update-staging").exists()
    assert not (dst.parent / "ui-tui.prostor-update-old").exists()
...
def test_atomic_replace_clears_stale_staging_leftovers(tmp_path: Path) -> None:
    ...
    stale_staging = dst.parent / "ui-tui.prostor-update-staging"
    stale_backup = dst.parent / "ui-tui.prostor-update-old"
    stale_staging.mkdir()
    stale_backup.mkdir()
