...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "prostor gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`prostor gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `prostor gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `prostor gateway restart`)."""
        unit_path = tmp_path / "prostor-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``PROSTOR_HOME`` into the unit's
        ...
        # Realistic generated unit referencing a pytest tmpdir PROSTOR_HOME
        polluted_unit = (
            "[Service]\n"
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E PROSTOR_HOME like
        ``/tmp/prostor-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``prostor gateway restart`` with a /tmp PROSTOR_HOME exported; the
        ...
        must refuse ANY temp-dir PROSTOR_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "prostor-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="PROSTOR_HOME=/tmp/prostor-e2e-41264"\n'
            "WorkingDirectory=/tmp/prostor-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        ...
        assert (
            ...
            == "/tmp/prostor-e2e-41264"
        )

    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="PROSTOR_HOME=/var/tmp/prostor-x"\n'
    ...
    def test_accepts_macos_real_home_plist(self):
        plist = (
            "<dict>\n  <key>PROSTOR_HOME</key>\n"
            "  <string>/Users/alice/.prostor</string>\n</dict>\n"
        )
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="PROSTOR_HOME=/tmpfs-data/.prostor"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "prostor gateway install" in out
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("PROSTOR_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("PROSTOR_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("PROSTOR_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.prostor.gateway.plist"
        ...
        # PROSTOR_HOME resolves under the (pytest tmp) test PROSTOR_HOME.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist>--replace\n<key>PROSTOR_HOME</key>"
                "<string>/Users/alice/.prostor</string></plist>"
            ),
        )
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test PROSTOR_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>PROSTOR_HOME</key>"
                "<string>/Users/alice/.prostor</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup prostor gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: prostor gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup prostor gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.prostor.gateway"])
            ),
        )
...
class TestSystemUnitProstorHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'PROSTOR_HOME=/home/alice/.prostor' in unit
        assert '/root/.prostor' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: PROSTOR_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
    ...
    def test_system_unit_preserves_custom_prostor_home(self, monkeypatch):
        ...
        assert 'PROSTOR_HOME=/opt/prostor-shared' in unit
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "prostor gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.prostor/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".prostor" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".prostor"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile PROSTOR_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "prostor"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.prostor/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".prostor" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".prostor" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".prostor" / "profiles" / "mybot"
        ...
        assert f'PROSTOR_HOME={target_home / ".prostor" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".prostor" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.prostor.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".prostor" / "prostor-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".prostor" / "prostor-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/prostor", str(tmp_path / "alice"))
        assert result == "/opt/prostor"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".prostor" / "prostor-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".prostor" / "prostor-agent"
        ...
        # WorkingDirectory is anchored at the target user's PROSTOR_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.prostor" in unit
        assert "WorkingDirectory=/home/alice/.prostor/prostor-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "prostor-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "prostor-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'prostor gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'prostor gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'prostor gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyProstorUnitDetection:
    ...
    renamed to ``prostor-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``prostor.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``prostor-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_ignores_profile_unit_prostor_gateway_coder(self, tmp_path, monkeypatch):
        ...
        for base in (user_dir, system_dir):
            (base / "prostor-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "prostor-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "prostor-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_ignores_unrelated_prostor_service(self, tmp_path, monkeypatch):
        ...
        (user_dir / "prostor.service").write_text(
            "[Unit]\nDescription=Some Other Prostor\n[Service]\n"
            "ExecStart=/opt/other-prostor/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "prostor.service" in out
        assert "prostor gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "prostor.service"
...
class TestRemoveLegacyProstorUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop prostor.service" in c for c in cmds_joined)
        assert any("--user disable prostor.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo prostor gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop prostor.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable prostor.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "prostor.service"
        system_legacy = system_dir / "prostor.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (prostor-gateway-coder.service)
        ...
        profile_unit = user_dir / "prostor-gateway-coder.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `prostor gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `prostor gateway --help` shows it.
        import subprocess
        import sys
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "prostor-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "prostor-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "prostor-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``prostor gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "prostor-gateway")
        ...
        assert "sudo systemctl start prostor-gateway" in out
        assert "sudo prostor gateway uninstall --system" in out
        assert "prostor gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart prostor-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop prostor-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``prostor gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (PROSTOR_HOME), never the source checkout / worktree, so a relocated or
