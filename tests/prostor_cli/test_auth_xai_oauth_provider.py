"""Tests for xAI Grok OAuth — tokens stored in Prostor auth store (~/.prostor/auth.json)."""
...
def _setup_prostor_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Prostor auth store at the given root."""
...
def test_xai_oauth_authorize_url_includes_referrer_prostor_agent():
    """Attribution: xAI's OAuth server can identify Prostor-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "prostor-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "prostor auth add xai-oauth" in body
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    prostor_home = tmp_path / "prostor"
    _setup_prostor_auth(prostor_home, refresh_token="")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    prostor_home = tmp_path / "prostor"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_prostor_auth(prostor_home, access_token=fresh)
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    monkeypatch.delenv("PROSTOR_XAI_BASE_URL", raising=False)
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("prostor_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_prostor_auth(prostor_home, access_token=fresh)
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    monkeypatch.setenv("PROSTOR_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``PROSTOR_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    prostor_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    prostor_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (prostor_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    prostor_home = tmp_path / "prostor"
    _seed_xai_oauth_state(prostor_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    ...
    monkeypatch.setattr("prostor_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((prostor_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    prostor_home = tmp_path / "prostor"
    _seed_xai_oauth_state(prostor_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    ...
    monkeypatch.setattr("prostor_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``prostor model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``prostor model``
    ...
    assert "prostor model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from prostor_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Prostor that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("prostor_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `prostor model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `prostor auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from agent.credential_pool import load_pool

    prostor_home = tmp_path / "prostor"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_prostor_auth(prostor_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`prostor auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `prostor auth remove`.
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``prostor auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `prostor auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((prostor_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Prostor invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_prostor_auth(prostor_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    ...
    monkeypatch.setattr("prostor_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("prostor_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    prostor_home = tmp_path / "prostor"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_prostor_auth(prostor_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Prostor process refreshed the singleton
    ...
    profiles + Prostor processes."""
    from agent.credential_pool import load_pool

    prostor_home = tmp_path / "prostor"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_prostor_auth(prostor_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    ...
    (prostor_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_prostor_auth(prostor_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``prostor model`` -> xAI Grok OAuth (or another process
    ...
    _setup_prostor_auth(prostor_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    ...
    # Simulate the user re-running `prostor model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    _setup_prostor_auth(prostor_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`prostor auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``prostor auth status``, ``prostor setup``, and the
    ``prostor`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    prostor_home = tmp_path / "prostor"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_prostor_auth(prostor_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
