...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `prostor tools` → X (Twitter) Search to flip the
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `prostor tools`,
    ...
    config = {"platform_toolsets": {"cli": ["prostor-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[prostor-cli, spotify]`` (composite + configurable) must keep the full
    ``prostor-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``prostor-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when prostor-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["prostor-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``prostor-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["prostor-cli", "terminal"]}}
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (prostor-cli, prostor-telegram, etc.) must NOT
    ...
    ``prostor tools``, but prostor-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "prostor-cli",  # <-- the culprit
    ...
    # prostor-cli must NOT survive — it's a platform default, not an MCP server
    assert "prostor-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_prostor_telegram():
    ...
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "prostor-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "prostor-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "prostor-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `prostor tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's PROSTOR_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"PROSTOR_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"PROSTOR_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`prostor tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["prostor-_test_platform"] = {
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'prostor-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `prostor tools`,
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `prostor tools`, the toolset appears."""
...
#    reported as added/removed by `prostor tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `prostor tools` printed ``- kanban``
