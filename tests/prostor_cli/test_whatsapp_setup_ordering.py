...
Before the fix, ``prostor whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``prostor gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    prostor = home / ".prostor"
    prostor.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("PROSTOR_HOME", str(prostor))
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``prostor whatsapp`` with an existing paired session and
