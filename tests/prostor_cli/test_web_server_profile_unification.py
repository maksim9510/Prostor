...
profile switcher can target any profile's PROSTOR_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via PROSTOR_HOME.
"""
import pytest
import yaml
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_prostor_home(self, isolated_profiles, monkeypatch):
        ...
        assert env["PROSTOR_HOME"] == str(isolated_profiles["worker_beta"])
