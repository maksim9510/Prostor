...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``prostor auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``prostor auth add openai-codex``.
    ...
    (prostor_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `prostor auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``prostor auth add openai-codex`` for accounts B and C
    ...
    (prostor_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``prostor auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``prostor auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Prostor store
    assert not (codex_home / "auth.json").exists()

    # Prostor auth store should have the tokens
    data = _read_codex_tokens()
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    assert "prostor auth" not in rendered
