"""Tests for IRC gateway configuration via `prostor setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Prostor install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Prostor install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#prostor")
            monkeypatch.setenv("IRC_NICKNAME", "prostor-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `prostor setup gateway` experience with IRC present."""
