"""Tests for Bug #12905 fix — stale OAuth token detection in prostor model flow.

Bug 3: `prostor model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.prostor/.env but no valid
