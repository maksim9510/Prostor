...
The Python launcher (`prostor --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Prostor session
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["prostor", "--tui", "-c", "abc"])
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("PROSTOR_TUI", raising=False)
        monkeypatch.setenv("PROSTOR_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `prostor --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
