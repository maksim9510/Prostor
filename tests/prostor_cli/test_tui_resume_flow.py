...
def main_mod(monkeypatch):
    import prostor_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["prostor", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "prostor_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["prostor", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["prostor", "--tui", "--help"])
...
def test_fast_tui_launch_is_termux_only(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["prostor", "--tui"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PROSTOR_TUI", raising=False)
    monkeypatch.setattr(
        sys, "argv", ["prostor", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PROSTOR_TUI", raising=False)
    monkeypatch.delenv("PROSTOR_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("PROSTOR_FAST_STARTUP_BANNER", raising=False)
    ...
    assert os.environ["PROSTOR_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["PROSTOR_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["prostor", "-z", "hello", "--model", "gpt-test", "--provider", "openai"],
    )
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
...
def test_termux_fast_cli_launch_version_skips_update_check(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PROSTOR_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["prostor", "version"])
    monkeypatch.setattr(
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PROSTOR_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["prostor", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Prostor Agent v" in out
...
def test_termux_fast_cli_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PROSTOR_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["prostor", "chat", "--help"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("PROSTOR_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("PROSTOR_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["prostor", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_prostor_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("PROSTOR_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["prostor", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "prostor_cli.plugins",
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "prostor -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """prostor -z bypasses ProstorCLI, but recall still needs SessionDB."""
    from prostor_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "prostor_state", mod("prostor_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "prostor_cli.config",
        mod("prostor_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "prostor_cli.models",
        mod("prostor_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "prostor_cli.runtime_provider",
        mod(
            "prostor_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "prostor_cli.tools_config",
        mod("prostor_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["PROSTOR_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/prostor-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["PROSTOR_MODEL"] == "nous/prostor-test"
    assert env["PROSTOR_INFERENCE_MODEL"] == "nous/prostor-test"
    assert env["PROSTOR_TUI_PROVIDER"] == "nous"
    assert env["PROSTOR_INFERENCE_PROVIDER"] == "nous"
    assert env["PROSTOR_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["PROSTOR_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("prostor-tui-active-session-")
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_prostor_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/prostor-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("prostor_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("PROSTOR_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "PROSTOR_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["PROSTOR_TUI_RESUME"] == "20260518_000000_goodid"


def test_make_tui_argv_dev_prebuilds_prostor_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("PROSTOR_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "prostor_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "prostor --tui --resume 20260409_000001_abc123" in out
    assert 'prostor --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "prostor --tui --resume actual_session" in out
