...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `prostor gateway restart` spawned a
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the prostor user. The new
...
# unprivileged prostor user from every lifecycle op. These tests run
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "prostor -p coder gateway run" in run_text
    assert "s6-setuidgid prostor" in run_text
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid prostor prostor -p coder gateway run --replace" in run_text
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``prostor gateway run``, an agent action, the Open WebUI helper) holds
    the per-PROSTOR_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root PROSTOR_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `prostor gateway run --replace` (no -p flag).
    assert "prostor gateway run --replace" in default_text
    assert "prostor -p default" not in default_text
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "prostor profile create typo" in msg
...
#
# `prostor gateway stop` inside a container dispatches through
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    dropped prostor user and s6-log crash-loops on `mkdir: Permission denied`.
    """
    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.register_profile_gateway("coder")
    ...
    parent_chown = 'chown prostor:prostor "$PROSTOR_HOME/logs/gateways"'
    ...
    assert 'chown -R prostor:prostor "$PROSTOR_HOME/logs/gateways"' not in log_text
    ...
    leaf_idx = log_text.index('chown -R prostor:prostor "$log_dir"')
