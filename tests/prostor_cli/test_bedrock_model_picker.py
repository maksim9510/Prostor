...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (PROSTOR_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via PROSTOR_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in PROSTOR_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from prostor_cli.providers import PROSTOR_OVERLAYS
        assert "bedrock" in PROSTOR_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from prostor_cli.providers import PROSTOR_OVERLAYS
        assert PROSTOR_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from prostor_cli.providers import PROSTOR_OVERLAYS
        assert PROSTOR_OVERLAYS["bedrock"].auth_type == "aws_sdk"
