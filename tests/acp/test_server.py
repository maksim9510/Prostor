...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "prostor-agent"
        assert resp.agent_info.version == PROSTOR_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Prostor' interactive model/provider setup in a terminal. "
                    "Use this when Prostor has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Prostor provider",
                "type": "terminal",
            }
        ]
...
class TestPrompt:
    ...
    async def test_prompt_propagates_hermes_session_id_env(self, agent, monkeypatch):
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("PROSTOR_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "PROSTOR_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("PROSTOR_SESSION_ID") is None, (
            "PROSTOR_SESSION_ID must be restored after the agent call so "
    ...
    async def test_prompt_restores_prior_hermes_session_id(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("PROSTOR_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("PROSTOR_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("PROSTOR_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert PROSTOR_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["prostor-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["prostor-acp", "mcp-srv"]
