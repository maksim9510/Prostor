...
def _restore_tool_modules():
    original_prostor_home = os.environ.get("PROSTOR_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_prostor_home is None:
            os.environ.pop("PROSTOR_HOME", None)
        else:
            os.environ["PROSTOR_HOME"] = original_prostor_home
        _reset_modules(("tools", "prostor_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    os.environ["PROSTOR_HOME"] = str(prostor_home)
    sys.modules["prostor_cli.config"] = types.SimpleNamespace(
        get_prostor_home=lambda: prostor_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="prostor-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": prostor_home / "modal_snapshots.json",
