...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"PROSTOR_SESSION_KEY": "bob"}, clear=False):
...
class TestProstorConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.prostor/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.prostor/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.prostor/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.prostor/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.prostor/config.yaml"
        )
        assert dangerous is True
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.prostor/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.prostor/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.prostor/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.prostor/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.prostor/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.prostor/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Prostor config.yaml in a project dir is handled by the project
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.prostor/config.yaml/.env must require approval — the
...
class TestGatewayProtection:
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart prostor-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep prostor) bypasses the pkill/killall name-matching
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("PROSTOR_GATEWAY_SESSION", "PROSTOR_CRON_SESSION",
                      "PROSTOR_YOLO_MODE",
                      "PROSTOR_SESSION_KEY", "PROSTOR_INTERACTIVE")
        }
        os.environ.pop("PROSTOR_YOLO_MODE", None)
        os.environ.pop("PROSTOR_INTERACTIVE", None)
        # PROSTOR_CRON_SESSION takes priority over PROSTOR_GATEWAY_SESSION in
        ...
        os.environ.pop("PROSTOR_CRON_SESSION", None)
        os.environ["PROSTOR_GATEWAY_SESSION"] = "1"
        os.environ["PROSTOR_SESSION_KEY"] = self.SESSION_KEY
