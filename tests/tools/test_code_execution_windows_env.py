...
``prostor_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``PROSTOR_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # PROSTOR_* handling (#27303): only the operational allowlist passes;
        # every other PROSTOR_* is dropped (the broad prefix was removed).
        "PROSTOR_HOME": "/home/alice/.prostor",        # allowlisted → kept
        "PROSTOR_PROFILE": "default",                 # allowlisted → kept
        "PROSTOR_INTERACTIVE": "1",                   # not allowlisted → dropped
        "PROSTOR_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "PROSTOR_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``prostor_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``prostor_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        stub = generate_prostor_tools_module(["terminal"], transport="file")
...
# let the child import prostor_tools, a user script that printed non-ASCII
