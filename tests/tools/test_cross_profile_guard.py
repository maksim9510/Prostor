...
def fake_prostor(tmp_path, monkeypatch):
    """Build a two-profile Prostor layout and point PROSTOR_HOME at
    the prostor-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_prostor):
        from tools.file_tools import write_file_tool
        target = fake_prostor["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_prostor):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_prostor["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "prostor-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_prostor):
    ...
    def test_non_prostor_path_unaffected(self, fake_prostor, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_prostor):
    ...
    def test_cross_profile_patch_bypass(self, fake_prostor):
        from tools.file_tools import patch_tool
        target = fake_prostor["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_prostor):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        assert "not found in active profile 'prostor-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_prostor, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_prostor["root"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.prostor/profiles/<name>/."""
        # Don't set PROSTOR_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_prostor_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_prostor_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_prostor):
        """When active profile is 'prostor-security', the prompt warns
        ...
        assert "Active Prostor profile" in src
        assert "cross_profile=True" in src
        assert "~/.prostor/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Prostor profile: default" in src
        assert "Active Prostor profile: {active_profile}" in src
