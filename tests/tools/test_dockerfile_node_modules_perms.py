...
Older images made /opt/prostor/ui-tui and /opt/prostor/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV PROSTOR_TUI_DIR=/opt/prostor/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R prostor:prostor /opt/prostor/ui-tui" not in text
    assert "chown -R prostor:prostor /opt/prostor/node_modules" not in text
