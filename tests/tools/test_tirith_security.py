...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._prostor_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(prostor_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(prostor_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_prostor_bin(self):
        ...
        with open(prostor_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(prostor_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._prostor_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == prostor_bin
            assert _tirith_mod._resolved_path == prostor_bin
            mock_clear.assert_called_once()
...
class TestProstorHomeIsolation:
    ...
    def test_failure_marker_respects_prostor_home(self):
        """_failure_marker_path must use PROSTOR_HOME, not hardcoded ~/.prostor."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"PROSTOR_HOME": "/custom/prostor"}):
            result = _failure_marker_path()
        assert result == "/custom/prostor/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets PROSTOR_HOME; verify it's active."""
        prostor_home = os.getenv("PROSTOR_HOME")
        assert prostor_home is not None, "PROSTOR_HOME should be set by conftest"
        assert "prostor_test" in prostor_home, "Should point to test temp dir"

    def test_get_prostor_home_fallback(self):
        """Without PROSTOR_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_prostor_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove PROSTOR_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".prostor")
            result = _get_prostor_home()
