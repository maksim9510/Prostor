...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad PROSTOR_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: PROSTOR_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("PROSTOR_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("PROSTOR_CRON_SESSION", "1")
    monkeypatch.delenv("PROSTOR_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_prostor_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "PROSTOR_HOME": "/h", "PROSTOR_PROFILE": "p",
        "PROSTOR_CONFIG": "/c.yaml", "PROSTOR_ENV": "/e",
        # other PROSTOR_* → dropped (broad prefix removed)
        "PROSTOR_BASE_URL": "https://x", "PROSTOR_INTERACTIVE": "1",
        "PROSTOR_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("PROSTOR_HOME", "PROSTOR_PROFILE", "PROSTOR_CONFIG", "PROSTOR_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "PROSTOR_BASE_URL", "PROSTOR_INTERACTIVE", "PROSTOR_KANBAN_DB",
...
def test_env_scrub_logs_dropped_prostor_vars(caplog):
    """Dropping a non-allowlisted, non-secret PROSTOR_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset PROSTOR_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "PROSTOR_HOME": "/h",          # allowlisted → kept, not logged
        "PROSTOR_BASE_URL": "https://x",   # dropped → logged
        "PROSTOR_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "PROSTOR_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "PROSTOR_HOME" in out and "PATH" in out
    assert "PROSTOR_BASE_URL" not in out and "PROSTOR_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "PROSTOR_BASE_URL" in msgs and "PROSTOR_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "PROSTOR_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped PROSTOR_* vars."""
    import logging
