...
Hosted/container Prostor keeps user-writable state under PROSTOR_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/prostor must remain root-owned/non-writable by the runtime prostor user so
...
def test_stage2_does_not_chown_install_tree_to_prostor(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to prostor; "
            "the Dockerfile keeps /opt/prostor immutable and writable state "
            "belongs under PROSTOR_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "PROSTOR_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/prostor" in stage2_text
