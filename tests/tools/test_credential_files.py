...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        prostor_home.mkdir()
        (prostor_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(prostor_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.prostor/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        (prostor_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        (prostor_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        (prostor_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.prostor/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        prostor_home = tmp_path / ".prostor"
        (prostor_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"PROSTOR_HOME": str(prostor_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.prostor")

        assert mounts[0]["container_path"] == "/home/user/.prostor/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.prostor/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.prostor/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape PROSTOR_HOME."""
        monkeypatch.setenv("PROSTOR_HOME", str(tmp_path / ".prostor"))
        (tmp_path / ".prostor").mkdir()

        # Create a sensitive file one level above prostor_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside prostor_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside PROSTOR_HOME must still be registered."""
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
        (prostor_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_prostor_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of PROSTOR_HOME must be allowed."""
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        subdir = prostor_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside PROSTOR_HOME pointing outside must be rejected."""
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))

        # Create a sensitive file outside prostor_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside prostor_home pointing outside
        symlink = prostor_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes PROSTOR_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, prostor_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape PROSTOR_HOME."""
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
        ...
        sensitive.write_text("{}")
        self._write_config(prostor_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(prostor_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside PROSTOR_HOME via config must still mount."""
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))

        (prostor_home / "oauth.json").write_text("{}")
        self._write_config(prostor_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        (prostor_home / "cache" / "documents").mkdir(parents=True)
        (prostor_home / "cache" / "audio").mkdir(parents=True)
        (prostor_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.prostor/cache/documents" in paths
        assert "/root/.prostor/cache/audio" in paths
        assert "/root/.prostor/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.prostor/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir()
        # Use legacy dir name — get_prostor_dir prefers old if it exists
        (prostor_home / "document_cache").mkdir()
        (prostor_home / "image_cache").mkdir()
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(prostor_home / "document_cache") in host_paths
        assert str(prostor_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.prostor/cache/documents" in container_paths
        assert "/root/.prostor/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        prostor_home = tmp_path / ".prostor"
        img_dir = prostor_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.prostor/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.prostor")
            == "/home/agent/.prostor/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        prostor_home = tmp_path / ".prostor"
        (prostor_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(prostor_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        prostor_home = tmp_path / ".prostor"
        doc_dir = prostor_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        prostor_home = tmp_path / ".prostor"
        ss_dir = prostor_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.prostor/cache/screenshots/session_abc/screen1.png"
