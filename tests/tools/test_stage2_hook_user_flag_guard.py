...
guidance, while still allowing:
  ...
  - root start (id -u == 0)
  - `--user <prostor-uid>` (the supported non-root start, #34648 / #34837)
...
root, and the baked image dirs are owned by the prostor build UID, so an
arbitrary pinned UID can't write them (EACCES on a bind mount, hard crash on a
named volume). The supported path is root start + PROSTOR_UID/PROSTOR_GID (or the
PUID/PGID aliases), which remaps the prostor user and chowns the volume.

The guard fires only when the current UID is neither root NOR the prostor UID,
so the #34648 `--user 10000:10000` case (pinning to the prostor UID itself) is
unaffected.
...
def _guard_block(text: str) -> str:
    ...
    assert m, "expected the --user guard block (cur_uid + non-root/non-prostor check)"
    return m.group(1)
...
def test_guard_present_and_mentions_remediation(path: Path) -> None:
    ...
    # Must check non-root AND non-prostor-uid (so --user 10000:10000 is allowed).
    assert '"$cur_uid" != 0' in block
    assert '"$cur_uid" != "$(id -u prostor)"' in block
    assert "exit 1" in block
    # Must point users at the supported env vars.
    assert "PROSTOR_UID" in block and "PROSTOR_GID" in block
