"""Contract test: the s6-overlay stage2 hook seeds $PROSTOR_HOME/logs/gateways
as the prostor user.
...
prostor user and s6-log crash-loops on `mkdir: Permission denied`.

Seeding `logs/gateways` in stage2 (cont-init runs before any service starts)
guarantees the parent already exists prostor-owned by the time the first
...
def test_logs_gateways_is_seeded(stage2_text: str) -> None:
    ...
    assert '"$PROSTOR_HOME/logs/gateways"' in block, (
        "logs/gateways must be seeded prostor-owned in stage2 so profiles "
    ...
    assert '"$PROSTOR_HOME/logs"' in block
