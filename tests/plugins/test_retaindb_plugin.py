...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure PROSTOR_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", prostor_home=str(tmp_path / ".prostor"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", prostor_home=profile_home)
        assert p._client.project == "prostor-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".prostor" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        prostor_home = tmp_path / ".prostor"
        prostor_home.mkdir(exist_ok=True)
        monkeypatch.setenv("PROSTOR_HOME", str(prostor_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", prostor_home=str(prostor_home))
        return p
