...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate PROSTOR_HOME for each test.

    The global hermetic fixture already redirects PROSTOR_HOME to a tempdir,
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "prostor-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "prostor-agent":
                raise AssertionError("quick() descended into protected prostor-agent/")
