...
Run from the PR worktree:
    ...
    cd ~/.prostor/prostor-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.prostor/prostor-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.prostor/prostor-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a prostor-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a prostor-agent checkout"
)
...
# Isolated PROSTOR_HOME for the config write.
