...
package-lock.json. Without PROSTOR_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV PROSTOR_TUI_DIR=/opt/prostor/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the prostor user, return stdout."""
    inner = (
        "source /opt/prostor/.venv/bin/activate && "
        "cd /opt/prostor && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the prostor user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "prostor", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$PROSTOR_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/prostor/ui-tui", (
        f"PROSTOR_TUI_DIR={r.stdout.strip()!r} (expected /opt/prostor/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    # With PROSTOR_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
