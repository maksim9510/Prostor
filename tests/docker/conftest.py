...
Override the image with ``PROSTOR_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("PROSTOR_TEST_IMAGE", "prostor-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``PROSTOR_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("PROSTOR_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged prostor user
# ---------------------------------------------------------------------------
#
# Background: every Prostor runtime path inside the container drops to UID
# 10000 (the ``prostor`` user) via ``s6-setuidgid prostor``. ``docker exec``
...
# fails (PermissionError swallowed) for prostor, so a test that ran as root
...
# helpers below run every probe under ``-u prostor`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: prostor).
