...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves prostor_cli.kanban_db on its own
  ...
  - worker logs are captured to PROSTOR_HOME/kanban/logs/<task>.log
...
def main():
    home = tempfile.mkdtemp(prefix="prostor_e2e_")
    os.environ["PROSTOR_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from prostor_cli import kanban_db as kb

    # Point the `prostor` CLI child processes will run at the worktree
    # prostor_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "prostor")
...
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/prostor_e2e_*/worker_*.log
    import glob
