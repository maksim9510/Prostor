...
Reads PROSTOR_KANBAN_TASK from env, heartbeats periodically, does short
work, completes via the CLI. Designed to be spawned by the dispatcher
exactly the way `prostor chat -q` would be, minus the LLM cost.
"""
...
def main():
    tid = os.environ["PROSTOR_KANBAN_TASK"]
    workspace = os.environ.get("PROSTOR_KANBAN_WORKSPACE", "")

    # Announce via CLI (goes through real argparse + init_db + etc)
    subprocess.run(
        ["prostor", "kanban", "heartbeat", tid, "--note", "started"],
