...
normal tests assume away:
  ...
  - Filesystem: PROSTOR_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own PROSTOR_HOME, collect failures.
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "PROSTOR_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        try:
            ...
            home_abs = str(Path(os.environ["PROSTOR_HOME"]).resolve())
...
def _(home, kb):
    ...
    weird = tempfile.mkdtemp(prefix="prostor with spaces ")
    ...
    try:
        ...
        print(f"  PROSTOR_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    ...
    try:
        ...
        print(f"  PROSTOR_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same PROSTOR_HOME share DB correctly")
    finally:
