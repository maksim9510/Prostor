...
``prostor_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``prostor_cli.config`` (get_env_value / save_env_value) and
...
def _patch_setup_io(monkeypatch, prompts, saved):
    ...
    # Manifest writing reaches out to prostor_cli.slack_cli + filesystem; stub it.
    import prostor_cli.slack_cli as slack_cli_mod
