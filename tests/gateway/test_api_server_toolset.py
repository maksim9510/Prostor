...
class TestHermesApiServerToolset:
    ...
    def test_toolset_exists(self):
        ts = get_toolset("prostor-api-server")
        assert ts is not None

    def test_toolset_validates(self):
        assert validate_toolset("prostor-api-server")

    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("prostor-api-server")
...
class TestApiServerAdapterToolset:
    ...
    def test_create_agent_reads_config_toolsets(self):
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
            ...
            # No platform_toolsets override — should fall back to prostor-api-server default
