...
($INSTALL_DIR), not into the shared $PROSTOR_HOME.

Background (shared-$PROSTOR_HOME bug)
------------------------------------
$PROSTOR_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.prostor:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$PROSTOR_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``prostor update``).
...
``~/.prostor/prostor-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$PROSTOR_HOME/\.install_method"', text), (
        "install.sh must not stamp $PROSTOR_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `prostor update`"
    )
