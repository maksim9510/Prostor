...
class TestProstorConstantsFallback:
    """Tests for _prostor_home.py fallback when prostor_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_prostor_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _prostor_home.py with prostor_constants blocked."""
        monkeypatch.setitem(sys.modules, "prostor_constants", None)
        spec = importlib.util.spec_from_file_location("_prostor_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_prostor_home_env_var(self, monkeypatch, tmp_path):
        """When prostor_constants is missing, PROSTOR_HOME comes from env var."""
        monkeypatch.setenv("PROSTOR_HOME", str(tmp_path / "custom-prostor"))
        module = self._load_helper(monkeypatch)
        assert module.get_prostor_home() == tmp_path / "custom-prostor"

    def test_fallback_defaults_to_dot_prostor(self, monkeypatch):
        """When prostor_constants is missing and PROSTOR_HOME unset, default to ~/.prostor."""
        monkeypatch.delenv("PROSTOR_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_prostor_home() == Path.home() / ".prostor"

    def test_fallback_ignores_empty_prostor_home(self, monkeypatch):
        """Empty/whitespace PROSTOR_HOME is treated as unset."""
        monkeypatch.setenv("PROSTOR_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_prostor_home() == Path.home() / ".prostor"

    def test_fallback_display_prostor_home_shortens_path(self, monkeypatch):
        """Fallback display_prostor_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("PROSTOR_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_prostor_home() == "~/.prostor"

    def test_fallback_display_prostor_home_profile_path(self, monkeypatch):
        """Fallback display_prostor_home() handles profile paths under ~/."""
        monkeypatch.setenv("PROSTOR_HOME", str(Path.home() / ".prostor/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_prostor_home() == "~/.prostor/profiles/coder"

    def test_fallback_display_prostor_home_custom_path(self, monkeypatch):
        """Fallback display_prostor_home() returns full path for non-home locations."""
        monkeypatch.setenv("PROSTOR_HOME", "/opt/prostor-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_prostor_home() == "/opt/prostor-custom"

    def test_delegates_to_prostor_constants_when_available(self):
        """When prostor_constants IS importable, _prostor_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_prostor_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import prostor_constants
        assert module.get_prostor_home is prostor_constants.get_prostor_home
        assert module.display_prostor_home is prostor_constants.display_prostor_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Prostor Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "prostor-agent[google]" in out
