...
class TestBuildContextFilesPrompt:
    def test_empty_dir_loads_seeded_global_soul(self, tmp_path):
        ...
        assert "Prostor Agent" in result
    ...
    def test_hermes_md_parent_dir_discovery(self, tmp_path):
        ...
        (tmp_path / ".git").mkdir()
        (tmp_path / ".prostor.md").write_text("Root project rules.")
    ...
    def test_hermes_md_beats_agents_md(self, tmp_path):
        ...
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Prostor project rules" in result
...
class TestFindHermesMd:
    ...
    def test_stops_at_git_root(self, tmp_path):
        ...
        (tmp_path / ".prostor.md").write_text("outside")
        repo = tmp_path / "repo"
        repo.mkdir()
        (repo / ".git").mkdir()
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """PROSTOR_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("PROSTOR_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
