...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Prostor passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("PROSTOR_HOME", "/users/alice/.prostor/profiles/backend-worker")
        monkeypatch.setenv("PROSTOR_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "PROSTOR_KANBAN_DB",
            "/users/alice/.prostor/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.prostor/kanban/boards/smoke"]'
            in cmd
        )
