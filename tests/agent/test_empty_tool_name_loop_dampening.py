...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated PROSTOR_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="prostor_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".prostor"))
    prev_home = os.environ.get("PROSTOR_HOME")
    os.environ["PROSTOR_HOME"] = os.path.join(test_home, ".prostor")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("prostor_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["PROSTOR_HOME"] = prev_home
