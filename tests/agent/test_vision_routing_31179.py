...
def isolated_home(monkeypatch):
    """Temp PROSTOR_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="prostor_test_31179_")
    prostor_home = os.path.join(test_home, ".prostor")
    os.makedirs(prostor_home)
...
def _fresh_modules():
    """Drop cached prostor modules so each test reloads against current env."""
