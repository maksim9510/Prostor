...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hermes_env(self):
        ...
        # even ~/.prostor/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.prostor/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Prostor cache files must remain blocked."""
