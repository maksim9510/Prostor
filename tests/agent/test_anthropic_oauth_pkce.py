...
History:
  - PR #1775 first fixed this on ``run_prostor_oauth_login()``.
  - PR #2647 (b17e5c10) added ``run_prostor_oauth_login_pure()`` and silently
...
def _patch_oauth_flow(
    ...
    *,
    ...
    monkeypatch.setattr(
        "prostor_cli.auth._can_open_graphical_browser", lambda: True
    )
...
def test_authorization_url_state_is_not_pkce_verifier(monkeypatch, tmp_path):
    ...
    from agent.anthropic_adapter import run_prostor_oauth_login_pure

    result = run_prostor_oauth_login_pure()
