"""Tests for the cross-Prostor-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Prostor profile's
...
Reference: May 2026 incident — a prostor-security profile session
accidentally edited skills under both ~/.prostor/profiles/prostor-security/skills/
AND ~/.prostor/skills/ (the default profile's skills), realizing only
...
@pytest.fixture
def fake_prostor(tmp_path, monkeypatch):
    """Build a fake Prostor layout:
    ...
    sec_home = root / "profiles" / "prostor-security"
    ...
    monkeypatch.setattr(prostor_constants, "get_default_prostor_root", lambda: root)
...
def _set_active_home(monkeypatch, prostor_home: Path):
    """Point file_safety._prostor_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_prostor_home_path", lambda: prostor_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_prostor, monkeypatch):
        _set_active_home(monkeypatch, fake_prostor["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_prostor, monkeypatch):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "prostor-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_prostor, monkeypatch):
        """If PROSTOR_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_prostor_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_prostor, monkeypatch):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_prostor["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_prostor, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_prostor["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "prostor-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_prostor, monkeypatch):
        ...
        assert result["target_profile"] == "prostor-security"

    def test_named_to_named_cross_profile(self, fake_prostor, monkeypatch):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_prostor["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_prostor, monkeypatch, area):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_prostor["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_prostor_path_returns_none(self, fake_prostor, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Prostor root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_prostor_config_not_classified_as_cross_profile(self, fake_prostor, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_prostor["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_prostor, monkeypatch):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_prostor, monkeypatch):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "prostor-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_prostor, monkeypatch):
        _set_active_home(monkeypatch, fake_prostor["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_prostor["default_home"] / "skills" / "foo" / "SKILL.md")
