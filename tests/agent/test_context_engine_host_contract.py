...
engine plugins (e.g. prostor-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``prostor plugins`` picker."""
    from prostor_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "prostor_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "prostor_cli.plugins.get_plugin_context_engine",
