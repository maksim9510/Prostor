...
for the credential stores while leaving arbitrary ``PROSTOR_HOME`` files
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Prostor cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into PROSTOR_HOME's
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at PROSTOR_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == PROSTOR_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at PROSTOR_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in PROSTOR_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_prostor_files_outside_home_not_blocked(
    ...
    outside PROSTOR_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside PROSTOR_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside PROSTOR_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, PROSTOR_HOME = <root>/profiles/<name>, but
