...
def _resolve_prostor_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.prostor",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``PROSTOR_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside PROSTOR_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    prostor_home = _resolve_prostor_home()

    # Reject absolute paths — they bypass the PROSTOR_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to PROSTOR_HOME)",
            relative_path,
        )
        return False

    host_path = prostor_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape PROSTOR_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, prostor_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.prostor/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="prostor-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching prostor_constants.get_prostor_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_prostor_dir()`` for backward compatibility with old directory layouts.
    """
    from prostor_constants import get_prostor_dir

    mounts: list[dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_prostor_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.prostor``,
    SSH ``<remote_home>/.prostor``, etc.) and whether translation is wanted.
