...
`health_report` (surfaced by `prostor computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Prostor runs in).
_BLOCKED_KEY_COMBOS = {
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("PROSTOR_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown PROSTOR_COMPUTER_USE_BACKEND={backend_name!r}")
            try:
                _backend.start()
            except Exception:
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `prostor computer-use install`. "
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/prostor-agent#47072 — trycua/cua#1961
...
def check_computer_use_requirements() -> bool:
    ...
    `prostor computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
