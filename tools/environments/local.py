...
# Bedrock-specific bearer token, which is a Prostor inference secret exactly
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Prostor-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("PROSTOR_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. Prostor Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set PROSTOR_GIT_BASH_PATH to your bash.exe location."
    )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Prostor trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``PROSTOR_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under PROSTOR_HOME.  Using
