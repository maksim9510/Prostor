...
def _get_active_profile_name() -> str:
    """Return the active Prostor profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale prostor-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=prostor-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Prostor process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=prostor-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a prostor process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Prostor
    process leaves its container behind permanently because there's no
    subsequent Prostor process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=prostor-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=prostor-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``PROSTOR_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("PROSTOR_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using PROSTOR_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'prostor'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``prostor-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.prostor/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. prostor-agent:latest) already use /init as PID 1
        ...
        container_name = f"prostor-{uuid.uuid4().hex[:8]}"
        # Labels make prostor-created containers identifiable to:
        #   * the orphan reaper (`prostor-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`prostor-task-id`, `prostor-profile`)
        #   * operators running `docker ps --filter label=prostor-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Prostor profile is captured at
        ...
        label_args = [
            "--label", "prostor-agent=1",
            "--label", f"prostor-task-id={task_label}",
            "--label", f"prostor-profile={profile_name}",
        ]
        ...
        self._labels = {
            "prostor-agent": "1",
            "prostor-task-id": task_label,
            "prostor-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Prostor process
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("prostor-task-id", "")
        profile_label = self._labels.get("prostor-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"prostor-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``prostor-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=prostor-agent=1",
                    "--filter", f"label=prostor-task-id={task_label}",
                    "--filter", f"label=prostor-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Prostor process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Prostor exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Prostor process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Prostor startup. That covers the
        ...
        # outlives Prostor processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"prostor-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``prostor /quit``
