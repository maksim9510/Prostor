...
提供以下工具函数，供 prostor-agent 的 "prostor-yuanbao" toolset 使用：
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（PROSTOR_SESSION_CHAT_ID）。
...
def _check_yuanbao():
    ...
    try:
        from gateway.session_context import get_session_env
        if get_session_env("PROSTOR_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
