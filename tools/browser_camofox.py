...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.prostor/.env``.
...
def _managed_persistence_enabled() -> bool:
    """Return whether Prostor-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: str | None, camofox_cfg: dict[str, Any]) -> dict[str, str] | None:
    ...
    so Prostor operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: dict[str, Any]) -> bool:
    """Return whether Prostor should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for Prostor talking to the
...
def _adopt_existing_tab(session: dict[str, Any]) -> dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside Prostor. Gateway
...
def _get_session(task_id: str | None) -> dict[str, Any]:
    ...
    derived from the Prostor profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if identity_override:
            ...
            session = {
                "user_id": f"prostor_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_prostor_home() / "browser_screenshots"
