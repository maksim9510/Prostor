...
Backend is selected during ``prostor tools`` setup (web.backend in config.yaml).
When available, Prostor can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Prostor config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Prostor' config/.env layer (``prostor config set``, ``prostor tools``)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``prostor tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `prostor tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `prostor tools` to set one up."
                ),
            }
        else:
            logger.info(
