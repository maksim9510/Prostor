...
`mcp_tool.py`, and `prostor_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ProstorMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_prostor_provider_class() -> Optional[type]:
    ...
    class ProstorMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._prostor_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ProstorTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ProstorTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._prostor_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ProstorTokenStorage
                        if isinstance(storage, ProstorTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._prostor_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ProstorTokenStorage
            if not isinstance(storage, ProstorTokenStorage):
                return
    ...
    return ProstorMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_PROSTOR_PROVIDER_CLS: Optional[type] = _make_prostor_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ProstorMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `prostor mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``prostor mcp remove <name>`` and (indirectly) by
        ``prostor mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
