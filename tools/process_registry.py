...
CHECKPOINT_PATH = get_prostor_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/prostor_bg_{session.id}.log"
        pid_path = f"{temp_dir}/prostor_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/prostor_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `prostor update` restarting the gateway) is still
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Prostor'}"
    elif _reason == "lost":
