...
~/.prostor/skills/. Existing skills (bundled, hub-installed, or user-created)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`prostor curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> dict[str, Any] | None:
    ...
    Searches the local skills dir (~/.prostor/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> list[tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Prostor profiles.
    ...
    # Default profile (~/.prostor/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.prostor/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`prostor -p "
            ...
            base += (
                ...
                f"Switch profiles (`prostor -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `prostor curator unpin <name>`. "
