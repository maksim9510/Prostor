#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Prostor Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.prostor/skills/ during setup.  They are discoverable via the Skills Hub
...
def _load_prostor_index() -> dict | None:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for PROSTOR_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if PROSTOR_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - PROSTOR_INDEX_CACHE_FILE.stat().st_mtime
            if age < PROSTOR_INDEX_TTL:
                return json.loads(PROSTOR_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(PROSTOR_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Prostor index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Prostor index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        PROSTOR_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        PROSTOR_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ProstorIndexSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> SkillBundle | None:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "prostor-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "prostor-index"),
            identifier=entry.get("identifier", ""),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "prostor-index"
