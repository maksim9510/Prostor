...
provider** — turn on a backend by enabling a plugin (``prostor plugins
enable video_gen/<name>``) and selecting it in ``prostor tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: dict[str, Any] = {
    ...
    # skill at github/prostor-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `prostor tools` "
...
def _missing_provider_error(configured: str | None) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `prostor tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `prostor tools` → "
...
# `prostor tools` or `/skills`, the schema rebuilds automatically.
...
def _build_dynamic_video_schema() -> dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `prostor tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
