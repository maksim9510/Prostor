...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.prostor/config.yaml``. Prostor
...
Configuration is loaded from ~/.prostor/config.yaml under the 'tts:' key.
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.prostor/config.yaml
# ===========================================================================
def _load_tts_config() -> dict[str, Any]:
    """
    Load TTS configuration from ~/.prostor/config.yaml.
...
# custom voice-cloning scripts, etc.) into Prostor without any Python code
...
#           output_format: wav
#
# Prostor writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `prostor model` or set XAI_API_KEY.")
    ...
    # output_format when Prostor actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Prostor caches Piper voice models.
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.prostor/config.yaml (tts: section).
    ...
    platform = get_session_env("PROSTOR_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'prostor-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run prostor setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'prostor setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'prostor tools' and select Piper under TTS, or install manually: "
