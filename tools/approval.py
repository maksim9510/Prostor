...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("PROSTOR_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("PROSTOR_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("PROSTOR_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("PROSTOR_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set PROSTOR_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind PROSTOR_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds PROSTOR_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("PROSTOR_CRON_SESSION"):
        return False
...
# active profile home path such as /home/prostor/.prostor/config.yaml. The
# resolved-absolute form is folded into the ~/.prostor/ patterns at detection
...
# go stale when PROSTOR_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_PROSTOR_ENV_PATH = (
    r'(?:~\/.prostor/|'
    r'(?:\$home|\$\{home\})/.prostor/|'
    r'(?:\$prostor_home|\$\{prostor_home\})/)'
    r'\.env\b'
)
# ~/.prostor/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _PROSTOR_ENV_PATH; matches the PROSTOR_HOME override form as
# well as ~/.prostor/.
_PROSTOR_CONFIG_PATH = (
    r'(?:~\/.prostor/|'
    r'(?:\$home|\$\{home\})/.prostor/|'
    r'(?:\$prostor_home|\$\{prostor_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_PROSTOR_ENV_PATH}|'
    rf'{_PROSTOR_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bprostor\s+gateway\s+(stop|restart)\b', "stop/restart prostor gateway (kills running agents)"),
    (r'\bprostor\s+update\b', "prostor update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `prostor gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart prostor-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart prostor-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(prostor|gateway|cli\.py)\b', "kill prostor/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill prostor` but not
    # `kill -9 $(pgrep -f prostor)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Prostor file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.prostor/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Prostor-managed security file (~/.prostor/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_PROSTOR_CONFIG_PATH}|{_PROSTOR_ENV_PATH})', "in-place edit of Prostor config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_PROSTOR_CONFIG_PATH}|{_PROSTOR_ENV_PATH})', "in-place edit of Prostor config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_PROSTOR_CONFIG_PATH}|{_PROSTOR_ENV_PATH})', "in-place edit of Prostor config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.prostor/ form so the Prostor config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/prostor/.prostor/config.yaml`) rather than
    # ~, $HOME, or $PROSTOR_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live PROSTOR_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_prostor_home(command)
    return command
...
def _rewrite_resolved_prostor_home(command: str) -> str:
    """Rewrite the resolved absolute Prostor home prefix to ``~/.prostor/``.

    Resolves the active ``PROSTOR_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.prostor/`` so the static ``_PROSTOR_CONFIG_PATH`` / ``_PROSTOR_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from prostor_constants import get_prostor_home
        home = get_prostor_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate PROSTOR_HOME (e.g. "/" or "") rewriting
        ...
        # /home/prostor/.prostor or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.prostor/")
    return command
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("PROSTOR_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set PROSTOR_INTERACTIVE or PROSTOR_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("PROSTOR_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("PROSTOR_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("PROSTOR_EXEC_ASK")
