...
transport, discovers their tools, and registers them into the prostor-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.prostor/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.prostor/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Prostor Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Prostor MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Prostor gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Prostor' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (PROSTOR_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit PROSTOR_SESSION_PLATFORM, so the
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Prostor' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Prostor is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `prostor mcp login {server_name}` (or delete the tokens "
            f"file under ~/.prostor/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local PROSTOR_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local PROSTOR_HOME override onto the MCP loop.
...
def _load_mcp_config() -> dict[str, dict]:
    """Read ``mcp_servers`` from the Prostor config file.
    ...
    try:
        ...
        if _env_enabled("PROSTOR_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Prostor' MEDIA: tag convention and out to messaging
            ...
            # Prostor' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Prostor's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Prostor registry schema format.
...
def probe_mcp_server_tools() -> dict[str, list[tuple]]:
    ...
    Designed for ``prostor tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Prostor registry.
...
def shutdown_mcp_servers():
    ...
    async def _shutdown():
        ...
        for server, result in zip(servers_snapshot, results, strict=False):
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple prostor
