...
`prostor skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.prostor.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> BlueprintSpec | None:
    ...
    prostor = meta.get("prostor") if isinstance(meta, dict) else None
    blueprint = prostor.get("blueprint") if isinstance(prostor, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.prostor.blueprint must be a mapping")
...
def export_blueprint(job: dict[str, Any], body: str, *, blueprint_name: str | None = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.prostor.blueprint`` block) they can hand
    to ``prostor skills publish`` to share. ``body`` is the plain-language
