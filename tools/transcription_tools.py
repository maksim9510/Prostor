...
LOCAL_STT_COMMAND_ENV = "PROSTOR_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "PROSTOR_LOCAL_STT_LANGUAGE"
...
#
# The single-env-var ``PROSTOR_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"prostor-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set PROSTOR_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="prostor-local-stt-") as output_dir:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `prostor model` or set XAI_API_KEY",
        }
    ...
    language = str(
        ...
        or os.getenv("PROSTOR_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
