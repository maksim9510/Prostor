...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/prostor/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when prostor
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# prostor process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main prostor process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Prostor WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via PROSTOR_UID at runtime
RUN useradd -u 10000 -m -d /opt/data prostor
...
#
# ui-tui/packages/prostor-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/prostor-ink/ ui-tui/packages/prostor-ink/
...
# lock on the @prostor/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/prostor/.venv at first
...
# ---------- Permissions ----------
# Link prostor-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Keep /opt/prostor immutable for the runtime prostor user. Hosted/container
...
# supervised Prostor processes drop to the non-root prostor user.
USER root
RUN mkdir -p /opt/prostor/bin && \
    cp /opt/prostor/docker/prostor-exec-shim.sh /opt/prostor/bin/prostor && \
    chmod 0755 /opt/prostor/bin/prostor && \
    printf 'docker\n' > /opt/prostor/.install_method && \
    chown -R root:root /opt/prostor && \
    chmod -R a+rX /opt/prostor && \
    chmod -R a-w /opt/prostor
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $PROSTOR_HOME. $PROSTOR_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``prostor update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the prostor user via
# `s6-setuidgid prostor` in its run script. If PROSTOR_UID is unset, services
# run as the default prostor user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `prostor dump` reports
...
# Static services declared at build time: main-prostor + dashboard.
...
# `exec prostor`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $PROSTOR_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/prostor/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-prostor-setup && \
    chmod +x /etc/cont-init.d/01-prostor-setup
...
ENV PROSTOR_TUI_DIR=/opt/prostor/ui-tui
ENV PROSTOR_HOME=/opt/data
ENV PROSTOR_WRITE_SAFE_ROOT=/opt/data
ENV PROSTOR_DISABLE_LAZY_INSTALLS=1

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> prostor ...` they default to root, and any file the
# command writes under $PROSTOR_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/prostor/bin/prostor, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid prostor`
# when invoked as root. Non-root callers (supervised processes,
# `--user prostor`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/prostor/.venv/bin/prostor). See the shim source for
# the opt-out env var (PROSTOR_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> prostor ...` and any
# subprocess that doesn't activate the venv first still find prostor.
#
# /opt/prostor/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/prostor/bin:/opt/prostor/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. prostor
# subcommand vs. no-args), drops to the prostor user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/prostor/docker/main-wrapper.sh" ]
CMD [ ]
