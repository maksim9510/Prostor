"""Centralized logging setup for Prostor Agent.
...
# essentially always in Prostor (TUI, gateway, ``hy_memory`` server, MCP
...
# Used by _ComponentFilter and exposed for ``prostor logs --component``.
COMPONENT_PREFIXES = {
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the prostor group. However, both ``_open()`` (initial creation)
