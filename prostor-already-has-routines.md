# Prostor Agent Has Had "Routines" Since March
...
Prostor equivalent — works today:
```bash
...
Prostor equivalent — works today:
```bash
prostor webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Prostor implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Prostor Agent |
|---|---|---|
...
## Things Prostor Does That Routines Can't
...
Prostor has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Prostor Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
prostor gateway setup    # enable webhooks
prostor webhook subscribe pr-review \
  --events "pull_request" \
...
*Prostor Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
