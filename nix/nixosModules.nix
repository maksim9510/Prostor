# nix/nixosModules.nix — NixOS module for prostor-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: prostor runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $PROSTOR_HOME/.env
# and read by prostor at startup — no container recreation needed for env changes.
#
# Tool resolution: the prostor wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.prostor-agent;
    effectivePackage =
    ...
    prostor-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-prostor-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "prostor-config-attrs";
      description = "Prostor YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "prostor-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # PROSTOR_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "prostor-documents" { } (
      ''
        mkdir -p $out
    ...
    # prostor user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "prostor-container-entrypoint" ''
      set -eu

      PROSTOR_UID="''${PROSTOR_UID:?PROSTOR_UID must be set}"
      PROSTOR_GID="''${PROSTOR_GID:?PROSTOR_GID must be set}"

      # ── Group: ensure a group with GID=$PROSTOR_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$PROSTOR_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="prostor"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$PROSTOR_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$PROSTOR_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$PROSTOR_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$PROSTOR_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="prostor"
        TARGET_HOME="/home/prostor"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$PROSTOR_UID" -g "$PROSTOR_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$PROSTOR_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$PROSTOR_UID:$PROSTOR_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure PROSTOR_HOME is owned by the target user.
      ...
      if [ -n "''${PROSTOR_HOME:-}" ] && [ -d "$PROSTOR_HOME" ]; then
        find "$PROSTOR_HOME" \! -user "$PROSTOR_UID" -exec chown "$PROSTOR_UID:$PROSTOR_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/prostor-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/prostor-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/prostor ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/prostor
        chmod 0440 /etc/sudoers.d/prostor
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$PROSTOR_UID" --regid="$PROSTOR_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $PROSTOR_HOME/.env.
    ...
    # Default: /var/lib/prostor/workspace → /data/workspace.
  ...
  in {
    options.services.prostor-agent = with lib; {
      enable = mkEnableOption "Prostor Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = prostor-agent;
        description = "The prostor-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/prostor";
        description = "State directory. Contains .prostor/ subdir (PROSTOR_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Prostor config (attrset). Deep-merged across module
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $PROSTOR_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $PROSTOR_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `prostor gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the prostor user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the prostor plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Prostor discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          prostor_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as prostor (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in prostor-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the prostor CLI to environment.systemPackages and export
          PROSTOR_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.prostor symlink to the service
            stateDir. These users are automatically added to the prostor group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.prostor-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the prostor CLI to system PATH and export PROSTOR_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.prostor/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.PROSTOR_HOME = "${cfg.stateDir}/.prostor";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.prostor-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the prostor user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.prostor-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed prostor
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure prostor is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.prostor        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.prostor/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.prostor/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.prostor/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.prostor/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.prostor/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."prostor-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.prostor
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.prostor ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.prostor ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.prostor -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.prostor/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.prostor/$_subdir"
            chmod 2770 "${cfg.stateDir}/.prostor/$_subdir"
            find "${cfg.stateDir}/.prostor/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # prostor-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.prostor/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.prostor/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.prostor/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.prostor/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.prostor/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.prostor/.managed
          chmod 0644 ${cfg.stateDir}/.prostor/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.prostor/.container-mode <<'PROSTOR_CONTAINER_MODE_EOF'
    ...
    PROSTOR_CONTAINER_MODE_EOF
          ...
          '' else ''
            rm -f ${cfg.stateDir}/.prostor/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.prostor";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.prostor" ]; then
                  rm -f "${symlinkPath}"
                  echo "prostor-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.prostor -> stateDir/.prostor for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.prostor";
                target = "${cfg.stateDir}/.prostor";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "prostor-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.prostor/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.prostor/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.prostor/auth.json
              fi
            ''}
          ''}
          ...
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ...
            cat > "$ENV_FILE" <<'PROSTOR_NIX_ENV_EOF'
    ${envFileContent}
    PROSTOR_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.prostor/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.prostor/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.prostor/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.prostor-agent = {
          description = "Prostor Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            PROSTOR_HOME = "${cfg.stateDir}/.prostor";
            PROSTOR_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # so interactive users in the prostor group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.prostor-agent = {
          description = "Prostor Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              PROSTOR_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              PROSTOR_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env PROSTOR_UID="$PROSTOR_UID" \
                --env PROSTOR_GID="$PROSTOR_GID" \
                --env PROSTOR_HOME=${containerDataDir}/.prostor \
                --env PROSTOR_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/prostor gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
