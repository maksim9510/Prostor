...
let
  ...
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "prostor-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/prostor-desktop $out/bin
    cp -r ${renderer}/* $out/share/prostor-desktop/
    ...
    substituteInPlace $out/share/prostor-desktop/electron/main.cjs \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Prostor Agent";
    homepage = "https://github.com/NousResearch/prostor-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "prostor-desktop";
  };
}
