...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "prostor-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "prostor-build-package" { } ''
          echo "PASS: package built at ${prostor-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "prostor-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "prostor-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${prostor-agent}/bin/prostor || (echo "FAIL: prostor binary missing"; exit 1)
          test -x ${prostor-agent}/bin/prostor-agent || (echo "FAIL: prostor-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${prostor-agent}/bin/prostor version 2>&1 | grep -qi "prostor" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "prostor-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in prostor prostor-agent prostor-acp; do
            test -x ${prostor-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "prostor-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking prostor --help ==="
          ${prostor-agent}/bin/prostor --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${prostor-agent}/bin/prostor --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "prostor-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${prostor-agent}/share/prostor-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${prostor-agent}/share/prostor-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "PROSTOR_BUNDLED_SKILLS" ${prostor-agent}/bin/prostor || \
            (echo "FAIL: PROSTOR_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: PROSTOR_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "prostor-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${prostor-agent}/share/prostor-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${prostor-agent}/share/prostor-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "PROSTOR_BUNDLED_PLUGINS" ${prostor-agent}/bin/prostor || \
            (echo "FAIL: PROSTOR_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: PROSTOR_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "prostor-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${prostor-agent}/share/prostor-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${prostor-agent}/share/prostor-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${prostor-agent}/share/prostor-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "PROSTOR_BUNDLED_LOCALES" ${prostor-agent}/bin/prostor || \
            (echo "FAIL: PROSTOR_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: PROSTOR_BUNDLED_LOCALES set in wrapper"
          ...
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without PROSTOR_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "prostor-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${prostor-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${prostor-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "PROSTOR_TUI_DIR" ${prostor-agent}/bin/prostor || \
            (echo "FAIL: PROSTOR_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: PROSTOR_TUI_DIR set in wrapper"
        ...
        # Verify PROSTOR_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        prostor-node = pkgs.runCommand "prostor-node-version" { } ''
          set -e
          echo "=== Checking PROSTOR_NODE in wrapper ==="
          grep -q "PROSTOR_NODE" ${prostor-agent}/bin/prostor || \
            (echo "FAIL: PROSTOR_NODE not set in wrapper"; exit 1)
          echo "PASS: PROSTOR_NODE present in wrapper"

          PROSTOR_NODE=$(sed -n "s/^export PROSTOR_NODE='\(.*\)'/\1/p" ${prostor-agent}/bin/prostor)
          test -x "$PROSTOR_NODE" || (echo "FAIL: PROSTOR_NODE=$PROSTOR_NODE not executable"; exit 1)
          echo "PASS: PROSTOR_NODE executable at $PROSTOR_NODE"

          NODE_MAJOR=$("$PROSTOR_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All PROSTOR_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify PROSTOR_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "prostor-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(PROSTOR_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking PROSTOR_MANAGED guards ==="
          check_blocked "config set" ${prostor-agent}/bin/prostor config set model foo
          check_blocked "config edit" ${prostor-agent}/bin/prostor config edit
        ...
        in pkgs.runCommand "prostor-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${prostor-agent}/bin/prostor; then
        ...
        in pkgs.runCommand "prostor-config-roundtrip" {
          ...
          merge_and_load() {
            local prostor_home="$1"
            export PROSTOR_HOME="$prostor_home"
            ${configMergeScript} ${nixSettings} "$prostor_home/config.yaml"
            ${prostorVenv}/bin/python3 -c '
import json, sys
