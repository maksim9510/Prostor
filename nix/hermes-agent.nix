# nix/prostor-agent.nix — Overridable Prostor Agent package
#
...
let
  ...
  # to a dev checkout — the loader reads them from PROSTOR_BUNDLED_PLUGINS.
  ...
  # at by PROSTOR_BUNDLED_LOCALES so the wrapped binary always resolves human
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in prostor sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/prostor-agent $out/bin
  ...
  passthru = {
    ...
    # ("existing prostor on PATH") then picks up the fully wrapped
    # `prostor` binary — venv with all deps, bundled skills/plugins,
    ...
    devShellHook = ''
      STAMP=".nix-stamps/prostor-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "prostor-agent: installing Python dependencies..."
