...
on Japanese locales, etc.).  Prostor's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Prostor is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``PROSTOR_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("PROSTOR_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Prostor-managed Git install directories so
    ...
    # already-running shells don't see the change — which means prostor
    ...
    # install without Prostor-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Prostor's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Prostor's
    ``prostor config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Prostor (User env var in Windows Settings, or
    export in a PowerShell profile) and Prostor picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\prostor\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``prostor`` from right after install) retain their old PATH.

    Any subprocess Prostor spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Prostor-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "prostor", "git", "cmd"),
        os.path.join(local_appdata, "prostor", "git", "bin"),
        os.path.join(local_appdata, "prostor", "git", "usr", "bin"),
        # Prostor venv Scripts directory — host of the prostor.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes prostor, but harmless to include.
        os.path.join(local_appdata, "prostor", "prostor-agent", "venv", "Scripts"),
