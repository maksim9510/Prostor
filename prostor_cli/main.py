...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``prostor_cli`` not importable.
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m prostor_cli.main
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from prostor_cli.models import CANONICAL_PROVIDERS as _cp
...
def main():
    ...
    try:
        from prostor_cli.pets import register_cli as _register_pets_cli
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from prostor_cli.tools_config import _cua_driver_cmd
            ...
            if path:
                version = ""
                try:
                    from prostor_cli.tools_config import _cua_driver_env
