"""``prostor debug`` debug tools for Prostor Agent.

Currently supports:
    prostor debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.prostor/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[prostor debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.prostor/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``prostor debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``prostor debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Prostor version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `prostor debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "prostor-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``prostor debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.prostor/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``prostor debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "prostor-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``prostor dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``prostor debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "prostor debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `prostor debug share --local` to print the report instead.\n")
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: prostor debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'prostor debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``prostor debug`` call.
    ...
    # best-effort — any failure is swallowed so ``prostor debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: prostor debug <command>")
        print()
        print("Commands:")
