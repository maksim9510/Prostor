...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``prostor model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `prostor model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``prostor model`` universe.
