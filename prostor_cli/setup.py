"""
Interactive setup wizard for Prostor Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Prostor Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Prostor using environment variables or config commands:")
    print_info("  prostor config set model.provider custom")
    print_info("  prostor config set model.base_url http://localhost:8080/v1")
    print_info("  prostor config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'prostor setup' in an interactive terminal to use the full wizard.")
    print()
...
def _print_setup_summary(config: dict, prostor_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'prostor setup' to configure"))
    ...
    # Video generation — opt-in via `prostor tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'prostor setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'prostor setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'prostor setup terminal'"))
    ...
    print()
    print(f"   {color('prostor setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('prostor setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('prostor setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('prostor setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('prostor setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('prostor config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('prostor config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('prostor config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('prostor', Colors.GREEN)}              Start chatting")
    print(f"   {color('prostor gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('prostor doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `prostor tools` and `prostor setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``prostor model``)
    ...
    provider added to ``prostor model`` is automatically available here.
    ...
    try:
        ...
        print_info("You can try again later with: prostor model")
    ...
    # on demand via `prostor auth add`, `prostor setup` vision, and
    # `prostor setup tts`. This keeps both quick and full setup thin.
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.prostor/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Prostor auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'prostor setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `prostor setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `prostor setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into PROSTOR_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("PROSTOR_MAX_ITERATIONS")
    ...
    print_info("  Run `prostor setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # PROSTOR_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_bluebubbles():
    ...
    print_info("Connects Prostor to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'prostor setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     prostor config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   prostor gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `prostor setup tools` and `prostor tools` use the same flow:
...
# Config values may have different semantics between OpenClaw and Prostor.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Prostor to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Prostor at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Prostor at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Prostor at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Prostor at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Prostor equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Prostor config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Prostor.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Prostor's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(prostor_home: Path) -> bool:
    ...
    print_info("Prostor can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: prostor claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: prostor claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Prostor configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Prostor (use prostor claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``prostor setup --portal`` and ``prostor portal``. This is the
    ...
    Prostor session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``prostor setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``prostor model`` runs
    ...
    print(color("│     ⚕ Prostor Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    try:
        ...
        print_info("  You can retry later with `prostor portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `prostor portal`: %s", exc)
        print()
    ...
    print_info("  Run `prostor portal info` to inspect routing.")
    print_info("  Run `prostor` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      prostor setup           — full or quick (auto-detected)
      prostor setup model     — just model/provider
      prostor setup tts       — just text-to-speech
      prostor setup terminal  — just terminal backend
      prostor setup gateway   — just messaging platforms
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Prostor Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Prostor Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Prostor Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Prostor configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'prostor setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Prostor?",
            [
    ...
    # Tune later with `prostor setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, prostor_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'prostor setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    prostor setup")
    if gateway_choice != 0:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``prostor tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``prostor setup agent`` /
    ``prostor config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, prostor_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `prostor update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        prostor tools")
        print_info("  Seed skills:         prostor skills opt-in --sync")
        print_info("  Add MCP servers:     prostor mcp add")
...
def _blank_slate_walkthrough(config: dict, prostor_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`prostor update` runs from re-injecting them. Opt back in any")
            print_info("time with `prostor skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `prostor tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `prostor plugins list` / `prostor plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `prostor plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `prostor mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `prostor mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   prostor tools")
    print_info("  Seed skills:         prostor skills opt-in --sync")
    print_info("  Add MCP servers:     prostor mcp add")
    print_info("  Tune agent settings: prostor setup agent")
    print()
...
def _run_quick_setup(config: dict, prostor_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'prostor setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Prostor to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'prostor setup gateway'.")
