...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Prostor-managed OAuth credentials."""

    auth_hint = "prostor auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`prostor auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`prostor auth reset xai-oauth` or re-authenticate with "
                    "`prostor auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `prostor auth add xai-oauth --type oauth`."
            )
