...
Reads the user's Nous OAuth state from ``~/.prostor/auth.json`` through the
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `prostor auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `prostor auth add nous` to re-authenticate."
                )
