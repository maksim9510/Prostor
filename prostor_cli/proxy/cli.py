"""CLI handlers for the ``prostor proxy`` subcommand."""
...
from prostor_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "prostor proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"prostor auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Prostor proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Prostor proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: prostor proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``prostor proxy <subcommand>``."""
    ...
    print(
        "prostor proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  prostor proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
