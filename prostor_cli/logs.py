"""``prostor logs`` — view and filter Prostor log files.
...
Usage examples::
    ...
    prostor logs                    # last 50 lines of agent.log
    prostor logs -f                 # follow agent.log in real time
    prostor logs errors             # last 50 lines of errors.log
    prostor logs gateway -n 100    # last 100 lines of gateway.log
    prostor logs gui -f            # follow gui.log (dashboard/pty/ws)
    prostor logs desktop -f        # follow desktop.log (Electron app boot/backend)
    prostor logs --level WARNING    # only WARNING+ lines
    prostor logs --session abc123   # filter by session ID substring
    prostor logs --component tools  # only tool-related lines
    prostor logs --since 1h         # lines from the last hour
    prostor logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if follow:
        print(f"--- {display_prostor_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_prostor_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_prostor_home()}/logs/")
        return

    print(f"Log files in {display_prostor_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'prostor chat' to generate logs)")
