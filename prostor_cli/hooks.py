"""prostor hooks — inspect and manage shell-script hooks.

Usage::

    prostor hooks list
    prostor hooks test <event> [--for-tool X] [--payload-file F]
    prostor hooks revoke <command>
    prostor hooks doctor

Consent records live under ``~/.prostor/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.prostor/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``prostor hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: prostor hooks {list|test|revoke|doctor}")
        print("Run 'prostor hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.prostor/config.yaml.")
        print("See `prostor hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `prostor hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `prostor hooks test` and `prostor hooks doctor`
...
def _print_run_result(result: dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Prostor wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `prostor hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `prostor hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `prostor hooks doctor`.")
