"""MCP picker — interactive `prostor mcp picker` (also the default `prostor mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``prostor mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `prostor plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Prostor session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: list[_Row]) -> None:
    ...
    print()
    print(color(
        "  Install: prostor mcp install <name>    Picker: prostor mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Prostor is
...
def show_catalog() -> None:
    """`prostor mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`prostor mcp picker` (and default `prostor mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`prostor mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `prostor mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
