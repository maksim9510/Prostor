"""
Profile management for multiple isolated Prostor instances.

Each profile is a fully independent PROSTOR_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.prostor/profiles/<name>/`` by default.

The "default" profile is ``~/.prostor`` itself — backward compatible,
zero migration needed.

Usage::

    prostor profile create coder          # fresh profile + bundled skills
    prostor profile create coder --clone  # also copy config, .env, SOUL.md, skills
    prostor profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    prostor -p coder chat                 # or via flag
    prostor profile use coder             # set as sticky default
    prostor profile delete coder          # remove profile + alias + service
"""
...
# default profile (``~/.prostor``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   prostor-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `prostor backup` archives
...
# Marker file written by `prostor profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `prostor
...
# `prostor skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Prostor infrastructure directories that only the default profile
         (``~/.prostor``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.prostor) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "prostor-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "prostor", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the prostor root, NOT to the current PROSTOR_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where PROSTOR_HOME points outside
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Prostor installation itself or a common system binary.  "
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Prostor. If we
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``prostor -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``prostor profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``prostor profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Prostor config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own PROSTOR_HOME.
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``prostor update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.prostor)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.prostor/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `prostor -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Prostor profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `prostor update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`prostor profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Prostor (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `prostor profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `prostor -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``prostor profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "PROSTOR_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.prostor).\n"
            "To remove everything, use: prostor uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``PROSTOR_HOME`` and binds the port resolved by ``gateway/config.py``
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set PROSTOR_HOME so _profile_suffix resolves correctly
    ...
    try:
        ...
        if old_home is not None:
            os.environ["PROSTOR_HOME"] = old_home
        elif "PROSTOR_HOME" in os.environ:
            del os.environ["PROSTOR_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.prostor/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: prostor profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from PROSTOR_HOME.

    Returns ``"default"`` if PROSTOR_HOME is not set or points to ``~/.prostor``.
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.prostor itself — its parent is ~/ and its
        # directory name is ".prostor", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: prostor profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.prostor itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.prostor). "
            "Specify a different name: prostor profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a PROSTOR_HOME path string.

    Called early in the CLI entry point, before any prostor modules
    are imported, to set the PROSTOR_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
