"""Migrate Prostor' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Prostor' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Prostor mcp_servers.<n>.command/args/env  → codex stdio transport
  Prostor mcp_servers.<n>.url/headers       → codex streamable_http transport
  Prostor mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Prostor mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Prostor-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Prostor' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Prostor config.")
...
# Prostor keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Prostor' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, prostor_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Prostor MCP server config to the codex inline-table dict
    ...
    if not isinstance(prostor_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(prostor_cfg.get("command"))
    has_url = bool(prostor_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (PROSTOR_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Prostor)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Prostor' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Prostor' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Prostor
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="prostor-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a PROSTOR_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed prostor-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.prostor`` symlink that happens to live under ``/private/var/folders``
...
def _build_prostor_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Prostor' own
    ...
    so a prostor installed under /opt/, /usr/local/, or a venv all work.
    PROSTOR_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # PROSTOR_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_prostor_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever PROSTOR_HOME its launcher sets
    ...
    # — that would override the launcher's PROSTOR_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("PROSTOR_HOME", tmp_path) would
    ...
    prostor_home = os.environ.get("PROSTOR_HOME") or ""
    if prostor_home and _looks_like_test_tempdir(prostor_home):
        prostor_home = ""
    if prostor_home:
        env["PROSTOR_HOME"] = prostor_home
    # PYTHONPATH passes through so a worktree-launched prostor finds the
    ...
    env["PROSTOR_QUIET"] = "1"
    env["PROSTOR_REDACT_SECRETS"] = env.get("PROSTOR_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_prostor_tools: bool = True,
) -> MigrationReport:
    """Translate Prostor mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.
    ...
    Args:
        ...
        expose_prostor_tools: when True (default), register Prostor' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Prostor for tools
    ...
    prostor_servers = (prostor_config or {}).get("mcp_servers") or {}
    if not isinstance(prostor_servers, dict):
    ...
    for name, cfg in prostor_servers.items():
    ...
    # Inject Prostor' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Prostor for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/prostor_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_prostor_tools:
        translated["prostor-tools"] = _build_prostor_tools_mcp_entry()
        if "prostor-tools" not in report.migrated:
            report.migrated.append("prostor-tools")
