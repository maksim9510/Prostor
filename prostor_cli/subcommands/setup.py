"""``prostor setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Prostor Agent with an interactive wizard. "
        "Run a specific section: prostor setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'prostor setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
