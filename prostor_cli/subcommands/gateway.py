...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if PROSTOR_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy prostor.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Prostor gateway unit files "
            "(e.g. prostor.service) left over from older installs. Profile "
            "units (prostor-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.prostor/.env. "
            "Requires being logged in (prostor setup). Not available in managed installs."
        ),
    )
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `prostor proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
