"""
Unified self-relaunch for Prostor CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``prostor sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``prostor`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched prostor."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def relaunch(
    ...
    *,
    ...
    relaunch contract wants: "run prostor again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    That's functionally equivalent — the user sees "prostor exited, then
    new prostor started" — just with two PIDs in play instead of one.
    """
