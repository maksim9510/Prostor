...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # <prostor-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `prostor --profile <name>
    ...
    raise SystemExit(f"`prostor auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Prostor reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `prostor auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `prostor auth` is called bare."""
