"""`prostor checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.prostor/checkpoints/``.  Actions:

    prostor checkpoints               # same as `status`
    prostor checkpoints status        # total size, project count, breakdown
    prostor checkpoints list          # per-project checkpoint counts + workdir
    prostor checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    prostor checkpoints clear [-f]    # nuke the entire base (asks first)
    prostor checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    prostor checkpoints
    prostor checkpoints prune --retention-days 3 --max-size-mb 200
    prostor checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: prostor checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``prostor checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `prostor checkpoints` → status
