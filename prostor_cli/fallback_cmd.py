"""
prostor fallback — manage the fallback provider chain.
...
Subcommands:
  ...
  prostor fallback add      Pick provider + model via the same picker as `prostor model`,
                           then append the selection to the chain
  prostor fallback remove   Pick an entry to delete from the chain
  prostor fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.prostor/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `prostor model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  prostor fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    ...
    print("  `prostor model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `prostor fallback list` to view, or `prostor fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``prostor fallback [subcommand]``."""
