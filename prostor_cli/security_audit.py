"""On-demand supply-chain audit for Prostor Agent installs.

Scans three surfaces a Prostor user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Prostor venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.prostor/plugins``
...
def _discover_plugins(prostor_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.prostor/plugins``.
...
def _osv_query_batch(components: list[Component]) -> dict[Component, list[str]]:
    ...
    for chunk_start in range(0, len(components), OSV_BATCH_MAX):
        ...
        for comp, result in zip(chunk, results, strict=False):
...
def run_audit(
    *,
    ...
    home = prostor_home or Path(get_prostor_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, prostor_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(prostor_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `prostor security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, prostor_home=home
    )
    if total == 0:
