...
A system-level directory (default ``/etc/prostor``, root-owned and not
...
_DEFAULT_MANAGED_DIR = Path("/etc/prostor")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/prostor`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``PROSTOR_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$PROSTOR_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/prostor`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("PROSTOR_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
