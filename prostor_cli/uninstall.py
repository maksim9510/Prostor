"""
Prostor Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.prostor/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Prostor PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing prostor-agent or prostor PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Prostor Agent" comment and following line
                if '# Prostor Agent' in line or '# prostor-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('prostor' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing prostor
                if 'prostor' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the prostor wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "prostor",
        Path("/usr/local/bin/prostor"),
    ]
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``prostor`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Prostor home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``prostor gateway run`` processes
...
#
#   1. Sets User-scope env vars ``PROSTOR_HOME`` and ``PROSTOR_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\prostor\git\cmd``, ``%LOCALAPPDATA%\prostor\git\bin``,
#      ``%LOCALAPPDATA%\prostor\git\usr\bin``, ``%LOCALAPPDATA%\prostor\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\prostor\git\`` and Node to
#      ``%LOCALAPPDATA%\prostor\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``prostor dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\prostor\gateway-service\`` and sometimes
...
def remove_hermes_env_vars_windows() -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("PROSTOR_HOME", "PROSTOR_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its PROSTOR_HOME directory.

    We shell out to ``prostor -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current PROSTOR_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's PROSTOR_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``prostor uninstall --gui``. Removes the desktop app's built
    ...
    print(color("│         ⚕ Prostor Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Prostor agent is still installed. Run 'prostor' to use the CLI,")
    print("or 'prostor uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    # full wipe (code + ~/.prostor data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Prostor Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their PROSTOR_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Prostor data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Prostor code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``prostor`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$PROSTOR_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        if removed_env:
            ...
            log_info("No Prostor-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing prostor command...")
    ...
    if removed_node_links:
        ...
        log_info("No Prostor-managed node/npm/npx symlinks found")
    ...
    # 5. Optionally remove ~/.prostor/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile PROSTOR_HOME dirs live under
    ...
    print()
    print("Thank you for using Prostor Agent! ⚕")
    print()
