"""Persistent session goals — the Ralph loop for Prostor.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Prostor feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.ProstorCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    ``prostor_home`` path so profile switches still pick up the right DB.
...
def run_kanban_goal_loop(
    *,
    ...
    (``prostor -p <profile> chat -q "work kanban task <id>"``). The worker's
