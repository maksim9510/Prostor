...
# Match only the real Nous Research Prostor 3 / Prostor 4 chat families.
# The previous substring check (`"prostor" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``prostor-brain:qwen3-14b-ctx16k`` that just
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``prostor config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'prostor model' for available providers, or define it "
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Build curated model lists keyed by prostor provider ID
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Prostor's canonical write key (matches
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Prostor's own writer uses; a hyphen variant is accepted
