...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Prostor could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `prostor model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Prostor cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`prostor model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `prostor model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Prostor could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `prostor model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Prostor could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `prostor model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `prostor model` to refresh Prostor."
        return message
