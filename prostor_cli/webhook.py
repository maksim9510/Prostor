"""prostor webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.prostor/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: prostor gateway run
"""
...
def webhook_command(args):
    """Entry point for 'prostor webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: prostor webhook {subscribe|list|remove|test}")
        print("Run 'prostor webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (prostor gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: prostor webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (prostor gateway run)")
