"""prostor claw — OpenClaw migration commands.

Usage:
    prostor claw migrate              # Preview then migrate (always shows preview first)
    prostor claw migrate --dry-run    # Preview only, no changes
    prostor claw migrate --yes        # Skip confirmation prompt
    prostor claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    prostor claw migrate --no-backup  # Skip pre-migration snapshot
    prostor claw cleanup              # Archive leftover OpenClaw directories
    prostor claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Prostor may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Prostor gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Prostor gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'prostor gateway stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_prostor.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_prostor", script_path)
...
def claw_command(args):
    """Route prostor claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: prostor claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Prostor")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'prostor claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Prostor migration."""
    ...
    # migrate-prostor posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: prostor claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {prostor_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.prostor/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: prostor claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Prostor home ─────────
    # Delegates to prostor_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: prostor import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Prostor home."
            )
    ...
    # 'prostor claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: prostor claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: prostor claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  prostor claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  prostor claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  prostor config set OPENROUTER_API_KEY sk-or-v1-...")
