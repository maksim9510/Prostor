"""
MCP Server Management CLI — ``prostor mcp`` subcommand.

Implements ``prostor mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.prostor/config.yaml under the ``mcp_servers`` key.
"""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.prostor/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``prostor mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  prostor mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  prostor mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  prostor mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: prostor mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `prostor mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  prostor mcp add <name> --url <endpoint>')
        _info('  prostor mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `prostor mcp remove` + `prostor mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `prostor mcp login " + name + "`.")
            return
        if tools:
...
# ─── prostor mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'prostor mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``prostor mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("prostor mcp catalog                            List Nous-approved MCPs")
        _info("prostor mcp install <name>                     Install a catalog MCP")
        _info("prostor mcp serve                              Run as MCP server")
        _info("prostor mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("prostor mcp add <name> --command <cmd>         Add a stdio server")
        _info("prostor mcp add <name> --preset <preset>       Add from a known preset")
        _info("prostor mcp remove <name>                      Remove a server")
        _info("prostor mcp list                               List configured servers")
        _info("prostor mcp test <name>                        Test connection")
        _info("prostor mcp configure <name>                   Toggle tools")
        _info("prostor mcp login <name>                       Re-authenticate OAuth")
        print()
