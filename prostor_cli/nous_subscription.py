...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("PROSTOR_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `prostor model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("PROSTOR_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("PROSTOR_LOCAL_STT_COMMAND"))
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`prostor tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``prostor tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``prostor model``'s Nous
    login, this:
