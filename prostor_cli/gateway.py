"""
Gateway subcommand for prostor CLI.

Handles: prostor gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "prostor CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``prostor gateway status`` from falsely counting the ``prostor`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``prostor gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. PROSTOR_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``prostor update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Prostor profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Prostor profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``prostor update`` users
    # with no running gateway until they re-invoke ``prostor gateway``
...
def _sync_prostor_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = env.get("PROSTOR_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("PROSTOR_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["PROSTOR_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}prostor gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}prostor gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `prostor gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``prostor gateway status`` output so users with
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (PROSTOR_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Prostor Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``prostor gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``PROSTOR_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("PROSTOR_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "prostor-gateway"
SERVICE_DESCRIPTION = "Prostor Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current PROSTOR_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (PROSTOR_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary PROSTOR_HOME paths
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this PROSTOR_HOME.

    Default ``~/.prostor`` returns ``prostor-gateway`` (backward compatible).
    Profile ``~/.prostor/profiles/coder`` returns ``prostor-gateway-coder``.
    Any other PROSTOR_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Prostor installs that predate the
# prostor-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (prostor-gateway-*.service) and unrelated third-party
# "prostor" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("prostor.service",)
...
def _find_legacy_prostor_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``prostor.service`` before the rename to
    ``prostor-gateway.service``). When both a legacy unit and the current
    ``prostor-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``prostor-gateway-coder.service`` and unrelated third-party
      ``prostor-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``prostor.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Prostor gateway unit files if any are installed.
...
def remove_legacy_prostor_units(
    ...
    print()
    print("Legacy Prostor gateway unit(s) found:")
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: prostor gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo prostor gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    prostor gateway uninstall")
    print_info("    sudo prostor gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Prostor can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo prostor gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo prostor gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo prostor gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Prostor often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.prostor`` → ``ai.prostor.gateway.plist`` (backward compatible).
    Profile ``~/.prostor/profiles/coder`` → ``ai.prostor.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.prostor.gateway-{suffix}" if suffix else "ai.prostor.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.prostor/prostor-agent  -> /home/alice/.prostor/prostor-agent
      /opt/prostor                 -> /opt/prostor  (kept as-is)
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``prostor update`` later relocates/removes — the path
    ...
    ``PROSTOR_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if PROSTOR_HOME
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir PROSTOR_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved PROSTOR_HOME
    ...
    exported a throwaway ``PROSTOR_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``prostor gateway restart``
    with ``PROSTOR_HOME=/tmp/prostor-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="PROSTOR_HOME=..."`` lines and launchd
    ``<key>PROSTOR_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'PROSTOR_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>PROSTOR_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp PROSTOR_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: PROSTOR_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported PROSTOR_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # PROSTOR_HOME into the unit (manual E2E homes like /tmp/prostor-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Prostor install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo prostor gateway uninstall --system")
    print_info("         prostor gateway install")
    print_info("         prostor gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (prostor.service from pre-rename installs)
    # before installing the new prostor-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}prostor gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}prostor gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `prostor gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `prostor gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}prostor gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}prostor gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}prostor gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}prostor gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.prostor.gateway-{suffix}" if suffix else "ai.prostor.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``prostor gateway start/stop/status`` call).
...
# (the documented `nohup prostor gateway run` workaround). See #23387.
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup prostor gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: prostor gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup prostor gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Prostor install"
    )
    return True
...
def launchd_stop():
    ...
    # `prostor gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Prostor install")
    else:
        print("⚠ Service definition is stale relative to the current Prostor install")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``PROSTOR_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("PROSTOR_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``prostor gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``prostor gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``prostor gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other PROSTOR_HOME roots.
    """
    ...
    print("  Use 'prostor gateway restart' to replace it,")
    print("  or 'prostor gateway stop' first.")
    print("  Or use 'prostor gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("PROSTOR_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Prostor gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'prostor' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/prostor/docker/entrypoint.sh before the Prostor command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$PROSTOR_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set PROSTOR_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `prostor gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set PROSTOR_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `prostor gateway restart` which already
    ...
    # `prostor gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Prostor Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # PROSTOR_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. prostor) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Prostor delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Prostor connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: prostor pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Prostor will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``prostor setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run prostor under
        WSL.
    """
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Prostor will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Prostor will store the returned account_id/token in ~/.prostor/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `prostor gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `prostor pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `prostor pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "ProstorAgent"')
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.prostor/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.prostor/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: prostor gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: prostor gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: prostor gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo prostor gateway install --system"
                        )
                    print_info("  Or run in foreground:  prostor gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: prostor gateway run")
                print_info(
                    "  For persistence:   tmux new -s prostor 'prostor gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: prostor gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup prostor gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'prostor gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    if profile is None:
        ...
        # PROSTOR_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``prostor gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``prostor gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``prostor gateway run`` are unaffected.
      2. ``PROSTOR_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``prostor gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``PROSTOR_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("PROSTOR_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("PROSTOR_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${PROSTOR_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if PROSTOR_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or PROSTOR_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-prostor service's
    # pattern in docker/s6-rc.d/main-prostor/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: prostor gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: prostor gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s prostor 'prostor gateway run'"
                )
                print()
            ...
            print()
            print(
                "  prostor gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s prostor 'prostor gateway run'         # persistent via tmux"
            )
            print(
                "  nohup prostor gateway run > ~/.prostor/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  prostor profile create <name>     # creates the s6 service slot")
                print("  prostor -p <name> gateway start   # bring it up via s6")
                print("  prostor status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: prostor gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: prostor gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: prostor gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  prostor profile delete <name>     # tears down the s6 service slot")
                print("  prostor -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `prostor -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: prostor gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_PROSTOR_GATEWAY") == "1":
            print_error(
                ...
                "Use `prostor gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_PROSTOR_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `prostor gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: prostor gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  prostor gateway install")
                else:
                    print("To install as a service:")
                    print("  prostor gateway install")
                    print("  sudo prostor gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  prostor gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup prostor gateway run > ~/.prostor/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s prostor 'prostor gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  prostor gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  prostor gateway install  # Install as user service")
                    print(
                        "  sudo prostor gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Prostor gateway unit files from
        # pre-rename installs (e.g. prostor.service). Profile units and
        # unrelated third-party services are never touched.
