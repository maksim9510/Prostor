...
entries via ``prostor mcp catalog`` or the interactive ``prostor mcp picker``,
and install them with ``prostor mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into prostor-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``prostor mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.prostor/.env`` (the
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Prostor install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Prostor understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> list[CatalogEntry]:
    ...
    UIs can tell the user their Prostor is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> list[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Prostor
        understands. Update Prostor to install this entry.
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.prostor/mcp-installs/<name>`` and run
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``prostor mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `prostor mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `prostor mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `prostor mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `prostor mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `prostor auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`prostor auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Prostor session to load its tools.",
        Colors.GREEN,
    ))
