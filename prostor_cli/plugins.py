"""
Prostor Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with prostor-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.prostor/plugins/<name>/``
3. **Project plugins** – ``./.prostor/plugins/<name>/`` (opt-in via
   ``PROSTOR_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``prostor_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``PROSTOR_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("PROSTOR_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``PROSTOR_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.prostor/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("PROSTOR_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When PROSTOR_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Prostor loggers.
    """
    ...
    logger.debug(
        "PROSTOR_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManifest:
    ...
    #              in ~/.prostor/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``prostor plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``prostor honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``prostor <subcommand>``
    ...
    def register_slack_action_handler(
        ...
        Prostor' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``prostor model → Configure auxiliary models`` picker
    ...
    def register_skill(
        ...
        ``~/.prostor/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("PROSTOR_SAFE_MODE"):
            logger.info("PROSTOR_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        # 3. Project plugins (./.prostor/plugins/)
        if _env_enabled("PROSTOR_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".prostor" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set PROSTOR_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with prostor and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Prostor ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    f"not enabled in config (run `prostor plugins enable {lookup_key}` to activate)"
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `prostor plugins list`.
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="prostor-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> list[tuple]:
    ...
    Used by the ``prostor tools`` TUI so plugin-provided toolsets appear
