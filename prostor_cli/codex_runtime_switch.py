...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Prostor'
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "prostor"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Prostor' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Prostor for
        ...
        try:
            ...
            user_servers = [
                s for s in mig_report.migrated if s != "prostor-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Prostor tool callback are always-on production
            ...
            if "prostor-tools" in mig_report.migrated:
                msg_lines.append(
                    "Prostor tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Prostor runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Prostor tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Prostor runtime.")
