...
manual BotFather token copy-paste. Prostor talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.prostor-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "ProstorSetupBot"

DEFAULT_BOT_NAME = "Prostor Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``prostor_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    ...
    """
    _ = profile_name
    return f"prostor_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Prostor Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Prostor Telegram onboarding service.")
