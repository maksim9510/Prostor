...
#   - cli.py, prostor_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("PROSTOR_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("PROSTOR_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by PROSTOR_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like prostor-cli)
    ...
    if "execute_code" in available_tool_names:
        from tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, _get_execution_mode, build_execute_code_schema
    ...
    # window). Core Prostor tools (toolsets._PROSTOR_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from prostor_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from prostor_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from prostor_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from prostor_cli.plugins import get_pre_tool_call_block_message
        ...
        try:
            ...
            from prostor_cli.middleware import run_tool_execution_middleware
