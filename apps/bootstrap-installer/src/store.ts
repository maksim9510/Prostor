...
export const $prostorHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, prostorHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_prostor_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $prostorHome.set(prostorHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Prostor" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Prostor-Setup.exe --update);
...
export async function launchProstorDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_prostor_desktop', { installRoot })
}
