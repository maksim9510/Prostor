...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let prostor = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
...
/// which completes BEFORE we spawn `venv\Scripts\prostor.exe update`. And a
...
/// prostor.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named prostor.exe.)
...
mod tests {
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".prostor-update-in-progress");
