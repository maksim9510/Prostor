...
//!
//! Mirrors `prostor_constants.get_prostor_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\prostor
//!   macOS:   ~/.prostor
//!   Linux:   ~/.prostor  (override via $PROSTOR_HOME)
//!
//! NOTE (macOS): Python's get_prostor_home(), scripts/install.sh, and the
//! Electron desktop's resolveProstorHome() ALL use ~/.prostor on macOS — there
...
/// Returns the canonical Prostor home directory, respecting $PROSTOR_HOME if set.
pub fn hermes_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("PROSTOR_HOME") {
    ...
    {
        // %LOCALAPPDATA%\prostor — matches scripts/install.ps1's $ProstorHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("prostor");
        }
    }

    // macOS + Linux + fallback: ~/.prostor (matches Python get_prostor_home(),
    // install.sh, and the Electron desktop's resolveProstorHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".prostor");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".prostor")
}
...
/// PROSTOR_HOME so it survives repo checkout deletion (unlike anything under
/// prostor-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\prostor\prostor-setup.exe`; on other
...
pub fn copy_self_to_prostor_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to PROSTOR_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_PROSTOR_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".prostor-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under PROSTOR_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[prostor-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("PROSTOR_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_prostor_home() -> String {
