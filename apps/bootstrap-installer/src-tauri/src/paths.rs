...
pub fn hermes_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\prostor — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("prostor");
        }
    }

    // macOS + Linux + fallback: ~/.prostor (matches Python get_hermes_home(),
...
///
/// Lives directly under PROSTOR_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves PROSTOR_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    hermes_home().join(".prostor-update-in-progress")
}
