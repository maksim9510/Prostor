 ...
 *
 * Stamps the Prostor icon + identity onto the packed Windows Prostor.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `prostor desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Prostor')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Prostor'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Prostor.exe keeps the stock Electron icon`)
  }
}
