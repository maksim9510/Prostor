 ...
 * product name (`Prostor`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Prostor'
 *
 * This is a hard failure with no obvious cause for the user — `prostor desktop`
 ...
 * (the mac-unpacked Prostor.app bundle) and Windows (win-unpacked), so we
