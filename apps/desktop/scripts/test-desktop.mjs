...
// no longer bundles the Prostor Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Prostor.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Prostor'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Prostor.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'prostor'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function ensurePackagedApp() {
  if (process.env.PROSTOR_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Prostor-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Prostor-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Prostor-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Prostor-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.PROSTOR_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.PROSTOR_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
//   - The Prostor Agent Python payload is NOT shipped (it's fetched at first
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Prostor
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + PROSTOR_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  PROSTOR_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
