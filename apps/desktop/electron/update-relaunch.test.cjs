 ...
 *      internals; PROSTOR_HOME / PROSTOR_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.prostor/prostor-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'prostor')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  assert.equal(resolveUnpackedRelease('/usr/lib/prostor/prostor', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Prostor/prostor', ROOT, 'linux'), null)
  // dev electron
  assert.equal(resolveUnpackedRelease('/home/u/.prostor/prostor-agent/node_modules/electron/dist/electron', ROOT, 'linux'), null)
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'prostor'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'prostor')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'prostor://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]
  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'prostor://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves PROSTOR_HOME + PROSTOR_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    PROSTOR_HOME: '/home/u/.prostor',
    PROSTOR_DESKTOP_REMOTE_URL: 'http://box:9119',
    PROSTOR_DESKTOP_REMOTE_TOKEN: 'secret',
    PROSTOR_DESKTOP_HERMES_ROOT: '/home/u/dev/prostor',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.prostor/prostor-agent/apps/desktop/release/linux-unpacked/Prostor',
    args: ['prostor://open/agent/42', "--note=it's fine"],
    env: { PROSTOR_HOME: '/home/u/.prostor', PROSTOR_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export PROSTOR_HOME='\/home\/u\/\.prostor'/)
  assert.match(script, /export PROSTOR_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Prostor' 'prostor:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `prostor-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Prostor/Prostor',
    args: [],
    env: {},
    cwd: ''
  })
  const tmp = path.join(os.tmpdir(), `prostor-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  try {
    execFileSync('bash', ['-n', tmp], { stdio: 'pipe' })
  } finally {
  ...
  assert.match(script, /exec '\/opt\/Prostor\/Prostor'\n/)
})
