 ...
 *     `X-Prostor-Session-Token` header; WS uses `?token=`.
...
function pathWithGlobalRemoteProfile(path, profile, opts = {}) {
  ...
  let parsed
  try {
    parsed = new URL(rawPath, 'http://prostor.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a prostor session ACCESS-token cookie
