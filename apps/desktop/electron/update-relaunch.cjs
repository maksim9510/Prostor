 ...
 * Background
 * ----------
 * After `prostor update` + `prostor desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything PROSTOR_DESKTOP_* is preserved
// plus PROSTOR_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['PROSTOR_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['PROSTOR_DESKTOP_']
