 ...
 * the three user-facing uninstall modes to the `prostor uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m prostor_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `prostor_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'prostor_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Prostor.app/Contents/MacOS/Prostor  → …/Prostor.app
 *   Windows: …\Prostor\Prostor.exe                 → …\Prostor  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Prostor.app/Contents/MacOS/Prostor → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Prostor.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Prostor.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Prostor$/i.test(dir) || /[\\/]prostor-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/prostor
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import prostor_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import prostor_cli` resolves from source
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, prostorHome }) {
  ...
  // a problem, but Prostor install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
