...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Prostor.app/Contents/MacOS/Prostor', 'darwin'),
    '/Applications/Prostor.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Prostor.app/Contents/MacOS/Prostor', 'darwin'),
    '/Users/x/Applications/Prostor.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Prostor\\Prostor.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Prostor'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\prostor-desktop\\Prostor.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\prostor-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Prostor.exe', 'win32'), null)
})
...
test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/prostor/linux-unpacked/prostor', 'linux', {}),
    '/opt/prostor/linux-unpacked'
  )
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/prostor', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Prostor.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Prostor.app'), false)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Prostor" >nul 2>&1/)
