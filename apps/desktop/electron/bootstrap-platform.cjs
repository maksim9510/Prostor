 ...
 * null to keep it enabled. `PROSTOR_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.PROSTOR_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (PROSTOR_DESKTOP_DISABLE_GPU)'
