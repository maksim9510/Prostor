...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'prostor:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.prostorDesktop?.saveImageFromUrl) {
        if (await window.prostorDesktop.saveImageFromUrl(src)) {
