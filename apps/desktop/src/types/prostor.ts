...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Prostor doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in prostor_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `prostor model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `prostor model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `prostor tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/prostor/update/check` — the backend's own update state.
