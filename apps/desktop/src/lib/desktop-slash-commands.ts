...
/** Why a known Prostor command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Prostor profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Prostor Agent version', surface: exec() },
...
function isKnownProstorSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Prostor' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownProstorSlashCommand(normalized)
}
