...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.prostorDesktop?.terminal)
...
//      in prostor_cli/provider_catalog.py) — the SAME provider identity
//      `prostor model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Prostor can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Prostor can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Prostor never deletes creds another tool owns behind a silent API call.
