...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: ProstorConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: ProstorConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // PROSTOR_QWEN_ and PROSTOR_GEMINI_ both share the PROSTOR_ stem.
      expect(providerGroup('PROSTOR_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('PROSTOR_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
