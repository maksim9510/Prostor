...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../prostor'
...
import { useProstorConfig } from './session/hooks/use-prostor-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.prostorDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.prostorDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.prostorDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.prostorDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // prostor:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.prostorDesktop?.onDeepLink?.(payload => {
    ...
    void window.prostorDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.prostorDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshProstorConfig, sttEnabled, voiceMaxRecordingSeconds } = useProstorConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshProstorConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshProstorConfig])
