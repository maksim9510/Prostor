...
interface ProstorConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useProstorConfig({ activeSessionIdRef, refreshProjectBranch }: ProstorConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshProstorConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getProstorConfig(), getProstorConfigDefaults().catch(() => ({}))])
  ...
  return { refreshProstorConfig, sttEnabled, voiceMaxRecordingSeconds }
}
