...
import { prostorDirectiveFormatter, type SlashChipKind } from '@/components/assistant-ui/directive-text'
...
export function ChatBar({
  ...
  // Distinguish a cold start ("Starting Prostor...") from a dropped connection
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = prostorDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, PROSTOR_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, PROSTOR_PATHS_MIME)) {
      return
    }
