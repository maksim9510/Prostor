...
const SKIN_KEY = 'prostor-desktop-theme-v2'
const MODE_KEY = 'prostor-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'prostor-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'prostor-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'prostor-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.prostorTheme = skinName
  root.dataset.prostorMode = rendered
  ...
  window.prostorDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('prostor-boot-background', c.background)
    window.localStorage.setItem('prostor-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.prostorThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.prostorDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Prostor backend theme on connect. */
