...
const PROSTOR_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ProstorRefType = (typeof PROSTOR_REF_TYPES)[number]
...
const ICON_PATHS: Record<ProstorRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ProstorRefType] ?? ICON_FALLBACK
...
const PROSTOR_DIRECTIVE_RE = new RegExp(
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(PROSTOR_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ProstorRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ProstorRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Prostor directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.prostorDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.prostorDesktop?.readFileDataUrl(id)
