...
import { formatRefValue, prostorDirectiveFormatter } from './directive-text'
...
describe('prostorDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = prostorDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = prostorDirectiveFormatter.parse('@file:src/main.tsx the entry point')
