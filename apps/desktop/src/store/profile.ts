...
// /api/profiles/active `current`). "default" is the root ~/.prostor. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'prostor.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'prostor.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.prostorDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new PROSTOR_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.prostorDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.prostorDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'prostor.desktop.showAllProfiles'
...
// Switch to the default (root ~/.prostor) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.prostorDesktop?.touchBackend?.(target).catch(() => undefined)
}
