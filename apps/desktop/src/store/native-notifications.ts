...
const STORAGE_KEY = 'prostor:native-notifications'
...
// "Backgrounded" = the user isn't on Prostor. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.prostorDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.prostorDesktop
