...
const CONFIGURED_CACHE_KEY = 'prostor-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'prostor-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Prostor is ready', message: `${provider} connected.` })
}
...
async function fetchProviderDefaultModel(
  ...
  // `prostor model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Prostor still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Prostor still cannot resolve a usable provider.'
}
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Prostor still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Prostor still cannot reach ${url}.` }
    }
