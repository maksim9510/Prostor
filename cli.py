#!/usr/bin/env python3
"""
Prostor Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Prostor Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills prostor-agent-dev,github-auth
...
from prostor_cli.cli_pet_mixin import CLIPetMixin
from prostor_cli.cli_show_mixin import CLIShowMixin
...
try:
    from prostor_cli.pt_input_extras import (
...
from prostor_cli.banner import _format_context_length, format_banner_version_label
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("PROSTOR_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    If PROSTOR_IGNORE_USER_CONFIG=1 is set (via ``prostor chat --ignore-user-config``),
    the user config at ``~/.prostor/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({PROSTOR_HOME}/config.yaml)
    user_config_path = _prostor_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("PROSTOR_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Prostor, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Prostor - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see prostor_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and ProstorCLI.__init__ checks "default" first.
    ...
    # prostor_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `prostor config`/`doctor`/guards (which use load_config) honor it. The
    ...
    _is_gateway = os.environ.get("_PROSTOR_GATEWAY") == "1"
...
# Initialize centralized logging early — agent.log + errors.log in ~/.prostor/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from prostor_logging import setup_logging
...
try:
    from prostor_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``prostor --help`` paths inside the chat command flow).  See
...
def _prepare_deferred_agent_startup() -> None:
    ...
    _accept_hooks = os.environ.get("PROSTOR_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`prostor_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        _prostor_home_maint = _get_prostor_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_prostor_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`prostor_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``prostor/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("prostor-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``prostor/prostor-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``prostor -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("prostor/prostor-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. PROSTOR_LIGHT / PROSTOR_TUI_LIGHT env (true/false) — explicit override
#   2. PROSTOR_TUI_THEME=light|dark — explicit theme
#   3. PROSTOR_TUI_BACKGROUND=#RRGGBB — explicit bg hint
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_prostor_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._prostor_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_prostor_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._prostor_bp_start = now
        ...
        _vt100_mod._prostor_bp_timeout_patched = True
...
# ASCII Art - Prostor Caduceus (compact, fits in left panel)
PROSTOR_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if os.environ.get("PROSTOR_FAST_STARTUP_BANNER") == "1":
        from prostor_cli import __release_date__ as _release_date
        from prostor_cli import __version__ as _version

        version_line = f"Prostor Agent v{_version} ({_release_date})"
    else:
...
class ProstorCLI(CLIAgentSetupMixin, CLIApprovalMixin, CLIBillingMixin, CLICommandsMixin, CLIPetMixin, CLIShowMixin, CLIStatusBarMixin, CLIStreamingMixin, CLIUiMixin, CLIVoiceMixin):
    """
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: PROSTOR_MAX_TOKENS)
        ...
        elif os.getenv("PROSTOR_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("PROSTOR_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `prostor chat --ignore-rules` in prostor_cli/main.py. When true we
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `prostor update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Prostor processes for this PROSTOR_HOME.
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``prostor pets`` without a restart, mirroring
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from prostor_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from prostor_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``prostor doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``prostor doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Prostor needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Prostor LLM (not agentic)
        from prostor_cli.model_switch import is_nous_prostor_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_prostor_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Prostor 3 & 4 models are NOT agentic and are not "
                "designed for use with Prostor Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.prostor/images/ and appends the path to
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``prostor sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `prostor sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``prostor --resume <id>``
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from prostor_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `prostor model` and gateway pickers).
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in prostor_cli/commands.py instead of touching every dispatch site.
        from prostor_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from prostor_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from prostor_cli.skin_engine import get_active_skin
            ...
            from prostor_cli.main import _print_version_info
            ...
            try:
                # Discover from disk (bundled + user), matching `prostor plugins
                ...
                # catalog lives behind `prostor plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: prostor plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_prostor_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: prostor plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: prostor plugins list)")
                    print("  Enable/disable: prostor plugins enable/disable <name>")
            except Exception as e:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from prostor_cli.commands import COMMANDS
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``PROSTOR_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``PROSTOR_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from prostor_cli.partial_compress import (
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from prostor_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.prostor/skills/ and queue a note for the
        next user turn.
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _prostor_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard PROSTOR_HOME, so no -p needed.
            try:
            ...
            print(f"  prostor --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  prostor -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from prostor_cli.skin_engine import get_active_goodbye
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Prostor Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Prostor Agent! Type your message or /help for commands."
        ...
        try:
            from prostor_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(PROSTOR_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Prostor migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from prostor_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in prostor_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from prostor_cli.model_switch import resolve_persist_behavior
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Prostor binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        # Dynamic prompt: shows Prostor symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Prostor needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Prostor needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Prostor needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = ProstorCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_prostor_osd_patched", False):
                ...
                _pt_renderer._prostor_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``PROSTOR_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("PROSTOR_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the prostor process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: prostor setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `prostor sessions list`
...
def _run_kanban_goal_loop_q(cli: "ProstorCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``PROSTOR_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("PROSTOR_KANBAN_TASK") or "").strip()
    if not task_id:
        return
...
def main(
    ...
    if toolsets:
        ...
        # Coding posture (base Prostor): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside ProstorCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  PROSTOR_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``prostor chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Prostor" box, no tool-gen
                        ...
                        if os.environ.get("PROSTOR_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("PROSTOR_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from prostor_cli.kanban_db import (
                ...
                # Single-query mode (`prostor chat -q "…"`): skip the welcome
