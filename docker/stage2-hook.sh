...
#
# Wired into the image as /etc/cont-init.d/01-prostor-setup by the
...
PROSTOR_HOME="${PROSTOR_HOME:-/opt/data}"
...
# starts non-root. The baked install tree under /opt/prostor is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $PROSTOR_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass PROSTOR_UID/PROSTOR_GID — or the PUID/PGID aliases — which the
...
# `id -u prostor` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# prostor UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u prostor)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-prostor UID).
...
and the baked /opt/prostor install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e PROSTOR_UID=\$(id -u) -e PROSTOR_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the prostor user to that UID/GID at boot and chowns the data
...
# --- Bootstrap PROSTOR_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid prostor mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom PROSTOR_HOME paths whose parents only
# root can create (e.g. `PROSTOR_HOME=/home/prostor/.prostor` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for PROSTOR_UID/PROSTOR_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  PROSTOR_UID/
# PROSTOR_GID still win when both are set.  See #15290, salvages #25872.
PROSTOR_UID="${PROSTOR_UID:-${PUID:-}}"
PROSTOR_GID="${PROSTOR_GID:-${PGID:-}}"

if [ -n "${PROSTOR_UID:-}" ] && validate_uid_gid "$PROSTOR_UID" && [ "$PROSTOR_UID" != "$(id -u prostor)" ]; then
    echo "[stage2] Changing prostor UID to $PROSTOR_UID"
    usermod -u "$PROSTOR_UID" prostor
fi
if [ -n "${PROSTOR_GID:-}" ] && validate_uid_gid "$PROSTOR_GID" && [ "$PROSTOR_GID" != "$(id -g prostor)" ]; then
    echo "[stage2] Changing prostor GID to $PROSTOR_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$PROSTOR_GID" prostor 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised prostor user (UID 10000)
...
# prostor process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes prostor. Idempotent across container
...
#   - prostor is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G prostor 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] prostor already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" prostor 2>/dev/null; then
        echo "[stage2] Added prostor to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group prostor failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When PROSTOR_UID is remapped or the top-level $PROSTOR_HOME isn't owned by
# the runtime prostor UID, restore ownership to prostor — but ONLY for the
# directories prostor actually writes to. The full $PROSTOR_HOME may be a
...
if [ "$needs_chown" = true ]; then
    ...
    #
    # Top-level $PROSTOR_HOME: chown the directory itself (not its contents)
    # so prostor can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown prostor:prostor "$PROSTOR_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $PROSTOR_HOME failed (rootless container?) — continuing"
    # Prostor-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by prostor (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$PROSTOR_HOME/$sub" ]; then
            chown -R prostor:prostor "$PROSTOR_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $PROSTOR_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi
...
# prostor user. Hosted/container instances keep mutable state under
# $PROSTOR_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# PROSTOR_DISABLE_LAZY_INSTALLS=1. Keeping /opt/prostor root-owned and
...
# Always reset ownership of $PROSTOR_HOME/profiles to prostor on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> prostor …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as prostor and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$PROSTOR_HOME/profiles" ]; then
    chown -R prostor:prostor "$PROSTOR_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $PROSTOR_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged prostor runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$PROSTOR_HOME/cron" ]; then
    chown -R prostor:prostor "$PROSTOR_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of prostor-owned top-level state files on every boot.
# The targeted data-volume chown above only covers prostor-owned
# *subdirectories*; loose state files living directly under $PROSTOR_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> prostor …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged prostor runtime then hits
...
for f in \
    ...
    active_profile; do
    if [ -e "$PROSTOR_HOME/$f" ]; then
        chown prostor:prostor "$PROSTOR_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the prostor runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$PROSTOR_HOME/config.yaml" ]; then
    chown prostor:prostor "$PROSTOR_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$PROSTOR_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as prostor user ---
# Run as prostor via s6-setuidgid so dirs end up owned correctly (matters
...
as_hermes mkdir -p \
    ...
    "$PROSTOR_HOME/logs" \
    "$PROSTOR_HOME/logs/gateways" \
    "$PROSTOR_HOME/hooks" \
    "$PROSTOR_HOME/memories" \
    "$PROSTOR_HOME/skills" \
    "$PROSTOR_HOME/skins" \
    "$PROSTOR_HOME/plans" \
    "$PROSTOR_HOME/workspace" \
    "$PROSTOR_HOME/home" \
    "$PROSTOR_HOME/pairing" \
    "$PROSTOR_HOME/platforms/pairing"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/prostor/.install_method (see Dockerfile), NOT written here into
# $PROSTOR_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $PROSTOR_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.prostor:/opt/data) and sometimes shared with a
...
# 'prostor update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $PROSTOR_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/prostor stamp, so deleting the data-dir copy is safe).
if [ -f "$PROSTOR_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$PROSTOR_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$PROSTOR_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
if [ -f "$PROSTOR_HOME/.env" ]; then
    chown prostor:prostor "$PROSTOR_HOME/.env" 2>/dev/null || true
    chmod 600 "$PROSTOR_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $PROSTOR_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `prostor update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set PROSTOR_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$PROSTOR_HOME/config.yaml" ]; then
    s6-setuidgid prostor "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$PROSTOR_HOME/auth.json" ] && [ -n "${PROSTOR_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$PROSTOR_AUTH_JSON_BOOTSTRAP" > "$PROSTOR_HOME/auth.json"
    chown prostor:prostor "$PROSTOR_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$PROSTOR_HOME/auth.json"
fi
...
# can set PROSTOR_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# PROSTOR_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$PROSTOR_HOME/gateway_state.json" ] && \
        [ "${PROSTOR_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$PROSTOR_HOME/gateway_state.json"
    chown prostor:prostor "$PROSTOR_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$PROSTOR_HOME/gateway_state.json"
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/prostor/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Prostor spawns for the
...
# main-wrapper.sh propagates it into the supervised ``prostor`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-prostor, dashboard, etc.).
