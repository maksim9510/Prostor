...
Plugins built on Prostor Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Prostor
...
:class:`~prostor_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Prostor' main config
...
handles every provider, fallback chain, and per-task override Prostor
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`prostor_cli.plugins.PluginContext`
