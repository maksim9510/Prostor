...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Prostor with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.prostor/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Prostor
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``prostor tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``prostor setup`` for
