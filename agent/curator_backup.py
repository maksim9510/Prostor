...
A pre-run snapshot of ``~/.prostor/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.prostor/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.prostor/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def snapshot_skills(reason: str = "manual", *, protect_ids: set[str] | None = None) -> Path | None:
    """Create a tar.gz snapshot of ``~/.prostor/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.prostor/skills/ directory — nothing to back up")
        return None
...
def _prune_old(keep: int, protect: set[str] | None = None) -> list[str]:
...
def rollback(backup_id: str | None = None) -> tuple[bool, str, Path | None]:
    """Restore ``~/.prostor/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.prostor/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `prostor curator rollback --list` to see available snapshots)",
            None,
        )
