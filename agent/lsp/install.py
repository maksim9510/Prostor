...
appropriate.  All installs go to a Prostor-owned bin staging dir,
``<PROSTOR_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``prostor lsp
  status``.
...
# ``<PROSTOR_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def prostor_lsp_bin_dir() -> Path:
    """Return the Prostor-owned bin staging dir for LSP servers."""
    home = os.environ.get("PROSTOR_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".prostor")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = prostor_lsp_bin_dir().parent  # <PROSTOR_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = prostor_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    console script.
    """
    pip_target = prostor_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = prostor_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``prostor lsp status`` CLI to give users a quick
