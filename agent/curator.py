...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `prostor update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``prostor curator run --consolidate`` flag overrides this for
...
def should_run_now(now: datetime | None = None) -> bool:
    ...
    ``prostor update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``prostor curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `prostor curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Prostor' background skill CURATOR. This is an "
    ...
    "into ~/.prostor/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: prostor-config-*, prostor-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.prostor/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.prostor/logs/curator/``)
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every prostor install already has it
...
def _reconcile_classification(
    ...
    {e["name"] for e in heuristic.get("pruned", [])}
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: prostor curator status
        keep an umbrella stable: prostor curator pin document-tools
    ...
    lines.append("full report: prostor curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: prostor curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.prostor/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.prostor/skills/.archive/` for "
            "safety and can be restored via `prostor curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.prostor/skills/.archive/`. "
            "Restore any via `prostor curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `prostor curator restore <name>`")
    lines.append("- All archives live under `~/.prostor/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``prostor curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `prostor curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `prostor update` only if "
        ...
        # recorded in state so `prostor curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``prostor model`` →
...
def _run_llm_review(prompt: str) -> dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `prostor model` → auxiliary
