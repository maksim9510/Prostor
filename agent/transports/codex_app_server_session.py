...
Owns one Codex thread per Prostor session. Drives `turn/start`, consumes
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Prostor/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Prostor session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Prostor' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Prostor' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own prostor-tools callback we
            # auto-accept — the user already approved Prostor' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Prostor approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Prostor returns 'once', 'session', 'always', or 'deny'.
