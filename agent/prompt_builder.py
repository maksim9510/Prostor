...
DEFAULT_AGENT_IDENTITY = (
    "You are Prostor Agent, an intelligent AI assistant created by Nous Research. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.prostor/kanban.db`. Your task id is in "
    "`$PROSTOR_KANBAN_TASK`; your workspace is `$PROSTOR_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $PROSTOR_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `prostor kanban <verb>` for board operations. Use "
...
#
# The prostor-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to prostor-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Prostor "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Prostor WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Prostor itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Prostor runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Prostor itself is running. The host OS, home, and cwd "
                f"of the Prostor process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Prostor "
    ...
    # Prostor desktop GUI — any agent running under the desktop app should know
    # it. PROSTOR_DESKTOP marks the backend powering the chat; PROSTOR_DESKTOP_TERMINAL
    # marks a prostor launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("PROSTOR_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("PROSTOR_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Prostor desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Prostor
    ...
    extra = (os.getenv("PROSTOR_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_prostor_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.prostor/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("PROSTOR_PLATFORM")
        or get_session_env("PROSTOR_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Prostor Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `prostor-agent` skill "
            "first. It has the actual commands (e.g. `prostor config set …`, `prostor tools`, "
            "`prostor setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: prostor setup, prostor setup tools, prostor setup terminal, prostor status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    """
    try:
        from prostor_cli.config import ensure_prostor_home
        ensure_prostor_home()
    except Exception as e:
        logger.debug("Could not ensure PROSTOR_HOME before loading SOUL.md: %s", e)

    soul_path = get_prostor_home() / "SOUL.md"
...
def _load_prostor_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    if not prostor_md_path:
        return ""
    try:
        content = prostor_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = prostor_md_path.name
        try:
            rel = str(prostor_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".prostor.md", context_length=context_length,
            read_path=str(prostor_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", prostor_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .prostor.md / PROSTOR.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from PROSTOR_HOME is independent and always included when present.
    ...
    project_context = (
        _load_prostor_md(cwd_path, context_length)
    ...
    # SOUL.md from PROSTOR_HOME only — skip when already loaded as identity
    if not skip_soul:
