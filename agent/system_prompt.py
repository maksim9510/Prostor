...
upstream prefix cache warm.  See ``prostor-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: str | None = None) -> dict[str, str]:
    ...
    AIAgent.  Prostor never re-renders parts of this string mid-
    ...
    # Pointer to the prostor-agent skill + docs for user questions about Prostor itself.
    stable_parts.append(PROSTOR_AGENT_HELP_GUIDANCE)
    ...
    # PROSTOR_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Prostor, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Prostor profile the agent is running
    # under so it doesn't conflate ~/.prostor/skills/ (default profile) with
    # ~/.prostor/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Prostor profile: default. Other profiles (if any) live "
            "under ~/.prostor/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Prostor profile: {active_profile}. This session reads "
            f"and writes ~/.prostor/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.prostor/skills/, ~/.prostor/plugins/, "
            f"~/.prostor/cron/, ~/.prostor/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: str | None = None) -> str:
    ...
    one cached block — Prostor never rebuilds or reinjects parts of it
