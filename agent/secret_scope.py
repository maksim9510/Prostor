...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the PROSTOR_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "PROSTOR_KANBAN_DB", "PROSTOR_KANBAN_WORKSPACES_ROOT", "PROSTOR_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "PROSTOR_KANBAN_",
    "PROSTOR_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
