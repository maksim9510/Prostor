...
Prostor pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.prostor/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<prostor_home>/bin/bws`` on
  first use.  Prostor pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.prostor/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``prostor`` invocations don't
  hammer the API.
* Failures NEVER block Prostor startup.  Missing binary, no network,
...
# <prostor_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.prostor/.env (which
...
def _disk_cache_path(home_path: Path | None = None) -> Path:
    ...
    to `$PROSTOR_HOME` / `~/.prostor` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("PROSTOR_HOME", Path.home() / ".prostor"))
...
def find_bws(*, install_if_missing: bool = False) -> Path | None:
    ...
    Resolution order:
      1. ``<prostor_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _prostor_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``prostor secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _prostor_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="prostor-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "prostor-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<prostor_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from prostor_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_prostor_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `prostor secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `prostor secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `prostor secrets bitwarden setup` to install."
        )
        return result
