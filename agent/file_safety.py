...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved PROSTOR_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("PROSTOR_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Prostor path.

    Three categories are blocked:

      * Internal Prostor cache files under ``PROSTOR_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under PROSTOR_HOME and the global Prostor
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.prostor/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active PROSTOR_HOME (profile-aware) AND the global
    # Prostor root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in prostor_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Prostor credential store "
    ...
    for hd in prostor_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Prostor MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Prostor MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Prostor profiles are separate PROSTOR_HOME dirs under
...
#
# Reference: May 2026 incident where a prostor-security profile session
# edited skills under both ``~/.prostor/profiles/prostor-security/skills/``
# AND ``~/.prostor/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under PROSTOR_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from PROSTOR_HOME.

    ``~/.prostor``              -> ``"default"``
    ``~/.prostor/profiles/X``  -> ``"X"``
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Prostor scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Prostor entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Prostor profile {info['target_profile']!r}, but the "
...
#
#   <PROSTOR_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.prostor/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.prostor/…`` segment and warns
# regardless of which Prostor profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.prostor/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.prostor`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.prostor/…`` and returns the
    index where the inner Prostor-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .prostor / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".prostor":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Prostor state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.prostor``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.prostor`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Prostor resolver to
    succeed, so it works correctly even when called from contexts where
    PROSTOR_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Prostor process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real PROSTOR_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.prostor/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.prostor/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host PROSTOR_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Prostor state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Prostor "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real PROSTOR_HOME. Use the "
