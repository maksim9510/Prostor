...
#
# When a user invokes a /skill (or /bundle), Prostor expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_prostor_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``PROSTOR_PLATFORM`` env var and
    ``PROSTOR_SESSION_PLATFORM`` from the gateway session context. Returns
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.prostor/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.prostor.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_prostor_home()}/config.yaml):"]
