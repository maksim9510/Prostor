...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Prostor also honours PROSTOR_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("PROSTOR_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Prostor Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Prostor from probing-down to
...
_URL_TO_PROVIDER: dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Prostor' system prompt, but mapping
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Prostor grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from prostor_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Prostor sends to providers:
