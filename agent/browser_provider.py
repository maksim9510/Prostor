...
``kind: backend``) or ``~/.prostor/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``prostor tools``. Defaults to ``name``."""
        return self.name
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`prostor_cli.tools_config` to inject this provider as a
