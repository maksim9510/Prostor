...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.prostor/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Prostor ↔ models.dev
# ---------------------------------------------------------------------------
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``prostor config refresh``, the
...
# Gemma models whose TPM quotas are too small for normal Prostor agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    """
    # Resolve Prostor ID → models.dev ID
