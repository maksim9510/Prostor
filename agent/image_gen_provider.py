...
as ``kind: backend``) or ``~/.prostor/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``prostor tools``. Defaults to ``name.title()``."""
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``prostor tools``
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$PROSTOR_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$PROSTOR_HOME/cache/images/``.
