...
* First-use consent is gated by the allowlist under
  ``~/.prostor/shell-hooks-allowlist.json``.  Non-TTY callers must pass
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Prostor-canonical
...
def register_from_config(
    ...
    *,
    ...
    setting.  ``PROSTOR_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / PROSTOR_ACCEPT_HOOKS=1 / "
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / PROSTOR_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Prostor is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``PROSTOR_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("PROSTOR_ACCEPT_HOOKS", "").strip().lower()
...
def run_once(
    ...
    holding the canonical Prostor-wire-shape response."""
