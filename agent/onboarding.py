...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Prostor starts and finds ``~/.openclaw/``.

    Points users at ``prostor claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``prostor claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Prostor, run "
        "`prostor claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `prostor claw cleanup` (renames it to ~/.openclaw.pre-migration — "
