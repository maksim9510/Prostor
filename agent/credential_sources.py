"""Unified removal contract for every credential source Prostor reads from.

Prostor seeds its credential pool from many places:
    ...
    manual        — user ran `prostor auth add`
...
unify here is **removal**:
    ...
    ``prostor auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.prostor/.env  → clear it
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.prostor/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Prostor.",
            f"  The pool entry is now suppressed — Prostor will ignore "
            f"{env_var} until you run `prostor auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Prostor stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `prostor auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Prostor Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `prostor auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `prostor auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``prostor auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``prostor auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `prostor model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Prostor auth store is not enough — _seed_from_singletons() would
    ...
    via ``prostor auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `prostor auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `prostor auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Prostor stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `prostor auth add copilot` to re-enable if needed.",
    ])
