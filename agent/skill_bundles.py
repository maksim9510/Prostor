...
Storage
-------
Bundles live in ``~/.prostor/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``PROSTOR_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``prostor bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under PROSTOR_HOME.

    Honors ``PROSTOR_BUNDLES_DIR`` for tests; falls back to
    ``<PROSTOR_HOME>/skill-bundles``.
    """
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `prostor bundles` CLI subcommand.
# ---------------------------------------------------------------------------
