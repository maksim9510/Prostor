...
as ``kind: backend``) or ``~/.prostor/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``prostor tools``
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$PROSTOR_HOME/cache/videos/``.
