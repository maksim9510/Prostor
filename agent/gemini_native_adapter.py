...
Prostor keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Prostor's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Prostor)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Prostor typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Prostor retry/error classification."""
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (Prostor
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.prostor/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `prostor setup` "
    ...
    def _headers(self) -> dict[str, str]:
        headers = {
            ...
            "User-Agent": "prostor-agent (gemini-native)",
        }
