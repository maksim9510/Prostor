...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``PROSTOR_KANBAN_TASK`` / ``PROSTOR_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("PROSTOR_KANBAN_TASK") or os.getenv("PROSTOR_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Prostor Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``PROSTOR_PLATFORM`` or
            ``PROSTOR_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``prostor`` startup when ~120 skills
...
def get_external_skills_dirs() -> list[Path]:
    ...
    paths that resolve to the local ``~/.prostor/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``prostor`` cold-start time
...
def get_all_skills_dirs() -> list[Path]:
    """Return all skill directories: local ``~/.prostor/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: dict[str, Any]) -> dict[str, list]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    prostor = metadata.get("prostor") or {}
    if not isinstance(prostor, dict):
        prostor = {}
    return {
        "fallback_for_toolsets": prostor.get("fallback_for_toolsets", []),
        "requires_toolsets": prostor.get("requires_toolsets", []),
        "fallback_for_tools": prostor.get("fallback_for_tools", []),
        "requires_tools": prostor.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    prostor = metadata.get("prostor")
    if not isinstance(prostor, dict):
        return []
    raw = prostor.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Prostor metadata, VCS, virtualenv/dependency, cache, and skill
