# OpenAI-Compatible API Server for Prostor Agent
...
SSE streaming. By exposing this endpoint, prostor-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  prostor-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.prostor/.env`
2. Run `prostor gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with prostor-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns prostor-agent as a model) |
...
Authorization: Bearer prostor-api-key-here
...
But prostor-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "prostor-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured prostor-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of prostor-agent's core prompt
...
Once implemented, prostor-agent works as a drop-in backend for:
