"""Persistent slash-command worker — one ProstorCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``PROSTOR_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("PROSTOR_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("PROSTOR_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: ProstorCLI, command: str) -> str:
...
def main():
    ...
    os.environ["PROSTOR_SESSION_KEY"] = args.session_key
    os.environ["PROSTOR_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) ProstorCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ProstorCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
