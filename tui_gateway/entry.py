...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `PROSTOR_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("PROSTOR_TUI_SIDECAR_URL")
...
# tested; override via ``PROSTOR_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("PROSTOR_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``PROSTOR_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``prostor --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def main():
    ...
    # server stalled the whole shell on "summoning prostor…" for the full
