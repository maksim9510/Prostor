# ============================================================================
# Prostor Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # builds apps/desktop into a launchable Prostor.exe.
    #
    # Why opt-in:
    #   * Prostor-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Prostor.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Prostor.exe
    ...
    #     `prostor desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "|             * Prostor Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Resolve-UvCmd {
    ...
    # sibling process and PROSTOR_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Prostor uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\prostor\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Prostor needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Prostor copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\prostor\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``PROSTOR_GIT_BASH_PATH`` (User scope) so Prostor can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Prostor features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Prostor needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``PROSTOR_GIT_BASH_PATH`` (User env scope) so Prostor can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("PROSTOR_GIT_BASH_PATH", $candidate, "User")
            $env:PROSTOR_GIT_BASH_PATH = $candidate
            Write-Info "Set PROSTOR_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Prostor may not find Git Bash."
    Write-Info "If needed, set PROSTOR_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    Write-Info "Installing Prostor-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\prostor-node-extract"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\prostor-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `prostor
                    # update` path (#4735).
                    ...
                    $stashName = "prostor-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Prostor behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running prostor processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\prostor-agent-extract"
    ...
    # next `prostor update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        # DLLs by any running prostor process. Windows denies deletion of loaded
        # DLLs, so kill any prostor.exe tree before removing the venv.
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running prostor processes before recreating venv..."
            & taskkill /F /T /IM prostor.exe /FI "PID ne $myPid" 2>$null | Out-Null
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `prostor.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install prostor-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and prostor.exe broken with
    ...
    # here, not 30 seconds later when the user runs `prostor`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `prostor dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `prostor dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `prostor dashboard` should now work."
            } else {
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.prostor-bootstrap-complete which tells the Prostor
    ...
    # Prostor-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `prostor update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".prostor-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # (the default PS5 behaviour), and Prostor's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Prostor-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'prostor setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\prostor-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\prostor-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\prostor-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Prostor` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Prostor.exe. Only called from
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Prostor-managed
    ...
    # The --dir mode produces an unpacked Prostor.exe in
    ...
    # are NOT trying to work around). The Prostor icon + product name are
    # stamped onto Prostor.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\prostor-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Prostor.exe",
        "$desktopDir\release\win-arm64-unpacked\Prostor.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Prostor.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Prostor icon + identity are stamped onto Prostor.exe by the
    ...
    #    Prostor.exe. We deliberately do NOT point them at `prostor desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Prostor.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Prostor.lnk')
        )
        ...
        # with the Prostor icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.prostor/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'prostor setup'."
        return
    }
...
function Start-GatewayIfConfigured {
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'prostor whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: prostor gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: prostor gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: prostor gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   prostor              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   prostor setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   prostor config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   prostor config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   prostor gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   prostor update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Prostor repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Prostor-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Prostor to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
