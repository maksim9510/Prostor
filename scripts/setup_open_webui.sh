...
# Bootstrap Open WebUI against Prostor Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.prostor/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-prostor.sh
...
#   OPEN_WEBUI_NAME='Johnny Prostor'
...
#   PROSTOR_API_PORT=8642
#   PROSTOR_API_HOST=127.0.0.1
#   PROSTOR_API_MODEL_NAME='Prostor Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Prostor Agent WebUI}"
...
PROSTOR_ENV_FILE="${PROSTOR_ENV_FILE:-$HOME/.prostor/.env}"
PROSTOR_API_PORT="${PROSTOR_API_PORT:-8642}"
PROSTOR_API_HOST="${PROSTOR_API_HOST:-127.0.0.1}"
PROSTOR_API_CONNECT_HOST="${PROSTOR_API_CONNECT_HOST:-127.0.0.1}"
PROSTOR_API_MODEL_NAME="${PROSTOR_API_MODEL_NAME:-Prostor Agent}"
PROSTOR_API_BASE_URL="http://${PROSTOR_API_CONNECT_HOST}:${PROSTOR_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-prostor.sh"
LOG_DIR="$HOME/.prostor/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$PROSTOR_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$PROSTOR_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.prostor.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.prostor</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.prostor"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.prostor"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-prostor.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Prostor Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-prostor.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.prostor/logs/openwebui.log
StandardError=append:%h/.prostor/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-prostor.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$PROSTOR_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Prostor API server is configured...'
  upsert_env API_SERVER_ENABLED true "$PROSTOR_ENV_FILE"
  upsert_env API_SERVER_HOST "$PROSTOR_API_HOST" "$PROSTOR_ENV_FILE"
  upsert_env API_SERVER_PORT "$PROSTOR_API_PORT" "$PROSTOR_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$PROSTOR_API_MODEL_NAME" "$PROSTOR_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$PROSTOR_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Prostor gateway so API server settings take effect...'
  prostor gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${PROSTOR_API_CONNECT_HOST}:${PROSTOR_API_PORT}/health" >/dev/null; then
    log 'Prostor API server did not answer on the first check. Trying to start gateway in the background...'
    nohup prostor gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${PROSTOR_API_CONNECT_HOST}:${PROSTOR_API_PORT}/health" >/dev/null
  ...
  log "Prostor API endpoint: ${PROSTOR_API_BASE_URL}"
