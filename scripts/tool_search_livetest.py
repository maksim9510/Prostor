#!/usr/bin/env python3
"""Live test harness for Prostor Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any prostor imports.
ORIGINAL_HOME = os.environ.get("PROSTOR_HOME")
ORIGINAL_AUTH = Path.home() / ".prostor" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.prostor/ for one test, copying minimal credentials.
...
def run_one_scenario(scenario: dict[str, Any], enabled: bool, out_dir: Path) -> dict[str, Any]:
    ...
    os.environ["PROSTOR_HOME"] = str(home)
...
def main():
    ...
    # Restore original PROSTOR_HOME
    if ORIGINAL_HOME is not None:
        os.environ["PROSTOR_HOME"] = ORIGINAL_HOME
    else:
