...
# Python-driven tool session (e.g. Prostor terminal tool). A pre-set PYTHONPATH
...
if [ -n "${PROSTOR_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$PROSTOR_INSTALL_DIR"
...
#   code at /usr/local/lib/prostor-agent, command at /usr/local/bin/prostor,
#   data still at /root/.prostor (PROSTOR_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$PROSTOR_HOME/.no-bundled-skills so future"
            echo "                   'prostor update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Prostor.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.prostor/prostor-agent"
            echo "                   default (root, Linux): /usr/local/lib/prostor-agent"
            echo "  --prostor-home PATH  Data directory (default: ~/.prostor, or \$PROSTOR_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Prostor installs the code under"
            echo "  /usr/local/lib/prostor-agent and links the command into"
            echo "  /usr/local/bin/prostor (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$PROSTOR_HOME"
            echo "  (default /root/.prostor).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$PROSTOR_HOME/prostor-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs prostor setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `prostor update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Prostor-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Prostor Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install prostor command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `prostor` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $PROSTOR_HOME/prostor-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $PROSTOR_HOME/prostor-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/prostor-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $PROSTOR_HOME/prostor-agent — then preserve it)
#
# Always no-op when the user set --dir or $PROSTOR_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in PROSTOR_HOME.
    if is_termux; then
        INSTALL_DIR="$PROSTOR_HOME/prostor-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$PROSTOR_HOME/prostor-agent/.git" ]; then
            INSTALL_DIR="$PROSTOR_HOME/prostor-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/prostor-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/prostor-agent"
        ...
        # /usr/local/bin/prostor wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/prostor"
        log_info "  Data:    $PROSTOR_HOME (unchanged)"
...
# Point a Prostor-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $PROSTOR_HOME/node/bin — which is NOT on
...
# Prostor's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Prostor-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$PROSTOR_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$PROSTOR_HOME/node/etc"
...
install_uv() {
    ...
    log_info "Installing managed uv into $PROSTOR_HOME/bin ..."
    mkdir -p "$PROSTOR_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/prostor-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/prostor-uv-installer.$$.sh")"
    ...
    # directly into $PROSTOR_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$PROSTOR_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Prostor-
...
check_node() {
    ...
    # Repair pre-existing Prostor-managed installs where `npm install -g` lands
    ...
    if [ -x "$PROSTOR_HOME/node/bin/node" ] && node_satisfies_build "$("$PROSTOR_HOME/node/bin/node" --version)"; then
        export PATH="$PROSTOR_HOME/node/bin:$PATH"
        log_success "Node.js $("$PROSTOR_HOME/node/bin/node" --version) found (Prostor-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Prostor-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.prostor/node/..."
    ...
    # Place into ~/.prostor/node/ and symlink binaries into the same bin dir
    # the prostor command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$PROSTOR_HOME/node"
    mkdir -p "$PROSTOR_HOME"
    mv "$extracted_dir" "$PROSTOR_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$PROSTOR_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$PROSTOR_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$PROSTOR_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$PROSTOR_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.prostor/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Prostor install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Prostor Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `prostor update` path
                # (#4735).
                ...
                local stash_name
                stash_name="prostor-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Prostor behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"prostor-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up prostor command..."

    if [ "$USE_VENV" = true ]; then
        PROSTOR_BIN="$INSTALL_DIR/venv/bin/prostor"
    else
        PROSTOR_BIN="$(which prostor 2>/dev/null || echo "")"
        if [ -z "$PROSTOR_BIN" ]; then
            log_warn "prostor not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$PROSTOR_BIN" ]; then
        log_warn "prostor entry point not found at $PROSTOR_BIN"
    ...
    # Create a user-facing shim for the prostor command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $PROSTOR_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$PROSTOR_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/prostor"
    cat > "$command_link_dir/prostor" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PROSTOR_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/prostor"
    log_success "Installed prostor launcher → $command_link_display_dir/prostor"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "prostor command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v prostor' \
                >/dev/null 2>&1; then
        ...
        log_info "prostor not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Prostor Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Prostor Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Prostor Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so prostor works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.prostor directory structure (config at top level, code in subdir)
    mkdir -p "$PROSTOR_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.prostor/.env (top level, easy to find)
    if [ ! -f "$PROSTOR_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$PROSTOR_HOME/.env"
            log_success "Created ~/.prostor/.env from template"
        else
            touch "$PROSTOR_HOME/.env"
            log_success "Created ~/.prostor/.env"
        fi
    else
        log_info "~/.prostor/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$PROSTOR_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.prostor/config.yaml (top level, easy to find)
    if [ ! -f "$PROSTOR_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$PROSTOR_HOME/config.yaml"
            log_success "Created ~/.prostor/config.yaml from template"
        fi
    else
        log_info "~/.prostor/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$PROSTOR_HOME/SOUL.md" ]; then
        cat > "$PROSTOR_HOME/SOUL.md" << 'SOUL_EOF'
# Prostor Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Prostor communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.prostor/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.prostor/"

    # Seed bundled skills into ~/.prostor/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `prostor update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'prostor update'." \
            > "$PROSTOR_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $PROSTOR_HOME/.no-bundled-skills"
        log_info "  Future 'prostor update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.prostor/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.prostor/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$PROSTOR_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$PROSTOR_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.prostor/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$PROSTOR_HOME/.env"
    ...
    {
        echo ""
        echo "# Prostor Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Prostor will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (prostor --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `prostor update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'prostor setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Prostor to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$PROSTOR_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'prostor whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'prostor gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $PROSTOR_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $PROSTOR_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: prostor gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: prostor gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $PROSTOR_CMD gateway > "$PROSTOR_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.prostor/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: prostor gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: prostor gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $PROSTOR_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $PROSTOR_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $PROSTOR_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}prostor${NC}              Start chatting"
    echo -e "   ${GREEN}prostor setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}prostor config${NC}       View/edit configuration"
    echo -e "   ${GREEN}prostor config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}prostor gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}prostor update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'prostor' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'prostor' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'prostor' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$PROSTOR_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$PROSTOR_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$PROSTOR_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$PROSTOR_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$PROSTOR_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Prostor for pip install"
    ...
    PROSTOR_CMD="$(command -v prostor 2>/dev/null || echo "")"
    if [ -n "$PROSTOR_CMD" ]; then
        log_info "Running prostor setup..."
        "$PROSTOR_CMD" setup
    else
...
# extract a tree MISSING the electron binary, so the `electron`->`Prostor` rename
...
# _purge_electron_build_cache() used by `prostor desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Prostor-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `prostor desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Prostor" ]; then
            app="$desktop_dir/release/linux-unpacked/Prostor"
        elif [ -x "$desktop_dir/release/linux-unpacked/prostor" ]; then
            app="$desktop_dir/release/linux-unpacked/prostor"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Prostor.app" \
            "$desktop_dir/release/mac/Prostor.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Prostor is damaged and can't be opened").
    ...
    # checkout stays clean for the next `prostor update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Prostor-managed Node
            # provisioned during prerequisites/node-deps (at $PROSTOR_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $PROSTOR_HOME.
    # $PROSTOR_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'prostor update' on this host install.
