#!/usr/bin/env python3
"""Drive the Prostor TUI under PROSTOR_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.prostor/perf.log on exit.
...
Environment overrides:
  PROSTOR_PERF_LOG     (default ~/.prostor/perf.log)
  PROSTOR_PERF_NODE    (default node from $PATH)
  PROSTOR_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests PROSTOR_DEV_PERF wiring is broken).
"""
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — PROSTOR_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["PROSTOR_DEV_PERF"] = "1"
    env["PROSTOR_DEV_PERF_MS"] = str(args.threshold_ms)
    env["PROSTOR_DEV_PERF_LOG"] = str(log)
    env["PROSTOR_TUI_RESUME"] = sid
    ...
    node = os.environ.get("PROSTOR_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="PROSTOR_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "prostor-ink" / "src"
