"""SimpleX Chat platform adapter (Prostor plugin).
...
This adapter ships as a Prostor platform plugin under
``plugins/platforms/simplex/``. The Prostor plugin loader scans the
...
Optional environment variables:
    ...
    PROSTOR_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``prostor setup`` can describe it even when websockets is
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``prostor setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.prostor/.env`` via
    ``prostor_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "prostor_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.prostor/.env"
        )
        return
