"""ntfy platform adapter (Prostor plugin).
...
a Prostor dependency.

This adapter ships as a Prostor platform plugin under
``plugins/platforms/ntfy/``. The Prostor plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "prostor-in"              # subscribe topic (incoming)
          publish_topic: "prostor-out"     # optional — defaults to topic
...
async def _standalone_send(
    ...
    *,
    ...
    ``prostor cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Prostor dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `prostor gateway status` without
