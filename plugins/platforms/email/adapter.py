"""
Email platform adapter for the Prostor gateway.

Allows users to interact with Prostor by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "prostor-agent" "version" "{_hermes_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `prostor config show` already read. Without the
    ...
    async def connect(self) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `prostor gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Prostor Agent")
        ...
        msg_id = f"<prostor-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
