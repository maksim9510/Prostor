"""
LINE Messaging API platform adapter for Prostor Agent.
...
to Prostor Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``PROSTOR_HOME``. PR #8398.
        """
        from aiohttp import web
...
def is_connected(config) -> bool:
    """Surface in ``prostor status`` even before the adapter is instantiated."""
...
def _env_enablement() -> dict[str, Any] | None:
    ...
    Lets ``prostor status`` reflect a LINE configuration that lives entirely
