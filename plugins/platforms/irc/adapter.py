"""
IRC Platform Adapter for Prostor Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Prostor agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "prostor-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Prostor Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Prostor Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: prostor_, prostor_1, prostor_2...
...
def check_requirements() -> bool:
    ...
    # prostor setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `prostor gateway setup` flow for the IRC platform.

    Lazy-imports ``prostor_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Prostor to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #prostor — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.prostor/.env")
    print_info("Restart the gateway for changes to take effect: prostor gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``prostor cron`` running as a
    separate process from ``prostor gateway``).  Without this hook,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "prostor-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "prostor-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Prostor Agent (cron)")
