"""
``prostor photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Prostor
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `prostor photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Prostor Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``prostor photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Prostor Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.prostor/.env,
    ...
    print("  Start the gateway:  prostor gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `prostor photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`prostor photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.prostor/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.prostor/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  prostor gateway restart")
    return 0
...
#
# `prostor gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``prostor photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `prostor gateway setup` wizard."""
