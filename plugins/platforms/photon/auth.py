...
Credential storage mirrors every other Prostor channel:

    * runtime SDK creds  -> ``~/.prostor/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.prostor/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a prostor dependency
...
# registers Prostor as its own client_id.
...
# Default name of the project Prostor provisions for the operator.
DEFAULT_PROJECT_NAME = "Prostor Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of prostor-agent.
...
def load_project_credentials() -> tuple[str | None, str | None]:
    ...
    Precedence: process env (``~/.prostor/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.prostor/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.prostor/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `prostor photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `prostor photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `prostor photon setup`)"
    )
...
def credential_summary() -> dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `prostor photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `prostor photon setup`)"
