...
This plugin connects Prostor Agent to iMessage (and other Spectrum
...
TypeScript-only, Prostor runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Prostor-Sidecar-Token`.
...
There is no separate `login` command; like every other Prostor channel,
...
Run `prostor photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.prostor/.env` (the same place every other
...
Management metadata lives in `~/.prostor/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `prostor photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `prostor photon status`, a DM and a group roundtrip,
