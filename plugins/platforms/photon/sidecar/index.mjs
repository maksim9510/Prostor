// Prostor Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Prostor): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Prostor -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Prostor-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `prostor photon telemetry`)
...
// instead of a cryptic module-resolution error during import. Apply Prostor'
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-prostor-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Prostor supplied them so a known-good
