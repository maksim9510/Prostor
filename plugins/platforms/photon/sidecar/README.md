...
Small Node helper that bridges Prostor Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Prostor is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Prostor-Sidecar-Token`)
...
The Prostor plugin's `prostor photon setup` command runs `npm install`
here automatically.
