...
# ``model_tools`` import time, so ``prostor status``, ``prostor chat``, etc.
...
#     fresh "Prostor is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a prostor MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${PROSTOR_HOME}/google_chat_thread_counts.json so
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${PROSTOR_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Prostor is thinking…' marker message.
        ...
        "Prostor is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Prostor is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Prostor is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Prostor is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. prostor-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.prostor/.env")
    print_info("Restart the gateway: prostor gateway restart")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Prostor plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'prostor-agent[google_chat]'",
        ...
        # the "Prostor is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Prostor is thinking…' marker message "
