...
- Per-user tokens (keyed by sender email):
    ``${PROSTOR_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${PROSTOR_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${PROSTOR_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${PROSTOR_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${PROSTOR_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's PROSTOR_HOME helper so the token follows the user's
# profile (e.g. tests can override via PROSTOR_HOME=/tmp/...).
try:
    from prostor_constants import display_prostor_home, get_prostor_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where prostor_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _prostor_home.py shim).
...
def _prostor_home() -> Path:
    """Resolve PROSTOR_HOME at call time (NOT module import).

    Tests and ``PROSTOR_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.prostor/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _prostor_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _prostor_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _prostor_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _prostor_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _prostor_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: str | None = None) -> Any | None:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install prostor-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'prostor-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into PROSTOR_HOME."""
...
def exchange_auth_code(code: str, email: str | None = None) -> None:
    ...
    rel_label = (
        f"{display_prostor_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_prostor_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Prostor (native attachment delivery)"
    )
