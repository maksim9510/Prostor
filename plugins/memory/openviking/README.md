...
connection values into Prostor, or create a minimal `ovcli.conf` when one does
not exist.
...
| `OPENVIKING_AGENT` | `prostor` | Prostor peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Prostor lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Prostor sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
