...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Prostor peer ID in OpenViking (default: prostor)
...
_DEFAULT_AGENT = "prostor"
_AGENT_PROMPT_LABEL = "Prostor peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "PROSTOR_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Prostor slash-skill scaffolding before sending content to OpenViking.
...
def _load_prostor_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("PROSTOR_HOME", "")).expanduser() if os.environ.get("PROSTOR_HOME") else Path.home() / ".prostor"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this Prostor run."
    )
...
def _validate_profile_for_setup(profile: _OvcliProfile) -> tuple[bool, str, str | None]:
...
def _print_openviking_ready(message: str, path: Path | None = None) -> None:
    ...
    print("  Start a new Prostor session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Prostor only", "write values only to Prostor .env"),
    ...
    _save_prostor_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Prostor .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See prostor-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_prostor_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Prostor peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, prostor_home: str, config: dict) -> None:
        ...
        from prostor_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        prostor_home_path = Path(prostor_home)
        env_path = prostor_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_prostor_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Prostor run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Prostor' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Prostor canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See prostor-agent#28296.
    ...
    def _tool_read(self, args: dict) -> str:
        ...
        # (e.g. viking://user/prostor/.overview.md).
