...
Config file: `$PROSTOR_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `prostor` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `prostor-{identity}` → `prostor-coder`). |
...
All Supermemory API calls send `x-sm-source: prostor`, and document writes stamp
`metadata.sm_source: prostor`. This is a **functional routing key, not telemetry**:
it groups Prostor-written memories into a dedicated "Prostor" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Prostor profile:

```json
{
  "container_tag": "prostor-{identity}"
}
```

For a profile named `coder`, this resolves to `prostor-coder`. The default profile resolves to `prostor-default`. Without `{identity}`, all profiles share the same container.
