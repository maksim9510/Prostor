...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with prostor-agent)
2. User-installed providers: ``$PROSTOR_HOME/plugins/<name>/``
...
def _register_synthetic_package(name: str, search_locations: list[str]) -> None:
    ...
    ``ModuleNotFoundError: No module named '_prostor_user_memory'`` — the
...
def load_memory_provider(name: str) -> MemoryProvider | None:
    ...
    (``$PROSTOR_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> list[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _prostor_user_memory.<name>.cli, usually
