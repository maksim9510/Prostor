...
Handles: prostor honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "prostor" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Prostor profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'prostor honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'prostor honcho setup' first.\n")
        return
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Prostor profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $PROSTOR_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Prostor persistent cross-session memory.")
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Prostor gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    try:
        ...
        print("  Run: prostor config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    prostor honcho status     -- show full config")
    print("    prostor honcho mode       -- change recall/observation mode")
    print("    prostor honcho tokens     -- tune context and dialectic budgets")
    print("    prostor honcho peer       -- update peer names")
    print("    prostor honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Prostor profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: prostor honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'prostor honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'prostor honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: prostor honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        prostor = hosts.get(_host_key(), {})
        user = prostor.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = prostor.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = prostor.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = prostor.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Prostor' identity in Honcho. Seed with 'prostor honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: prostor honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: prostor honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = prostor.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = prostor.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = prostor.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Prostor asks Honcho's AI peer a question")
        ...
        print("\n  Set with: prostor honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'prostor honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'prostor honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    prostor honcho identity --show        — show both peer representations")
        print("    prostor honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Prostor + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Prostor\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Prostor persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  prostor honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'prostor honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'prostor honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  prostor honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'prostor'")
        print("  with Honcho configured and no prior session history.")
        print("  (Prostor calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    prostor honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'prostor honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Prostor, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'prostor honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    prostor honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  prostor honcho identity <path/to/SOUL.md>")
    ...
    print("    Prostor:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Prostor:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Prostor:   Honcho observes every message and updates representations")
    ...
    print("    Prostor:   per-session by default — each run gets its own session")
    print("              Map a custom name:  prostor honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. prostor honcho setup              — configure API key (required)")
        print("  2. prostor honcho migrate            — re-run this walkthrough")
    else:
        print("  1. prostor honcho status             — verify Honcho connection")
        print("  2. prostor                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. prostor honcho identity --show    — verify AI peer representation")
        print("  4. prostor honcho tokens             — tune context and dialectic budgets")
        print("  5. prostor honcho mode               — view or change memory mode")
    print()
...
def register_cli(subparser) -> None:
    """Build the ``prostor honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``prostor honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to prostor memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Prostor Honcho",
    )
