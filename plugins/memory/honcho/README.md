...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/prostor>
...
```bash
prostor memory setup honcho   # configure Honcho directly (works on a fresh install)
prostor memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
prostor config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.prostor/.env
```

> `prostor honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `prostor memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$PROSTOR_HOME/honcho.json` | Profile-local (isolated Prostor instances) |
| 2 | `~/.prostor/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Prostor profile: `prostor` (default) or `prostor_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `prostor honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Prostor session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`prostor-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`prostor`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-prostor-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening prostor in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Prostor session ID (timestamp + hex). Every `prostor` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Prostor profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`prostor`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Prostor profile: `prostor` (default) or `prostor_<profile>` (e.g. `prostor -p coder` -> host key `prostor_coder`). Older `prostor.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `PROSTOR_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `prostor memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `prostor honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `prostor memory setup`) |
| `prostor honcho status` | Show resolved config for active profile |
| `prostor honcho enable` / `disable` | Toggle Honcho for active profile |
| `prostor honcho mode <mode>` | Change recall or observation mode |
| `prostor honcho peer --user <name>` | Update user peer name |
| `prostor honcho peer --ai <name>` | Update AI peer name |
| `prostor honcho tokens --context <N>` | Set context token budget |
| `prostor honcho tokens --dialectic <N>` | Set dialectic max chars |
| `prostor honcho map <name>` | Map current directory to a session name |
| `prostor honcho sync` | Create host blocks for all Prostor profiles |
