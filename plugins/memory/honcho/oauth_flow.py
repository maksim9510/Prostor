...
``prostor://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Prostor OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "prostor-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``prostor-desktop`` / ``prostor-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Prostor.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local PROSTOR_HOME override can't reach.
