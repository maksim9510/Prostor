...
Configuration
-------------
Secret (lives in $PROSTOR_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $PROSTOR_HOME/mem0.json, set via `prostor memory
setup`):
  ...
  agent_id           — Agent identifier (default: prostor)
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $PROSTOR_HOME/mem0.json) —
