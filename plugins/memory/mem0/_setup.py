...
def _setup_oss(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "prostor-user")
    ...
    if env_writes:
        _write_env(Path(hermes_home) / ".env", env_writes)
    _save_mem0_json(hermes_home, {"mode": "oss", "user_id": user_id, "agent_id": "prostor", "oss": oss_config})
...
_PGVECTOR_CONTAINER = "prostor-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "prostor"
...
def _setup_oss_interactive(hermes_home: str, config: dict) -> None:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'prostor-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "prostor-user")

    agent_id = input("  Agent ID [prostor]: ").strip()
    agent_id = agent_id or "prostor"
...
def post_setup(hermes_home: str, config: dict) -> None:
    """Entry point called by prostor memory setup framework.
