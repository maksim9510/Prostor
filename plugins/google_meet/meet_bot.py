...
``$PROSTOR_HOME/workspace/meetings/<meeting-id>/``. The main prostor process
...
Run standalone for debugging::
    ...
    PROSTOR_MEET_URL=https://meet.google.com/abc-defg-hij \\
    PROSTOR_MEET_OUT_DIR=/tmp/meet-debug \\
    PROSTOR_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``PROSTOR_MEET_OUT_DIR``.
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("PROSTOR_MEET_URL", "").strip()
    out_dir_env = os.environ.get("PROSTOR_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("PROSTOR_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("PROSTOR_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("PROSTOR_MEET_GUEST_NAME", "Prostor Agent")
    duration_s = _parse_duration(os.environ.get("PROSTOR_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when PROSTOR_MEET_MODE=realtime.
    mode = os.environ.get("PROSTOR_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("PROSTOR_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("PROSTOR_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("PROSTOR_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("PROSTOR_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")
    ...
    if not url or not _is_safe_meet_url(url):
        ...
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: PROSTOR_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in PROSTOR_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("PROSTOR_MEET_LOBBY_TIMEOUT", "300")
            )
