...
Launched by ``prostor meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$PROSTOR_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``prostor meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_prostor_home()) / "workspace" / "meetings" / "node_token.json"
