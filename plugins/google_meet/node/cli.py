"""`prostor meet node ...` subcommand tree.

Wired into the existing ``prostor meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``prostor meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="prostor-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``prostor meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             prostor meet node approve <name> ws://<host>:{args.port} {token}")
        try:
