...
session-scoped state file under ``$PROSTOR_HOME/workspace/meetings/.active.json``
...
def start(
    ...
    *,
    ...
    env = os.environ.copy()
    env["PROSTOR_MEET_URL"] = url
    env["PROSTOR_MEET_OUT_DIR"] = str(out)
    env["PROSTOR_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["PROSTOR_MEET_HEADED"] = "1"
    if auth_state:
        env["PROSTOR_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["PROSTOR_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if PROSTOR_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["PROSTOR_MEET_MODE"] = mode
    if realtime_model:
        env["PROSTOR_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["PROSTOR_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["PROSTOR_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["PROSTOR_MEET_REALTIME_KEY"] = realtime_api_key
