"""disk-cleanup plugin — auto-cleanup of ephemeral Prostor session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``PROSTOR_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$PROSTOR_HOME/disk-cleanup/cleanup.log``.
...
def _extract_paths_from_terminal(args: dict[str, Any], result: str) -> set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/prostor-x/test_foo.py`
        try:
...
All operations are scoped to PROSTOR_HOME and /tmp/prostor-*.
...
def _handle_slash(raw_args: str) -> str | None:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside PROSTOR_HOME): "
            f"{path_arg}"
        )
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Prostor session files.",
    )
