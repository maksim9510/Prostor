...
Rules:
  ...
  - empty dirs    → always delete (under PROSTOR_HOME)
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$PROSTOR_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under PROSTOR_HOME or ``/tmp/prostor-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/prostor-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("prostor-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "prostor-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $PROSTOR_HOME that must NEVER be deleted by quick(),
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside PROSTOR_HOME)")
        return False
...
def quick() -> dict[str, Any]:
    ...
    # Remove empty dirs under PROSTOR_HOME, but never recurse into known
...
def guess_category(path: Path) -> str | None:
    ...
    try:
        ...
        # Path isn't under PROSTOR_HOME (e.g. /tmp/prostor-*) — fall through.
        pass
