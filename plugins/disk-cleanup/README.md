...
Auto-tracks and cleans up ephemeral files created during Prostor Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$PROSTOR_HOME` and `/tmp/prostor-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `PROSTOR_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `PROSTOR_HOME` or `/tmp/prostor-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$PROSTOR_HOME/disk-cleanup/` is itself excluded
- `$PROSTOR_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
