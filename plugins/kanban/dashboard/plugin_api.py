...
def get_board(
    ...
    through to the active board (``PROSTOR_KANBAN_BOARD`` env → on-disk
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``prostor kanban
...
def list_diagnostics(
    ...
    AND for ``prostor kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``prostor kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``prostor kanban specify <task_id>``.
    ...
    # PROSTOR_KANBAN_BOARD env var — this endpoint runs in FastAPI's
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``prostor kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.prostor/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.prostor/profiles/*`` on disk and every
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``prostor profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``prostor kanban decompose <task_id>``.
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.prostor/config.yaml.
