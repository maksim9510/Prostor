...
``{title, url, description, position}`` rows every other Prostor web
provider produces.
...
prefers Prostor-managed xAI Grok OAuth (via ``prostor auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.prostor/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``prostor tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `prostor auth` to sign in with "
