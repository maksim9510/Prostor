...
Optional Prostor observability plugin that maps Prostor observer hooks to
...
not replace Prostor Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Prostor emit NeMo Relay lifecycle events for
the work Prostor already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Prostor Agent can:

- Preserve Prostor execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
prostor plugins enable observability/nemo_relay
```

The `PROSTOR_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `PROSTOR_HOME` that the
agent run will use:

```bash
env PROSTOR_HOME=/tmp/prostor-nemo-relay-test \
  prostor plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`PROSTOR_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`PROSTOR_HOME` is the Prostor profile/config home used by both
`prostor plugins enable ...` and the later `prostor chat ...` run. If unset,
Prostor uses the user's default home, usually `~/.prostor`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export PROSTOR_HOME=/tmp/prostor-nemo-relay-test
prostor plugins enable observability/nemo_relay
prostor chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `prostor` command you run is built from the
...
uv run prostor plugins enable observability/nemo_relay
uv run prostor chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/prostor_agent-*.whl
...
The plugin can configure exporters directly from `PROSTOR_NEMO_RELAY_*`
...
```bash
export PROSTOR_NEMO_RELAY_ATOF_ENABLED=1
export PROSTOR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export PROSTOR_NEMO_RELAY_ATIF_ENABLED=1
export PROSTOR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `PROSTOR_NEMO_RELAY_ATOF_FILENAME`
- `PROSTOR_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `PROSTOR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `PROSTOR_NEMO_RELAY_ATIF_AGENT_NAME`
- `PROSTOR_NEMO_RELAY_ATIF_AGENT_VERSION`
- `PROSTOR_NEMO_RELAY_ATIF_MODEL_NAME`
- `PROSTOR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Prostor at it:

```bash
export PROSTOR_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Prostor Agent"
agent_version = "local"
```

When `PROSTOR_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`PROSTOR_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`PROSTOR_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Prostor does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Prostor keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Prostor routes LLM and tool
...
For the full generic Prostor middleware contract, see
...
export PROSTOR_HOME=/tmp/prostor-nemo-relay-docs/prostor-home
mkdir -p "$PROSTOR_HOME"

cat > "$PROSTOR_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Prostor session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export PROSTOR_NEMO_RELAY_ATOF_ENABLED=1
export PROSTOR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/prostor-nemo-relay-docs/subagent/atof
export PROSTOR_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export PROSTOR_NEMO_RELAY_ATOF_MODE=overwrite
export PROSTOR_NEMO_RELAY_ATIF_ENABLED=1
export PROSTOR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/prostor-nemo-relay-docs/subagent/atif
export PROSTOR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export PROSTOR_NEMO_RELAY_ATIF_AGENT_NAME='Prostor Agent E2E'
export PROSTOR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export PROSTOR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"prostor.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Prostor Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Prostor dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/prostor-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/prostor-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/prostor-nemo-relay-docs/workdir/beta.txt
cd /tmp/prostor-nemo-relay-docs/workdir

export PROSTOR_NEMO_RELAY_ATOF_ENABLED=1
export PROSTOR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/prostor-nemo-relay-docs/parallel/atof
export PROSTOR_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export PROSTOR_NEMO_RELAY_ATOF_MODE=overwrite
export PROSTOR_NEMO_RELAY_ATIF_ENABLED=1
export PROSTOR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/prostor-nemo-relay-docs/parallel/atif
export PROSTOR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export PROSTOR_NEMO_RELAY_ATIF_AGENT_NAME='Prostor Agent E2E'
export PROSTOR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Prostor sessions map to `agent` scopes.
- Prostor API request hooks map to `llm` scope start/end events.
- Prostor tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Prostor execution middleware to hand
...
Enable it for Prostor:

```bash
export PROSTOR_NEMO_RELAY_PLUGINS_TOML=/tmp/prostor-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Prostor routes execution
through these boundaries:

```text
Prostor provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Prostor provider adapter next_call(...)

Prostor tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Prostor tool dispatcher next_call(...)
```
...
middleware for a local Prostor run. This path requires a NeMo Relay runtime that
...
```bash
export PROSTOR_HOME=/tmp/prostor-middleware-test/prostor-home
mkdir -p "$PROSTOR_HOME" /tmp/prostor-middleware-test/nemo-relay
...
cat > /tmp/prostor-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/prostor-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/prostor-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Prostor Middleware E2E"
agent_version = "local"
...
export PROSTOR_NEMO_RELAY_PLUGINS_TOML=/tmp/prostor-middleware-test/nemo-relay/plugins.toml
