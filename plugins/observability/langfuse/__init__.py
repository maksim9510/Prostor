"""langfuse — Prostor plugin for Langfuse observability.

Traces Prostor conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Prostor plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``prostor plugins enable
observability/langfuse`` or ``prostor tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``prostor tools`` or ~/.prostor/.env):
  PROSTOR_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  PROSTOR_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  PROSTOR_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  PROSTOR_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  PROSTOR_LANGFUSE_RELEASE     - release/version tag
  PROSTOR_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  PROSTOR_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  PROSTOR_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("PROSTOR_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Langfuse | None:
    ...
    public_key = _env("PROSTOR_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("PROSTOR_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("PROSTOR_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("PROSTOR_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset PROSTOR_LANGFUSE_PUBLIC_KEY / PROSTOR_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("PROSTOR_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("PROSTOR_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid PROSTOR_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _trace_key(
    ...
    *,
    ...
    Older Prostor paths only expose ``task_id``/``session_id``. Newer paths
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Prostor turn",
                tags=["prostor", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Prostor branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Prostor also has a turn-scoped
    ...
    # messages is a list only for legacy Prostor branches that fired
    # pre_llm_call with API messages directly. Current Prostor fires
...
def register(ctx) -> None:
    ...
    # Prostor versions.  pre_api_request / post_api_request fire per API
