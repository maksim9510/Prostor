# Prostor Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: prostor-achievements plugin
...
### `GET /api/plugins/prostor-achievements/achievements`
...
### `POST /api/plugins/prostor-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/prostor-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/prostor-achievements/overview`
...
- Persist plugin data under:
  - `~/.prostor/plugins/prostor-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.prostor/plugins/prostor-achievements/scan_checkpoint.json`
...
- `~/.prostor/plugins/prostor-achievements/`
