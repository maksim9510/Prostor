...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``prostor gateway start/stop/restart``).
    """
    ...
    # `prostor gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged prostor
    ...
    detection always fails. Since every Prostor runtime call inside the
    container drops to prostor via ``s6-setuidgid``, that silent failure
...
#
# Per-profile gateways are registered dynamically when `prostor profile create`
# runs inside the container (Phase 4). Static services (main-prostor, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's PROSTOR_HOME blindly;
    derive the shared profile root from the current PROSTOR_HOME and map the
...
# UID/GID of the in-image ``prostor`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the prostor user.
    ...
    (the ``prostor`` user — UID 10000 — running every Prostor runtime
    ...
    is dispatched as the prostor user.
    ...
    with prostor ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           prostor:prostor, 0755 (parent must already exist)
    ``svc_dir/event/``                     prostor:prostor, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 prostor:prostor, 0755
    ``svc_dir/supervise/event/``           prostor:prostor, 03730
    ``svc_dir/supervise/control``          prostor:prostor, 0660    (FIFO)
    ...
    slot's supervise/ was prostor-owned.
    ...
    # when the parent slot's supervise/ is prostor-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``prostor -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`prostor profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-prostor, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources PROSTOR_HOME (and any extra env) via with-contenv —
             so e.g. ``-e PROSTOR_HOME=/data/prostor`` is honored at run
          ...
          4. Drops to the prostor user and exec's
             ``prostor -p <profile> gateway run`` (or just ``prostor
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``prostor gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root PROSTOR_HOME profile" (the implicit profile that exists at
        ...
        each supervised profile gateway loads its own ``PROSTOR_HOME``,
        ...
        # guard.
        lines.append("export PROSTOR_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's PROSTOR_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``prostor gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-PROSTOR_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The PROSTOR_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own PROSTOR_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "prostor gateway run --replace"
        else:
            gateway_cmd = f"prostor -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid prostor {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${PROSTOR_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the PROSTOR_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e PROSTOR_HOME=/data/prostor``
        gets its logs under /data/prostor/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``prostor logs`` reads and what
        ...
        return (
            ...
            f': "${{PROSTOR_HOME:=/opt/data}}"\n'
            f'log_dir="$PROSTOR_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as prostor and crash-loops on
            ...
            f'chown prostor:prostor "$PROSTOR_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R prostor:prostor "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid prostor s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``prostor gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``prostor -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with prostor ownership
            ...
            # the prostor user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is prostor-owned (see ``_seed_supervise_skeleton``), and on
