...
logger = logging.getLogger("prostor.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Prostor as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("PROSTOR_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(hermes_home: Optional[Path]) -> Optional[str]:
    ...
    home = hermes_home or Path(
        os.environ.get("PROSTOR_HOME", os.path.expanduser("~/.prostor"))
    )
    try:
