...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /prostor``, the
    ``login_url`` is prefixed (``/prostor/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["prostor.lastLocation"]`` fallback
