"""
Dump command for prostor CLI.

Outputs a compact, plain-text summary of the user's Prostor setup
...
def _redact(value: str) -> str:
    ...
    ``prostor dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `prostor auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `prostor auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["prostor-cli"])
