"""Shell completion script generation for prostor CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Prostor Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef prostor
# Prostor Agent zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    ...
    if [[ -d "$HOME/.prostor/profiles" ]]; then
        profiles+=($HOME/.prostor/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'prostor command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
