"""``prostor dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Prostor Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `prostor dashboard` cmdlines and SIGTERM them directly — the same
    # path `prostor update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running prostor dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running prostor dashboard processes and exit",
    )
    # Backward-compat shim: older Prostor desktop app shells (<= 0.15.x) spawn the
    # backend as `prostor dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Prostor couldn't start" with no actionable cause. Accept and silently
    ...
    # `prostor dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.prostor/.env.
    # Nested subparser so bare `prostor dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "PROSTOR_DASHBOARD_OAUTH_CLIENT_ID into ~/.prostor/.env, and prints "
            "how to engage the login gate. Requires being logged in (prostor setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://prostor.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via PROSTOR_DASHBOARD_PORTAL_URL. Mainly for "
