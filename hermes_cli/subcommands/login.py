...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``prostor --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``prostor login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``prostor model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `prostor auth` to manage credentials, "
            "`prostor model` to select a provider, or `prostor setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `prostor model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: prostor-cli)"
    )
