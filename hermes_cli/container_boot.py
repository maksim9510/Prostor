...
``$PROSTOR_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-prostor-setup
(the stage2 hook) has chowned the volume and seeded $PROSTOR_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    ``$PROSTOR_HOME/gateway_state.json`` (sibling to the profile root,
    ...
    # ``prostor gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `prostor profile create` (config.yaml
            # is not — that comes later via `prostor setup`). Use it as the
            ...
            # via ``prostor -p default-named gateway start`` if they
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("PROSTOR_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``prostor dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted PROSTOR_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `prostor -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit prostor
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the prostor user) won't hit EACCES. See
...
def main() -> int:
    ...
    # bind-mounted PROSTOR_HOME, both race to flock() the same s6-log lock
