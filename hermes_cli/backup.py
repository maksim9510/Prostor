...
`prostor import` restores from a backup zip, overlaying onto the current
PROSTOR_HOME root.
"""
...
# ``prostor-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/prostor-agent/``
...
# PROSTOR_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "prostor-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under PROSTOR_HOME) —
...
# File names that ``prostor import`` must never overwrite, matched by basename so
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``prostor-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/prostor-agent/`` — must be preserved.
        if part == "prostor-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside PROSTOR_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"prostor-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"prostor-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "prostor-agent" and not is_root)
        ]
    ...
    print(f"\nRestore with: prostor import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Prostor backup.
    ...
    # Look for telltale files that a prostor home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Prostor backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.prostor/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a prostor dir name
        if prefix in {".prostor", "prostor"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Prostor configuration.")
        ...
        if restored_profiles:
            ...
            for pname in gw_profiles:
                print(f"  prostor -p {pname} gateway install")

        print("Done. Your Prostor configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and prostor backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to PROSTOR_HOME).
...
# — `prostor update` snapshots this set before pulling so approved-user lists
...
# Relative path of the cron job database inside PROSTOR_HOME. Kept in sync with
...
def run_quick_backup(args) -> None:
    """CLI entry point for prostor backup --quick."""
...
def create_pre_update_backup(
    ...
    but writes to ``<PROSTOR_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `prostor claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    restorable with ``prostor import <archive>``.  Writes to
    ``<PROSTOR_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
