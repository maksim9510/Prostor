"""
Prostor Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``prostor desktop`` / ``prostor gui``)
     Built inside the agent checkout under ``$PROSTOR_HOME/prostor-agent/``:
       ...
       - ``$PROSTOR_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Prostor.app`` or ``~/Applications/Prostor.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Prostor`` (NSIS per-user)
...
the app name ("Prostor"), separate from ``$PROSTOR_HOME``:
  - macOS:   ``~/Library/Application Support/Prostor``
  - Windows: ``%APPDATA%\\Prostor``
  - Linux:   ``$XDG_CONFIG_HOME/Prostor`` (default ``~/.config/Prostor``)
...
``prostor uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Prostor"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Prostor"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Prostor" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Prostor.app"),
            home / "Applications" / "Prostor.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Prostor).
            local_base / "Programs" / "Prostor",
            # Older / alternate layout some builds used.
            local_base / "prostor-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Prostor")
    else:
        ...
        paths += [
            data_base / "applications" / "prostor.desktop",
            data_base / "applications" / "Prostor.desktop",
        ]
    return paths
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove prostor' or "
            "'sudo dnf remove prostor'). AppImage builds are a single file you "
