...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or prostor -c.",
    ...
    "Ctrl+Z suspends Prostor to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "prostor -c resumes your most recent CLI session. prostor -c \"project name\" resumes by title.",
    "prostor -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "prostor -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "prostor chat -t web,terminal enables only specific toolsets for a focused session.",
    "prostor chat -s github-pr-workflow preloads a skill at launch.",
    "prostor chat -q \"query\" runs a single non-interactive query and exits.",
    "prostor chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "prostor chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "prostor --yolo bypasses all dangerous command approval prompts for the entire session.",
    "prostor chat --source telegram tags the session for filtering in prostor sessions list.",
    "prostor -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "prostor doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "prostor dump outputs a compact setup summary — great for bug reports.",
    "prostor config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "prostor config edit opens config.yaml in your default editor.",
    "prostor config check scans for missing or stale configuration options.",
    "prostor sessions browse opens an interactive session picker with search.",
    "prostor sessions stats shows session counts by platform and database size.",
    "prostor sessions prune --older-than 30 cleans up old sessions.",
    "prostor skills search react --source skills-sh searches the skills.sh public directory.",
    "prostor skills check scans installed hub skills for upstream updates.",
    "prostor skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "prostor skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "prostor mcp add github --command npx adds MCP servers from the command line.",
    "prostor mcp serve runs Prostor itself as an MCP server for other agents.",
    "prostor auth add lets you add multiple API keys for credential pool rotation.",
    "prostor completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "prostor logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "prostor backup creates a zip backup of your entire Prostor home directory.",
    "prostor profile create coder creates an isolated profile that becomes its own command.",
    "prostor profile create work --clone copies your current config and keys to a new profile.",
    "prostor update syncs new bundled skills to ALL profiles automatically.",
    "prostor gateway install sets up Prostor as a system service (systemd/launchd).",
    "prostor memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "prostor webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: prostor tools disables unused tools, prostor skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "prostor models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as prostor -w).",
    ...
    "PROSTOR_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Prostor tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'prostor profile create coder' creates the 'coder' command.",
    "prostor profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Prostor prints a resume command with session ID and stats.",
    "prostor sessions export backup.jsonl exports all sessions for backup or analysis.",
    "prostor -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "prostor skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .prostor/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: prostor cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Prostor runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "prostor gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Prostor via a local macOS server.",
    ...
    "Context files (.prostor.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Prostor loads project context from .prostor.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "prostor mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "prostor mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "prostor mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — prostor tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.prostor/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "prostor chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "prostor plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "prostor acp runs Prostor as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "PROSTOR_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "prostor auth add nous or prostor auth add openai-codex sets up OAuth-based providers.",
    ...
    "prostor status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.prostor/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Prostor your own.",
    ...
    "Before a gateway session reset, Prostor auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.prostor/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Prostor state via PROSTOR_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "PROSTOR_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in PROSTOR_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.prostor/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'prostor profile create ls' would be rejected.",
    "prostor profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "prostor claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Prostor auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Prostor config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Prostor is working.',
    ...
    '/reload reloads ~/.prostor/.env into the running session — pick up new API keys without restarting.',
    ...
    "PROSTOR_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.prostor/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.prostor/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'prostor curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "prostor curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'prostor curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'prostor auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'PROSTOR_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "PROSTOR_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'prostor dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'prostor dashboard embeds the full Prostor TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.prostor/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.prostor/dashboard-plugins/ — no npm build required.',
    ...
    'PROSTOR_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'PROSTOR_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'PROSTOR_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'PROSTOR_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.prostor/cache/piper-voices/.',
    ...
    'prostor -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'prostor chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'prostor chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'prostor chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "prostor chat --source tool tags programmatic chats so they don't clutter prostor sessions list.",
    'prostor dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'prostor sessions rename <ID> "new title" renames any past session; prostor sessions delete <ID> removes one.',
    'prostor import restores a session export or profile archive produced by sessions export or profile export.',
    'prostor fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'prostor pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'prostor setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'prostor status --deep runs the full health sweep across every component; plain prostor status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'PROSTOR_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'PROSTOR_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.prostor/plugins/ — trust-gated by design.',
    "PROSTOR_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'PROSTOR_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'PROSTOR_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'PROSTOR_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'PROSTOR_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'PROSTOR_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'PROSTOR_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'PROSTOR_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'PROSTOR_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'PROSTOR_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'PROSTOR_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Prostor block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from prostor sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.prostor/dashboard-plugins/.',
]
