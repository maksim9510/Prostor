...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``prostor gateway run --replace`` /
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `prostor kanban list` mid-integrity-probe) block
...
def create_task(
    ...
    *,
    ...
    each name to ``prostor --skills ...``. Use this to pin a task to a
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Prostor checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Prostor treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Prostor creates/reuses that linked worktree. With
      no ``workspace_path``, Prostor anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Prostor uses
      ``wt/<task-id>``.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    hermes_bin = _safe_which_no_cwd("prostor") if _IS_WINDOWS else shutil.which("prostor")
    if hermes_bin:
...
def _default_spawn(
    ...
    *,
    ...
    # If PROSTOR_HOME is absent from the child's env, get_hermes_home() falls
