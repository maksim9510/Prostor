...
import { PROSTOR_BASE_PATH, getWsTicket } from "@/lib/api";
  ...
  async connect(token?: string): Promise<void> {
    ...
    if (token) {
      ...
      if (!authParamValue) {
        this.setState("error");
        throw new Error(
          "Session token not available — page must be served by the Prostor dashboard",
        );
      }
    }
    ...
    const ws = new WebSocket(
      `${scheme}//${location.host}${PROSTOR_BASE_PATH}/api/ws?${authParamName}=${encodeURIComponent(authParamValue)}`,
    );
    this.ws = ws;
