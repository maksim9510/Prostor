...
export const PROSTOR_BASE_PATH = readBasePath();
const BASE = PROSTOR_BASE_PATH;
...
const SESSION_HEADER = "X-Prostor-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    if (!window.__HERMES_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        alreadyReloaded =
          sessionStorage.getItem("prostor.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("prostor.tokenReloadAttempted", "1");
        } catch {
...
async function getSessionToken(): Promise<string> {
  ...
  throw new Error("Session token not available — page must be served by the Prostor dashboard server");
}
...
export interface SkillHubSource {
  ...
  /** prostor-index only: whether the centralized index loaded. */
  available?: boolean;
}
