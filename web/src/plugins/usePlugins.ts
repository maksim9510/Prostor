...
import { api, PROSTOR_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${PROSTOR_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      script.setAttribute("data-prostor-plugin", manifest.name);
