...
const BACKEND = process.env.PROSTOR_DASHBOARD_URL ?? "http://127.0.0.1:9119";
  ...
  return {
    ...
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[prostor] Could not find session token in ${BACKEND} — ` +
              `is \`prostor dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        console.warn(
          `[prostor] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`prostor dashboard\` or set PROSTOR_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Same host as `prostor dashboard` must serve these; Vite has no
