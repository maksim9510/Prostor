# Prostor Agent Security Policy

This document describes Prostor Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/prostor-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Prostor Agent does not operate a bug
bounty program.**
...
- Environment details (`prostor version`, commit SHA, OS, Python
  version).
...
Prostor Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Prostor Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Prostor Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Prostor Agent's documentation
...
Prostor Agent supports two OS-level isolation postures. They address
...
Prostor Agent supports this in two ways:

- **Prostor Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Prostor Agent's in-process heuristics
...
Prostor Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Prostor Agent
itself. Bugs in Prostor Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Prostor Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Prostor Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Prostor Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  prostor-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Prostor Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Prostor Agent. Bugs in Prostor
...
- Prostor Agent includes supply-chain guards for MCP server
