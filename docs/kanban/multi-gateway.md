...
Prostor supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.prostor/config.yaml`:
...
Or set the env var: `PROSTOR_KANBAN_DISPATCH_IN_GATEWAY=false`
