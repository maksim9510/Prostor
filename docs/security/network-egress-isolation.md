...
When running Prostor inside Docker, the default `network_mode: host` gives the
...
The Prostor [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ prostor-agent │   │ prostor-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   PROSTOR_UID=$(id -u) PROSTOR_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=prostor,prostor-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://prostor-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Prostor trust model and vulnerability reporting
