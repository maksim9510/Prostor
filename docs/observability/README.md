# Prostor Observer Hooks

Prostor observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Prostor' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Prostor catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Prostor only builds
