...
ACP 适配器将 Prostor 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
Zed ACP Registry 路径通过 `uvx --from 'prostor-agent[acp]==<version>' prostor-acp` 启动同一适配器，指向 `prostor-agent` PyPI 发布包。
...
`acp_adapter/tools.py` 将 Prostor 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
而是复用 Prostor 的运行时解析器：

- `acp_adapter/auth.py`
- `prostor_cli/runtime_provider.py`

因此 ACP 通告并使用当前配置的 Prostor provider/凭据。它还始终通告一个终端 setup 认证方法（`prostor-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 Prostor 的交互式模型/provider 配置。
...
- ACP 会话持久化至共享的 `~/.prostor/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `prostor-acp` toolset 定义
- `prostor_cli/main.py` — `prostor acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `prostor-acp` 脚本