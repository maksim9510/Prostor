...
不设置这些值将保持旧版默认值（`PROSTOR_API_TIMEOUT=1800`s、`PROSTOR_API_CALL_STALE_TIMEOUT=300`s、原生 Anthropic 900s）。目前不适用于 AWS Bedrock（`bedrock_converse` 和 AnthropicBedrock SDK 路径均使用 boto3 及其自身的超时配置）。请参阅 [`cli-config.yaml.example`](https://github.com/maksim9510/Prostor/blob/main/cli-config.yaml.example) 中的注释示例。
