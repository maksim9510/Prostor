...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/prostor-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    prostor_meta = (fm.get("metadata") or {}).get("prostor") or {}
    tags = prostor_meta.get("tags") or []
    related = prostor_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `prostor skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Prostor Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Prostor ships with a large built-in skill library copied into `~/.prostor/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Prostor also syncs bundled skills on `prostor update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.prostor/skills/` tree, it is still shipped with Prostor; restore it with `prostor skills reset <name> --restore`.",
        "",
