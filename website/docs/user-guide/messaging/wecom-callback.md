...
Connect Prostor to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Prostor supports two WeCom integration modes:
...
> Run `prostor gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Prostor decrypts the message, queues it for the agent
...
(Use `prostor gateway start` only after `prostor gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Prostor and the token the
...
in `~/.prostor/.env` values around `=` will also break signature checks. After
fixing, restart `prostor gateway run`.
...
Check `prostor gateway run` logs for the bound host/port. If the adapter bound to
