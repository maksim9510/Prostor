...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Prostor Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Prostor to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.prostor/.env`:
...
All standard Prostor commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.prostor/logs/gateway.log`
...
4. Consider enabling verbose logging: `PROSTOR_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Prostor automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
PROSTOR_LOG_LEVEL=debug prostor gateway
```
...
```bash
prostor chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
