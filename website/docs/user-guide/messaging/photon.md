...
Connect Prostor to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Prostor — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Prostor runs it in a
...
prostor photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Prostor Agent` project on your account.
...
Runtime credentials are written to `~/.prostor/.env`
...
(device token, dashboard project id) lives in `~/.prostor/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Prostor
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Prostor replies with a pairing code. Approve it with:

```bash
prostor pairing approve photon <CODE>
```

Use `prostor pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.prostor/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.prostor/.env`):
...
By default Prostor responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Prostor` and
`@Prostor agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Prostor will reply.

## Status & troubleshooting

```bash
prostor photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Prostor uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run prostor photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `prostor photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `prostor photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `prostor photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Prostor sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Prostor wake words  | JSON list / comma / newline regex patterns for group mentions |
