...
Connect Prostor to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Prostor and the BlueBubbles server
...
Or set environment variables directly in `~/.prostor/.env`:
...
By default, Prostor responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Prostor uses conservative defaults for `Prostor` and `@Prostor agent` variants.
...
When someone messages your iMessage, Prostor automatically sends them a pairing code. Approve it with:
```bash
prostor pairing approve bluebubbles <CODE>
```
Use `prostor pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.prostor/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.prostor/.env`):
```bash
...
Prostor will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Prostor
Prostor → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Prostor sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Prostor wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.prostor/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Prostor resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `prostor logs gateway` for webhook errors (or `prostor logs -f` to follow in real-time)
