...
description: "Chat with Prostor from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Prostor from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Prostor](/guides/use-voice-mode-with-prostor).
...
```mermaid
flowchart TB
    subgraph Gateway["Prostor Gateway"]
...
For group chats, hooks, and automation flows, Prostor supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Prostor keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Prostor does not hide failures just because the text resembles a silence token.
...
```bash
prostor gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
prostor gateway              # Run in foreground
prostor gateway setup        # Configure messaging platforms interactively
prostor gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo prostor gateway install --system   # Linux only: install a boot-time system service
prostor gateway start        # Start the default service
prostor gateway stop         # Stop the default service
prostor gateway status       # Check default service status
prostor gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Prostor Agent to the latest version |
...
Configure per-platform overrides in `~/.prostor/gateway.json`:
...
prostor pairing approve telegram XKGH5N7P

# Other pairing commands:
prostor pairing list          # View pending + approved users
prostor pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Prostor appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.prostor/config.yaml`:
...
Off by default. When enabled, Prostor prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.prostor/config.yaml`:
...
```bash
PROSTOR_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
prostor gateway install               # Install as user service
prostor gateway start                 # Start the service
prostor gateway stop                  # Stop the service
prostor gateway status                # Check status
journalctl --user -u prostor-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo prostor gateway install --system
sudo prostor gateway start --system
sudo prostor gateway status --system
journalctl -u prostor-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `prostor update`. When `prostor update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart prostor-gateway` command (it never blocks on an interactive password prompt).
...
```bash
prostor gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `prostor update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo prostor update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/prostor-gateway`:

```
prostor ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed prostor-gateway*, /usr/bin/systemctl --no-ask-password start prostor-gateway*, /usr/bin/systemctl --no-ask-password restart prostor-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Prostor will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Prostor installations on the same machine (with different `PROSTOR_HOME` directories), each gets its own systemd service name. The default `~/.prostor` uses `prostor-gateway`; other installations use `prostor-gateway-<hash>`. The `prostor gateway` commands automatically target the correct service for your current `PROSTOR_HOME`.
:::

### macOS (launchd)

```bash
prostor gateway install               # Install as launchd agent
prostor gateway start                 # Start the service
prostor gateway stop                  # Stop the service
prostor gateway status                # Check status
tail -f ~/.prostor/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.prostor.gateway.plist`. It includes three environment variables:
...
- **PROSTOR_HOME** — scopes the gateway to your Prostor installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `prostor gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `PROSTOR_HOME` directory gets its own launchd label. The default `~/.prostor` uses `ai.prostor.gateway`; other installations use `ai.prostor.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `prostor-cli` | Full access |
| Telegram | `prostor-telegram` | Full tools including terminal |
| Discord | `prostor-discord` | Full tools including terminal |
| WhatsApp | `prostor-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `prostor-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `prostor-slack` | Full tools including terminal |
| Google Chat | `prostor-google_chat` | Full tools including terminal |
| Signal | `prostor-signal` | Full tools including terminal |
| SMS | `prostor-sms` | Full tools including terminal |
| Email | `prostor-email` | Full tools including terminal |
| Home Assistant | `prostor-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `prostor-mattermost` | Full tools including terminal |
| Matrix | `prostor-matrix` | Full tools including terminal |
| DingTalk | `prostor-dingtalk` | Full tools including terminal |
| Feishu/Lark | `prostor-feishu` | Full tools including terminal |
| WeCom | `prostor-wecom` | Full tools including terminal |
| WeCom Callback | `prostor-wecom-callback` | Full tools including terminal |
| Weixin | `prostor-weixin` | Full tools including terminal |
| BlueBubbles | `prostor-bluebubbles` | Full tools including terminal |
| QQBot | `prostor-qqbot` | Full tools including terminal |
| Yuanbao | `prostor-yuanbao` | Full tools including terminal |
| Microsoft Teams | `prostor-teams` | Full tools including terminal |
| API Server | `prostor-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `prostor-webhook` | Full tools including terminal |
| Raft | `prostor-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.prostor/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
