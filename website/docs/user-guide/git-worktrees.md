...
title: "Git Worktrees"
description: "Run multiple Prostor agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Prostor Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Prostor so each session has a clean, isolated working directory.

## Why Use Worktrees with Prostor?

Prostor treats the **current working directory** as the project root:

- CLI: the directory where you run `prostor` or `prostor chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.prostor/config.yaml`
...
git worktree add ../repo-feature feature/prostor-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/prostor-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Prostor there:

```bash
cd ../repo-feature

# Start Prostor in the worktree
prostor
```
...
git worktree add ../repo-experiment-a feature/prostor-a
git worktree add ../repo-experiment-b feature/prostor-b
```
...
Each Prostor process:

- Works on its own branch (`feature/prostor-a` vs `feature/prostor-b`).
...
- Prostor checkpoint data under `~/.prostor/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Prostor experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/prostor-checkpoints-docs`, `feature/prostor-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Prostor from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `prostor -w` (Automatic Worktree Mode)

Prostor has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `prostor/prostor-<hash>`).
...
```bash
prostor -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `prostor -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Prostor session its own clean checkout.
