...
| | |
|---|---|
| Source | Optional — install with `prostor skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Prostor Agent |
| License | MIT |
...
The script reads `~/.prostor/.env` for two optional defaults:
...
Helper script: `~/.prostor/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.prostor/.env`.
