...
| | |
|---|---|
| Source | Optional — install with `prostor skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Prostor Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Prostor terminal calls (desktop app flow only)
...
### Service Account (recommended for Prostor)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.prostor/.env` (the skill will prompt for this on first load).
...
## Prostor Execution Pattern (desktop app flow)

Prostor terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/prostor-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/prostor-op.sock"
