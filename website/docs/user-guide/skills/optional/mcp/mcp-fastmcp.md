...
| | |
|---|---|
| Source | Optional — install with `prostor skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Prostor or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Prostor. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.prostor/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.prostor/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Prostor integration, either:

- configure the server in `~/.prostor/config.yaml` using the `native-mcp` skill, or
...
### Prostor cannot see the deployed server

The server-building part may be correct while the Prostor config is not. Load the `native-mcp` skill and configure the server in `~/.prostor/config.yaml`, then restart Prostor.
