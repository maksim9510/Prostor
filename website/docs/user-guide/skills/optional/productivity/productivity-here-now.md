...
| | |
|---|---|
| Source | Optional — install with `prostor skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${PROSTOR_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${PROSTOR_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `prostor`)    |
