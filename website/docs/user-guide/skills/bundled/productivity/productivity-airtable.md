...
4. Store the token in `~/.prostor/.env` (or via `prostor setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.prostor/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
