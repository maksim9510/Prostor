...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`prostor-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-prostor-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Prostor terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Prostor itself, `model.provider: openai-codex` uses Prostor-managed Codex
OAuth from `~/.prostor/auth.json` after `prostor auth add openai-codex`. For the
...
## Prostor Gateway Caveat

When invoking the Codex CLI from a Prostor gateway/service context (for example,
