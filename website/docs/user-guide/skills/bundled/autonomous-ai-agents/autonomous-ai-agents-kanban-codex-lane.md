...
description: "Use when a Prostor Kanban worker wants to run Codex CLI as an isolated implementation lane while Prostor keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Prostor Kanban worker wants to run Codex CLI as an isolated implementation lane while Prostor keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`prostor-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-prostor-agent) |
...
This skill defines the lightweight Prostor+Codex dual-lane convention for Kanban workers. Prostor is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a Prostor worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn Prostor workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by Prostor in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Prostor can run the relevant tests itself after Codex exits.
...
1. Prostor owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Prostor board CLI as a substitute for the worker.
2. Prostor owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Prostor owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Prostor with the repo's canonical wrapper.
4. Prostor owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Prostor owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${PROSTOR_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Prostor edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Prostor can do the task directly.
...
Prostor owns the Kanban lifecycle; do not call Prostor kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Prostor reconciliation.
...
- Explicit statement: Prostor owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Prostor will run afterward.
...
Prostor must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Prostor.
...
- [ ] Prostor ran the canonical tests itself, using `scripts/run_tests.sh` for Prostor Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Prostor-run tests.
- [ ] Accepted commits/diffs were applied to the Prostor-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "prostor"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Prostor edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Prostor.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Prostor writes board state.
...
- [ ] Prostor reviewed `git diff` and safety-sensitive files.
- [ ] Prostor ran canonical tests independently.
