...
Prostor Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.prostor/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Prostor stores session history so it can resume conversations, but it does not
...
Prostor explicitly injects for that turn.
...
For example, if a user sends an image and asks Prostor to make a meme from it,
Prostor may inspect that image once with vision and run an image-processing
...
`prostor sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`prostor` or `prostor chat`) |
...
prostor -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
prostor -c "my project"   # → resumes "my project #3"
```
...
prostor --resume 20250305_091523_a1b2c3d4
prostor -r 20250305_091523_a1b2c3d4

# Resume by title
prostor --resume "refactoring auth"

# Or with the chat subcommand
prostor chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `prostor sessions list`.

### Conversation Recap on Resume

When you resume a session, Prostor displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Prostor session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.prostor/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `prostor -r "<title>"` from the shell) and pick up where the platform left off.
...
Prostor automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `prostor sessions list` or `prostor sessions browse`.
...
```bash
prostor sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Prostor creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`prostor -c "my project"`), it automatically picks the most recent session in the lineage.
...
Prostor provides a full set of session management commands via `prostor sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
prostor sessions list

# Filter by platform
prostor sessions list --source telegram

# Show more sessions
prostor sessions list --limit 50
```
...
prostor sessions export backup.jsonl

# Export sessions from a specific platform
prostor sessions export telegram-history.jsonl --source telegram

# Export a single session
prostor sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
prostor sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
prostor sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
prostor sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
prostor sessions prune

# Custom age threshold
prostor sessions prune --older-than 30

# Only prune sessions from a specific platform
prostor sessions prune --source telegram --older-than 60

# Skip confirmation
prostor sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`prostor insights`](/reference/cli-commands#prostor-insights).
...
When Prostor cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Prostor uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Prostor in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.prostor/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.prostor/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.prostor/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.prostor/sessions/`. They are no longer written or
read by Prostor. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Prostor process in the same `PROSTOR_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.prostor/config.yaml`:
...
prostor sessions delete <session_id>
