...
Type `@` followed by a reference to inject content directly into your message. Prostor expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Prostor env: `$PROSTOR_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$PROSTOR_HOME/skills/.hub/`
