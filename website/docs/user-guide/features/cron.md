...
Schedule tasks to run automatically with natural language or cron expressions. Prostor exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Prostor itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
Cron jobs use whatever provider `prostor model` selected. `prostor setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Prostor disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
prostor cron create "every 2h" "Check server status"
prostor cron create "every 1h" "Summarize new feed items" --skill blogwatcher
prostor cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Prostor normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Prostor will use the unified `cronjob` tool internally.
...
prostor cron create "every 1d at 09:00" \
...
```bash
prostor cron edit <job_id> --schedule "every 4h"
prostor cron edit <job_id> --prompt "Use the revised task"
prostor cron edit <job_id> --skill blogwatcher --skill maps
prostor cron edit <job_id> --add-skill maps
prostor cron edit <job_id> --remove-skill blogwatcher
prostor cron edit <job_id> --clear-skills
```
...
```bash
prostor cron list
prostor cron pause <job_id_or_name>
prostor cron resume <job_id_or_name>
prostor cron run <job_id_or_name>
prostor cron remove <job_id_or_name>
prostor cron edit <job_id_or_name> [...flags]
prostor cron status
prostor cron tick
```
...
```bash
prostor gateway install     # Install as a user service
sudo prostor gateway install --system   # Linux: boot-time system service for servers
prostor gateway             # Or run in foreground
...
On each tick Prostor:

1. loads jobs from `~/.prostor/cron/jobs.json`
...
A file lock at `~/.prostor/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.prostor/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.prostor/cron/output/`), but no message is sent to the delivery target.
...
Or set the `PROSTOR_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.prostor/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Prostor directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Prostor will write the check script to `~/.prostor/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.prostor/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.prostor/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.prostor/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.prostor/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Prostor reads Job 1's most recent output from `~/.prostor/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Prostor skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `prostor tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `prostor tools` cron-platform config wins; otherwise Prostor falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Prostor should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.prostor/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.prostor/scripts/.feed-changed.last"
...
:::tip
Prostor's own `~/.prostor/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/prostor-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.prostor/cron/jobs.json`. Output from job runs is saved to `~/.prostor/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Prostor resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
