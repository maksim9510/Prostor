...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Prostor automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Prostor auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
prostor auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
prostor auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
prostor auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  prostor_pkce          oauth   prostor_pkce ←
...
Run `prostor auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `prostor auth` | Interactive pool management wizard |
| `prostor auth list` | Show all pools and credentials |
| `prostor auth list <provider>` | Show a specific provider's pool |
| `prostor auth add <provider>` | Add a credential (prompts for type and key) |
| `prostor auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `prostor auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `prostor auth remove <provider> <index>` | Remove credential by 1-based index |
| `prostor auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `prostor auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `prostor model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via prostor model:
prostor auth list
# Shows:
...
prostor auth add Together.ai --api-key sk-together-second-key
```
...
Prostor automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Prostor PKCE OAuth | `~/.prostor/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `prostor auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `prostor auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Prostor can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Prostor-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`prostor_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`prostor_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.prostor/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Prostor' credential store, so its token remains persistable.
