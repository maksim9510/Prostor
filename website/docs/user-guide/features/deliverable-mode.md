...
When Prostor Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.prostor/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.prostor/config.yaml` (switchable per session
via `/personality`).
...
If you use Prostor' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.prostor/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Prostor Agent's deliverable
