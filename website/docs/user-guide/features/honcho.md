...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Prostor's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Prostor instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
prostor memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `prostor` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Prostor syncs them back at session init.
...
The `prostor honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `prostor memory setup honcho` (or run `prostor memory setup` and pick it from the list); the `prostor honcho` subcommand then appears on the next invocation.

```bash
prostor memory setup honcho    # Configure Honcho directly (works before activation)
prostor honcho status          # Connection status, config, and key settings
prostor honcho setup           # Redirects to `prostor memory setup` (post-activation alias)
prostor honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
prostor honcho peer            # Show or update peer names + dialectic reasoning level
prostor honcho mode            # Show or set recall mode (hybrid/context/tools)
prostor honcho tokens          # Show or set token budget for context and dialectic
prostor honcho identity        # Seed or show the AI peer's Honcho identity
prostor honcho sync            # Sync Honcho config to all existing profiles
prostor honcho peers           # Show peer identities across all profiles
prostor honcho sessions        # List known Honcho session mappings
prostor honcho map             # Map current directory to a Honcho session name
prostor honcho enable          # Enable Honcho for the active profile
prostor honcho disable         # Disable Honcho for the active profile
prostor honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `prostor honcho`

If you previously used the standalone `prostor honcho setup`:
...
No re-login or re-setup needed. Run `prostor memory setup` and select "honcho" — the wizard detects your existing config.
