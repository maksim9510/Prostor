...
If you're writing the worker code itself — the agent that runs *inside* a lane — the [`kanban-worker`](https://github.com/maksim9510/Prostor/blob/main/skills/devops/kanban-worker/SKILL.md) skill is the deeper procedural detail.
...
The [`kanban-worker`](https://github.com/maksim9510/Prostor/blob/main/skills/devops/kanban-worker/SKILL.md) skill has worked examples for both `kanban_complete` (truly terminal tasks — typo fixes, docs changes, research writeups) and the `review-required` block pattern.
...
The shape every kanban worker takes today: the assignee is a profile name, the dispatcher spawns `prostor -p <profile>`, the worker auto-loads the [`kanban-worker`](https://github.com/maksim9510/Prostor/blob/main/skills/devops/kanban-worker/SKILL.md) skill plus the `KANBAN_GUIDANCE` system-prompt block, and uses the `kanban_*` tools to terminate the run. No setup beyond defining the profile.

When you create profiles for your fleet, choose names that match the *role* you want the orchestrator to route to. The orchestrator (when there is one) discovers your profile names via `prostor profile list` — there's no fixed roster the system assumes (see the [`kanban-orchestrator`](https://github.com/maksim9510/Prostor/blob/main/skills/devops/kanban-orchestrator/SKILL.md) skill for the orchestrator side of the contract).
...
The historical issue for this is [#19931](https://github.com/maksim9510/Prostor/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://github.com/maksim9510/Prostor/pull/19924) — those describe the original architecture proposal but didn't land a runner.
...
- [`kanban-worker`](https://github.com/maksim9510/Prostor/blob/main/skills/devops/kanban-worker/SKILL.md) — the skill the worker process loads.
- [`kanban-orchestrator`](https://github.com/maksim9510/Prostor/blob/main/skills/devops/kanban-orchestrator/SKILL.md) — the orchestrator side.
