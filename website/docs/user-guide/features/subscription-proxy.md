...
chat completions — use your Prostor-managed provider subscription as their
...
| Use case | "Use Prostor as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Prostor stores
the refresh token in `~/.prostor/auth.json` — the same place all Prostor
...
Model:      Prostor-4-70B    # or Prostor-4.3-36B, Prostor-4-405B
```
...
interface in `prostor_cli/proxy/adapters/`.
...
If you see `not logged in`, run `prostor portal`. If you see
...
`prostor_cli/proxy/adapters/<provider>.py` and registering it in
