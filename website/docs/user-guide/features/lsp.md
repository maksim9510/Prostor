...
Prostor runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Prostor
...
1. Prostor captures a baseline of current diagnostics for the file.
...
…). Prostor auto-detects the binary on PATH or in
`<PROSTOR_HOME>/lsp/bin/`.
...
`node_modules` tree — Prostor installs both packages together when you
run `prostor lsp install typescript` or auto-install fires on first
use.

## CLI

```
prostor lsp status          # service state + per-server install status
prostor lsp list            # registry, optionally --installed-only
prostor lsp install <id>    # eagerly install one server
prostor lsp install-all     # try every server with a known recipe
prostor lsp restart         # tear down running clients
prostor lsp which <id>      # print resolved binary path
```

`prostor lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <PROSTOR_HOME>/lsp/bin
...
When `install_strategy: auto`, Prostor installs binaries into
`<PROSTOR_HOME>/lsp/bin/`. NPM packages land in
`<PROSTOR_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Prostor-owned and is
...
Servers are kept alive for the life of the Prostor process. There's
...
**`prostor lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<PROSTOR_HOME>/lsp/bin/`. Run
`prostor lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `prostor lsp status`**
...
When `prostor lsp status` shows a `Backend warnings` section, install
...
`~/.prostor/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.prostor/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `prostor lsp restart` to clear the set;
