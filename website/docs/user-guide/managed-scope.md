...
`~/.prostor/config.yaml`, `~/.prostor/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/prostor`:

```text
/etc/prostor/
├── config.yaml     # managed config layer (wins over ~/.prostor/config.yaml)
└── .env            # managed env layer (wins over ~/.prostor/.env + shell)
```
...
The location can be relocated with the `PROSTOR_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `PROSTOR_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Prostor.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export PROSTOR_MANAGED_DIR=/opt/org/prostor-policy
```

:::warning
A user who can set `PROSTOR_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `prostor doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/prostor/config.yaml` (managed) | `/etc/prostor/.env` (managed) |
| 2 | `~/.prostor/config.yaml` (user) | `~/.prostor/.env` (user) |
...
```bash
prostor config        # shows a header naming the managed source + the pinned keys
prostor doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Prostor refuses and names the source:

```bash
$ prostor config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/prostor/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `prostor config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/prostor

# Pin some config values for every user on this machine
sudo tee /etc/prostor/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/prostor/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/prostor
sudo chmod 0644 /etc/prostor/config.yaml /etc/prostor/.env
```

Changes take effect on the next Prostor start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`prostor doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Prostor as `root`), managed scope is advisory.
