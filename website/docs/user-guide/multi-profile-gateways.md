...
If you only run one Prostor agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Prostor agents that should all
...
(`ai.prostor.gateway-<name>.plist`) or systemd user service
(`prostor-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
prostor profile create coder
prostor profile create personal-bot
prostor profile create research
...
```bash
prostor config set gateway.multiplex_profiles true
prostor gateway restart
```

Equivalently, in the default profile's `~/.prostor/config.yaml`:
...
You do **not** run `prostor gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `prostor gateway start` / `run` is a
...
home). `prostor status` reports the multiplexer and the profiles it serves;
`prostor status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/prostor-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: prostor-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    prostor gateway "$action"
  else
    prostor -p "$profile" gateway "$action"
  fi
}
...
```bash
prostor-gateways start      # start every configured profile
prostor-gateways stop       # stop every configured profile
prostor-gateways restart    # restart all
prostor-gateways status     # status across all
prostor-gateways list       # delegates to `prostor gateway list`
```

:::tip
The `default` profile is targeted with `prostor gateway <action>` (no `-p`),
not `prostor -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `prostor -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.prostor.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/prostor-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.prostor.gateway.plist` /
`prostor-gateway.service`.
...
```bash
# Default profile
tail -f ~/.prostor/logs/gateway.log
tail -f ~/.prostor/logs/gateway.error.log

# Named profile
tail -f ~/.prostor/profiles/<name>/logs/gateway.log
tail -f ~/.prostor/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.prostor/logs/gateway.log ~/.prostor/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
prostor logs -f                  # follow default profile
prostor -p coder logs -f         # follow one profile
prostor logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
prostor profile list             # profiles + model + gateway state
prostor-gateways status          # full status across every profile
launchctl list | grep prostor    # macOS — PIDs and labels
systemctl --user list-units 'prostor-gateway-*'   # Linux — units
```
...
```
~/.prostor/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.prostor/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
prostor config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.prostor/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=prostor --why="gateways running" \
  sleep infinity &
...
`prostor-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.prostor/.env ~/.prostor/profiles/*/.env
```

## Updating the code

`prostor update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `prostor gateway start` after a previous `prostor gateway stop`. The
...
```bash
ps -ef | grep "prostor_cli.*-p <profile>"
cat ~/.prostor/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.prostor.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.prostor.gateway-<profile>.plist

# Linux
systemctl --user restart prostor-gateway-<profile>.service
```

### Health check

```bash
prostor doctor                  # default profile
prostor -p <profile> doctor     # one profile
```
