...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Prostor Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Prostor setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Prostor Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Prostor Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Prostor for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Prostor working on my machine | `prostor setup` | Run a real chat and verify it responds |
| I already know my provider | `prostor model` | Save the config, then start chatting |
| I want a bot or always-on setup | `prostor gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `prostor model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `prostor model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Prostor cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `prostor model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `prostor setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `prostor update`. Re-enable anything later with `prostor tools`, seed skills with `prostor skills opt-in --sync`, or tune settings with `prostor setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `prostor model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `prostor model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `prostor model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `prostor_cli/models.py` may change between releases) | `prostor model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `prostor model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `prostor model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `prostor model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `prostor model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `prostor model` (requires `copilot` CLI + `copilot login`) |
...
Prostor Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `prostor model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Prostor separates secrets from normal config:

- **Secrets and tokens** → `~/.prostor/.env`
- **Non-secret settings** → `~/.prostor/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
prostor config set model anthropic/claude-opus-4.6
prostor config set terminal.backend docker
prostor config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
prostor            # classic CLI
prostor --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Prostor ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `prostor` vs `prostor --tui`.
:::
...
```bash
prostor --continue    # Resume the most recent session
prostor -c            # Short form
```
...
```bash
prostor gateway setup    # Interactive platform configuration
```
...
- `prostor tools` — tune tool access per platform
- `prostor skills` — browse and install reusable workflows
...
```bash
prostor config set terminal.backend docker    # Docker isolation
prostor config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Prostor install directory (the curl installer placed it at
# ~/.prostor/prostor-agent on Linux/macOS or %LOCALAPPDATA%\prostor\prostor-agent on Windows):
cd ~/.prostor/prostor-agent
...
Skills are on-demand instruction documents that teach Prostor how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Prostor ships with a catalog of bundled skills already installed in `~/.prostor/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
prostor skills browse                      # list everything available
prostor skills search kubernetes           # find skills by keyword
prostor skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `prostor skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Prostor ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.prostor/prostor-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Prostor opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `prostor model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `prostor gateway setup` and check `prostor gateway status` |
| `prostor --continue` can't find old session | Switched profiles or session never saved | Check `prostor sessions list` and confirm you're in the right profile |
...
| `prostor doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `prostor model`
3. `prostor setup`
4. `prostor sessions list`
5. `prostor --continue`
6. `prostor gateway status`
...
| `prostor tools` | Configure which tools are enabled per platform |
| `prostor setup` | Full setup wizard (configures everything at once) |
| `prostor doctor` | Diagnose issues |
| `prostor update` | Update to latest version |
| `prostor gateway` | Start the messaging gateway |
| `prostor --continue` | Resume last session |
