...
description: "Run Prostor Agent directly on an Android phone with Termux"
---

# Prostor on Android with Termux

This is the tested path for running Prostor Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Prostor from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `prostor` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `prostor` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/prostor" "$PREFIX/bin/prostor"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `prostor` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.prostor/.env`.
...
### `prostor doctor` says ripgrep or Node is missing
