---
sidebar_position: 8
title: "Use Voice Mode with Prostor"
description: "A practical guide to setting up and using Prostor voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Prostor
...
- you want Prostor sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Prostor.
...
## Step 1: make sure normal Prostor works first
...
Prostor supports both local and cloud speech stacks.
...
### If you use `prostor setup`

If you choose NeuTTS in the setup wizard, Prostor checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Prostor transcribes and responds
...
- asking Prostor to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Prostor loop.

## Tuning CLI behavior

### Silence threshold

If Prostor starts/stops too aggressively, tune:
...
Prostor stays a normal chat bot, but can speak replies.
...
- you want Prostor to function like a portable research or ops assistant
...
Prostor joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Prostor detects speech boundaries
- transcripts are posted in the associated text channel
- Prostor responds in text and audio
...
1. get text Prostor working
2. install `prostor-agent[voice]`
