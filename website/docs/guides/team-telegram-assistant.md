...
This tutorial walks you through setting up a Telegram bot powered by Prostor Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Prostor Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.prostor/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Prostor itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Prostor Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_prostor_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Prostor Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.prostor/.env`:
...
```
[Gateway] Starting Prostor Gateway...
...
# View live logs
journalctl --user -u prostor-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `prostor gateway install` to update the plist.
:::
...
```bash
prostor gateway stop && prostor gateway start
```
...
prostor pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.prostor/.env`:
...
Control how much detail the bot shows when using tools. In `~/.prostor/config.yaml`:
...
Customize how the bot communicates by editing `~/.prostor/SOUL.md`:

For a full guide, see [Use SOUL.md with Prostor](/guides/use-soul-with-prostor).
...
```bash
# From the CLI
prostor cron list          # View all scheduled jobs
prostor cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u prostor-gateway` (Linux) or `~/.prostor/logs/gateway.log` (macOS) |
| Cron job output | `~/.prostor/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.prostor/cron/jobs.json` |
| Pairing data | `~/.prostor/pairing/` |
| Session history | `~/.prostor/sessions/` |
