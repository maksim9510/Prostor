...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `prostor send`."
---

# Pipe Script Output to Messaging Platforms

`prostor send` is a small, scriptable CLI that pushes a message to any
messaging platform Prostor is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | prostor send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `prostor
...
prostor send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
prostor send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
prostor send --list

# Filter by platform
prostor send --list telegram
```
...
Any platform Prostor ships adapters for works as a target:
...
`prostor send` resolves the message body in this order:

1. **Positional argument** — `prostor send --to telegram "hi"`
2. **`--file PATH`** — `prostor send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | prostor send --to telegram`

When stdin is a TTY (no pipe), Prostor does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  prostor send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `prostor send` reuses your Prostor config, the same script works on
any host where Prostor is installed — no need to export bot tokens into
...
`prostor send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  prostor send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | prostor send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.prostor/bin/prostor send \
...
```bash
./train.py --epochs 200 && \
  prostor send --to telegram "training done" || \
  prostor send --to telegram "training failed (exit $?)"
```
...
prostor send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(prostor send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `prostor send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `prostor send` calls
the platform's REST endpoint directly using credentials from
`~/.prostor/.env` and `~/.prostor/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `prostor gateway start` and retry.
...
# Machine-readable
prostor send --list --json
```

The listing is built from `~/.prostor/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`prostor gateway
...
| Approach | Multi-platform | Reuses Prostor creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `prostor send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`prostor send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `prostor send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `prostor send` shares with cron delivery.
