---
sidebar_position: 6
title: "Use MCP with Prostor"
description: "A practical guide to connecting MCP servers to Prostor Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Prostor

This guide shows how to actually use MCP with Prostor Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Prostor tool
- you want Prostor to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Prostor to internal APIs, databases, or company systems without modifying Prostor core

Do not use MCP when:
- a built-in Prostor tool already solves the job well
...
- Prostor remains the agent
- MCP servers contribute tools
- Prostor discovers those tools at startup or reload time
...
If you installed Prostor with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Prostor banner/status should show MCP integration when configured
- ask Prostor what tools it has available
...
## WSL2: bridge Prostor in WSL to Windows Chrome

This is the practical setup when:

- Prostor runs inside WSL2
...
In this setup, Prostor does **not** connect to Chrome directly. Instead:

- Prostor runs in WSL
- Prostor starts a local stdio MCP server
...
```text
Prostor (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Prostor stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Prostor core browser transport
...
```bash
prostor mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
prostor mcp test chrome-devtools-win
```

Then start a fresh Prostor session or run:
...
Once loaded, Prostor can use the MCP-prefixed browser tools directly. For example:
...
If Prostor runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Prostor from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Prostor from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Prostor:
...
2. Prostor-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Prostor will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Prostor to reason over a bounded workspace.
...
Now Prostor can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Prostor core.
...
Because Prostor now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
