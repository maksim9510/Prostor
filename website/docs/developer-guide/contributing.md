...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Prostor Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Prostor Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Prostor core? Start with [Build a Prostor Plugin](../guides/build-a-prostor-plugin.md)
- Building a new built-in core tool for Prostor itself? Start with [Adding Tools](./adding-tools.md)
...
- **Profile-safe paths**: Never hardcode `~/.prostor` — use `get_prostor_home()` from `prostor_constants` for code paths and `display_prostor_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/prostor-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Prostor officially supports **Linux, macOS, WSL2, and native Windows (via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. If you're doing Windows-heavy dev, run the Windows-footgun lint (`scripts/check-windows-footguns.py`) before pushing.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/prostor-agent/blob/main/LICENSE).
