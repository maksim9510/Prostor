...
sidebar_position: 1
title: "Architecture"
description: "Prostor Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Prostor Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ prostor_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ProstorCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── prostor_state.py           # SQLite session/state database with FTS5
├── prostor_constants.py       # PROSTOR_HOME, profile-aware paths
...
│
├── prostor_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `prostor` subcommands (large file)
...
│   ├── skills_config.py      # prostor skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # prostor tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # prostor gateway start/stop
│
...
```text
User input → ProstorCLI.process_input()
...
Three discovery sources: `~/.prostor/plugins/` (user), `.prostor/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `prostor plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-prostor-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Prostor as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`prostor -p <name>`) gets its own PROSTOR_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
