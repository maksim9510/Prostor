...
description: "How Prostor detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Prostor' browser tooling:
...
(`prostor-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Prostor `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `prostor-acp`, `prostor-api-server`, and core toolsets (gated on CDP reachability)
- `prostor_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
