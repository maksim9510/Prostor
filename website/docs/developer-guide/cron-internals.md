...
description: "How Prostor stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `prostor_cli/cron.py` | CLI `prostor cron` subcommands |
...
Jobs are stored in `~/.prostor/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `prostor cron` commands are run or during active CLI sessions.
...
```python
# ~/.prostor/scripts/check_competitors.py
...
2. **Environment variable** — `PROSTOR_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.prostor/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `prostor cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `prostor cron` CLI provides direct job management:

```bash
prostor cron list                    # Show all jobs
prostor cron create                  # Interactive job creation (alias: add)
prostor cron edit <job_id>           # Edit job configuration
prostor cron pause <job_id>          # Pause a running job
prostor cron resume <job_id>         # Resume a paused job
prostor cron run <job_id>            # Trigger immediate execution
prostor cron remove <job_id>         # Delete a job
```
