...
the actual `setup.sh` that creates the project workspace, configures Prostor
...
2. **Create profiles** — `prostor profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.prostor/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `prostor kanban create` assigned to the director
...
```bash
prostor profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.prostor/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.prostor/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Prostor
...
Each profile gets a `SOUL.md` at `~/.prostor/profiles/<name>/SOUL.md` that
...
```bash
prostor kanban create "Direct production of <video title>" \
    --assignee director \
...
the Prostor `.env` (`${PROSTOR_HOME:-$HOME/.prostor}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _prostor_env="${PROSTOR_HOME:-$HOME/.prostor}/.env"
    if grep -q "^${var}=" "$_prostor_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_prostor_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_prostor_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY prostor ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY prostor OPENROUTER_API_KEY || exit 1
# ...
```
