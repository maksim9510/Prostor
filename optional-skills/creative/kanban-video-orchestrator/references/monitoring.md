...
prostor kanban watch --tenant <project-slug>

# Snapshot of the board
prostor kanban list --tenant <project-slug>
prostor kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
prostor kanban stats --tenant <project-slug>

# Visual dashboard (browser)
prostor dashboard

# Inspect a specific task (includes comments + events)
prostor kanban show <task-id>

# Follow a single task's event stream
prostor kanban tail <task-id>
```

Verify available subcommands with `prostor kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `prostor kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `prostor kanban block <id>` then `prostor kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `prostor kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `prostor kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `prostor profile list` (profile exists?), `prostor status` (gateway/dispatcher up?), and verify tenant. |
...
prostor kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
prostor kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(prostor kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
prostor kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
prostor kanban block <task-id>
prostor kanban archive <task-id>

# Diagnose what happened
prostor kanban show <task-id>      # task body, comments, recent events
prostor kanban tail <task-id>      # follow the live event stream
prostor kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `prostor kanban create` for the director
...
while true; do
    clear
    prostor kanban list --tenant <slug>
    echo "---"
    prostor kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `prostor kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `prostor profile list` and start
