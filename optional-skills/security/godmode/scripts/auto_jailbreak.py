...
try:
    ...
    _SKILL_DIR = Path(os.getenv("PROSTOR_HOME", Path.home() / ".prostor")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Prostor config paths
# ═══════════════════════════════════════════════════════════════════

PROSTOR_HOME = Path(os.getenv("PROSTOR_HOME", Path.home() / ".prostor"))
CONFIG_PATH = PROSTOR_HOME / "config.yaml"
PREFILL_PATH = PROSTOR_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Prostor models — already uncensored, just needs clean prompt
    "prostor": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "prostor" in model_lower or "nous" in model_lower:
        return "prostor"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Prostor config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.prostor/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Prostor for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Prostor for changes to take effect.")
