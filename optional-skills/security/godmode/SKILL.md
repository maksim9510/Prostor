...
12. **execute_code sandbox has no env vars** — When Prostor runs auto_jailbreak via execute_code, the sandbox doesn't inherit the Prostor `.env`. Load dotenv explicitly: `import os; from dotenv import load_dotenv; load_dotenv(os.path.join(os.environ.get("PROSTOR_HOME", os.path.expanduser("~/.prostor")), ".env"))`
