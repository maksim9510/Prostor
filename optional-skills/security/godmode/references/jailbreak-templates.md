...
## 5. GODMODE FAST — Prostor 4 405B (Zero Refusal)

**Model:** `nousresearch/prostor-4-405b`
**Strategy:** Prostor models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Prostor Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.prostor/config.yaml`:
