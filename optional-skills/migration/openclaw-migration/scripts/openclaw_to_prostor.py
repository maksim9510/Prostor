#!/usr/bin/env python3
"""OpenClaw -> Prostor migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Prostor Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: dict[str, dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Prostor.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Prostor memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Prostor memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Prostor-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Prostor-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Prostor command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.prostor/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.prostor/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Prostor .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Prostor .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Prostor .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Prostor .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Prostor .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Prostor config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Prostor config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Prostor.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Prostor MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Prostor config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'prostor cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Prostor config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Prostor session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Prostor custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Prostor config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Prostor config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Prostor config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Prostor config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Prostor in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Prostor`` (prose), but lowercase matches
# like ``openclaw`` → ``prostor`` (so filesystem paths like ``~/.openclaw``
# become ``~/.prostor`` — the real Prostor home — not the broken ``~/.Prostor``).
_REBRAND_PATTERNS: list[tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Prostor'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Prostor'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Prostor'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Prostor`` but maps ``openclaw`` → ``prostor`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.prostor/config.yaml`` (the real Prostor home) instead of the broken
    ``~/.Prostor/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Prostor.
...
def write_report(output_dir: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Prostor Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-prostor/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Prostor config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: dict[str, int]) -> list[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-prostor/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: dict[str, int]) -> list[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Prostor session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Prostor config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: dict[str, Any] | None = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Prostor
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Prostor-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Prostor-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: prostor config set",
                        )
                    continue
        ...
        for oc_key, prostor_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and prostor_key not in secret_additions:
                secret_additions[prostor_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, prostor_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and prostor_key not in secret_additions:
                        secret_additions[prostor_key] = val.strip()
    ...
    def migrate_model_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        prostor_config = load_yaml_file(destination)
        current_model = prostor_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = prostor_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                prostor_config["model"] = {"default": model_str}
            dump_yaml_file(destination, prostor_config)
    ...
    def migrate_tts_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Prostor still uses "edge"
        ...
        prostor_config = load_yaml_file(destination)
        existing_tts = prostor_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Prostor destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Prostor-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Prostor-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: dict[str, Any] | None = None) -> None:
        ...
        prostor_cfg_path = self.target_root / "config.yaml"
        prostor_cfg = load_yaml_file(prostor_cfg_path)
        existing_mcp = prostor_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Prostor config")
                continue

            prostor_srv: dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                prostor_srv["command"] = srv["command"]
                if srv.get("args"):
                    prostor_srv["args"] = srv["args"]
                if srv.get("env"):
                    prostor_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    prostor_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                prostor_srv["url"] = srv["url"]
                if srv.get("headers"):
                    prostor_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    prostor_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                prostor_srv["enabled"] = False
            if srv.get("timeout"):
                prostor_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                prostor_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                prostor_srv["tools"] = {}
                if tools_cfg.get("include"):
                    prostor_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    prostor_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                prostor_srv["sampling"] = {
                    k: v for k, v in {
        ...
        if added > 0 and self.execute:
            self.maybe_backup(prostor_cfg_path)
            prostor_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(prostor_cfg_path, prostor_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'prostor cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = prostor_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Prostor reasoning_effort
        ...
        if compaction:
            compression = prostor_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = prostor_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            prostor_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = prostor_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'prostor gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("PROSTOR_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        prostor_cfg_path = self.target_root / "config.yaml"
        prostor_cfg = load_yaml_file(prostor_cfg_path)
        sr = prostor_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: dict[str, Any] | None = None) -> None:
        ...
        prostor_cfg_path = self.target_root / "config.yaml"
        prostor_cfg = load_yaml_file(prostor_cfg_path)
        custom_providers = prostor_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(prostor_cfg_path)
            prostor_cfg["custom_providers"] = custom_providers
            dump_yaml_file(prostor_cfg_path, prostor_cfg)
    ...
    def migrate_deep_channels(self, config: dict[str, Any] | None = None) -> None:
        ...
        # Map Discord-specific settings to Prostor config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            prostor_cfg_path = self.target_root / "config.yaml"
            prostor_cfg = load_yaml_file(prostor_cfg_path)
            discord_prostor = prostor_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_prostor["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_prostor["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                prostor_cfg["discord"] = discord_prostor
                dump_yaml_file(prostor_cfg_path, prostor_cfg)
    ...
    def migrate_browser_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        prostor_cfg_path = self.target_root / "config.yaml"
        prostor_cfg = load_yaml_file(prostor_cfg_path)
        browser_prostor = prostor_cfg.get("browser") or {}
        changed = False

        # Map fields that have Prostor equivalents
        if browser.get("cdpUrl"):
            browser_prostor["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_prostor["headless"] = browser["headless"]
            changed = True

        if changed:
            prostor_cfg["browser"] = browser_prostor
            if self.execute:
                self.maybe_backup(prostor_cfg_path)
                dump_yaml_file(prostor_cfg_path, prostor_cfg)
    ...
    def migrate_approvals_config(self, config: dict[str, Any] | None = None) -> None:
        ...
        if mode:
            ...
            prostor_mode = mode_map.get(mode, "manual")
            prostor_cfg.setdefault("approvals", {})["mode"] = prostor_mode
            if self.execute:
                self.maybe_backup(prostor_cfg_path)
                dump_yaml_file(prostor_cfg_path, prostor_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{prostor_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Prostor Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Prostor. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Prostor Config Not Overwritten)",
                "",
                "These items already existed in your Prostor config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Prostor agent discovers these",
            "directories, it may read/write to them instead of the Prostor state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `prostor claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `prostor claw cleanup` to fix it.",
            "",
            "## Prostor-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `prostor claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `prostor setup` to configure any remaining settings",
            "- Run `prostor mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `prostor cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `prostor cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `prostor gateway install` if you need the gateway service",
            "- Review `~/.prostor/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Prostor Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("PROSTOR_HOME") or str(Path.home() / ".prostor"), help="Prostor home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Prostor targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Prostor-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.prostor/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.prostor/config.yaml")
        print("    2. Run: prostor mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: prostor cron")
