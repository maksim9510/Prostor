---
name: telephony
description: Give Prostor phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
- save provider credentials into `${PROSTOR_HOME:-~/.prostor}/.env`
...
3. Buy it and save it into `${PROSTOR_HOME:-~/.prostor}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${PROSTOR_HOME:-~/.prostor}/.env`
