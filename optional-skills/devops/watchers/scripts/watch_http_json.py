...
Usage (via cron with --no-agent):
    ...
    prostor cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$PROSTOR_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Prostor-Watcher/1.0"})
