"""ACP auth helpers — detect and advertise Prostor authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "prostor-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Prostor runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Prostor can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Prostor.
    ...
    not have Prostor provider credentials configured yet, so Prostor always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Prostor using the currently configured "
