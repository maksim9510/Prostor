"""CLI entry point for the prostor-agent ACP adapter.

Loads environment variables from ``~/.prostor/.env``, configures logging
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="prostor-acp",
        description="Run Prostor Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Prostor version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Prostor provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.prostor/node/ "
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``prostor postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting prostor-agent ACP adapter")
