...
This is an additive Prostor extension surfaced under ACP ``_meta.prostor`` so
...
compression rotates the internal Prostor head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_prostor_session_id: The live internal Prostor DB session id
            (``state.agent.session_id``).
        previous_prostor_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"prostor": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_prostor_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_prostor_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_prostor_session_id}
    ...
    provenance: dict[str, Any] = {
        ...
        "parentProstorSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_prostor_session_id:
        provenance["previousProstorSessionId"] = previous_prostor_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    if prov is None:
        return None
    return {"prostor": {"sessionProvenance": prov}}
