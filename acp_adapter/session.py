"""ACP session manager — maps ACP sessions to Prostor AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.prostor/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Prostor itself is running in WSL.

    Windows ACP clients can launch ``prostor acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Prostor from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["prostor-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Prostor agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Prostor AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Prostor runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.prostor/state.db``) is lazily created.
        """
