"""ACP agent server — exposes Prostor Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Prostor running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Prostor cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Prostor/OpenAI-compatible user content payload."""
...
class ProstorACPAgent(acp.Agent):
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Prostor version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Prostor maps edit approval
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Prostor estimates ``used`` from the same
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="prostor-agent", version=PROSTOR_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Prostor' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Prostor setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Prostor
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Prostor on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Prostor' local kawaii waiting/status
        ...
        # not the executor's. Also set PROSTOR_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue PROSTOR_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind PROSTOR_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("PROSTOR_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("PROSTOR_SESSION_ID")
            ...
            try:
                ...
                # Restore PROSTOR_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("PROSTOR_INTERACTIVE", None)
                else:
                    os.environ["PROSTOR_INTERACTIVE"] = previous_interactive
                # Restore PROSTOR_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("PROSTOR_SESSION_ID", None)
                else:
                    os.environ["PROSTOR_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Prostor DB session id before the turn so we
        ...
        # Prostor' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Prostor Agent v{PROSTOR_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Prostor has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
