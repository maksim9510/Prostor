...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Prostor Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to PROSTOR_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/prostor), this function
    mirrors the bridge source to a writable PROSTOR_HOME location and returns that
    ...
    prostor_home_bridge = prostor_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to PROSTOR_HOME if needed
    if prostor_home_bridge.exists():
        return prostor_home_bridge

    # Mirror the bridge source to PROSTOR_HOME
    try:
        prostor_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            prostor_home_bridge,
            dirs_exist_ok=False,
