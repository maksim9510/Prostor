...
There are two ways to add a platform to the Prostor gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.prostor/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Prostor code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `prostor gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `prostor_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Prostor core.
...
And add it to the `prostor-gateway` composite:

```python
"prostor-gateway": {
    "includes": [..., "prostor-your-platform"]
}
```
...
## 12. Status Display (`prostor_cli/status.py`)
...
## 13. Gateway Setup Wizard (`prostor_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ prostor_cli/ toolsets.py \
  --include="*.py" -l | sort -u
