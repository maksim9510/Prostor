...
Module-level run.py helpers a handler needs (``_prostor_home``,
...
from prostor_cli.config import cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
    atomic_yaml_write,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Prostor commands.
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`prostor gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str | EphemeralReply:
        ...
        try:
            ...
            atomic_json_write(
                _prostor_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _prostor_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Prostor Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from prostor_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Prostor default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_prostor_home())
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _prostor_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.prostor/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``prostor skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _prostor_home
        from prostor_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `prostor skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.prostor/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from prostor_cli.session_listing import (
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `prostor bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Prostor Agent to the latest version.

        Spawns ``prostor update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``prostor update`` may trigger. Marker
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Prostor Agent')}"
        ...
        if not prostor_cmd:
            return t("gateway.update.prostor_cmd_not_found")
        ...
        # Spawn `prostor update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `prostor update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                prostor_cmd_str = " ".join(shlex.quote(part) for part in prostor_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {prostor_cmd_str} update --gateway"
