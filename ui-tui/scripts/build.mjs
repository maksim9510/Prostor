...
await build({
  ...
  // Skip the prebuilt @prostor/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@prostor/ink': resolve(root, 'packages/prostor-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The prostor_cli launcher
