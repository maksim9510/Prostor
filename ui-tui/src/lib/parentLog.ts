...
const logDir = join(process.env.PROSTOR_HOME?.trim() || join(homedir(), '.prostor'), 'logs')
...
// a real ~/.prostor (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('prostor-tui: parent lifecycle log unavailable\n')
    }
  }
}
