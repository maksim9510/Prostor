...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.PROSTOR_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Prostor on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via PROSTOR_TUI_TRUECOLOR=1.
