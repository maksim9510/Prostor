...
// PROSTOR_TUI_THEME=light (or PROSTOR_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Prostor Agent')
...
describe('detectLightMode', () => {
  ...
  it('honors PROSTOR_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ PROSTOR_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ PROSTOR_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ PROSTOR_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ PROSTOR_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ PROSTOR_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets PROSTOR_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', PROSTOR_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors PROSTOR_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ PROSTOR_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ PROSTOR_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', PROSTOR_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', PROSTOR_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses PROSTOR_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ PROSTOR_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // PROSTOR_TUI_THEME=light would flip the base palette and make these
