import type { RunExternalProcess } from '@prostor/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`prostor ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching prostor: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`prostor ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
