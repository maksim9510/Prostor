...
React + Ink terminal UI for Prostor. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `PROSTOR_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.prostor/.prostor_history` or under `PROSTOR_HOME`.
...
```text
ui-tui/
  packages/prostor-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      prostor-ink.d.ts    type declarations for @prostor/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ProstorCLI subprocess for slash commands
```
