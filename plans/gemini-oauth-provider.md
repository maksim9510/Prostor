...
## Token Lifecycle
- Store at `~/.prostor/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `prostor_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `prostor_cli/models.py` — Add Gemini model catalog
4. `prostor_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `prostor_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `prostor_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via PROSTOR_GEMINI_CLIENT_ID env var
...
- prostor-agent Copilot OAuth: `prostor_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
