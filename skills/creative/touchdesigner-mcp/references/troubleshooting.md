...
## 5. Prostor Configuration

### Config location

`$PROSTOR_HOME/config.yaml` (defaults to `~/.prostor/config.yaml` when `PROSTOR_HOME` is unset)
...
Restart the Prostor session for changes to take effect. The MCP connection is
