...
PROSTOR_HOME_DIR="${PROSTOR_HOME:-$HOME/.prostor}"
PROSTOR_CFG="${PROSTOR_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Prostor config has twozero_td MCP entry ──
if [[ ! -f "$PROSTOR_CFG" ]]; then
    echo -e " ${FAIL} Prostor config not found at ${PROSTOR_CFG}"
    manual_steps+=("Create ${PROSTOR_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$PROSTOR_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Prostor config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Prostor config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$PROSTOR_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${PROSTOR_CFG} manually"); }
    manual_steps+=("Restart Prostor session to pick up config change")
fi
