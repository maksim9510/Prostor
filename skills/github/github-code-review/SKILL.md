...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _prostor_env="${PROSTOR_HOME:-$HOME/.prostor}/.env"; [ -f "$_prostor_env" ] && grep -q "^GITHUB_TOKEN=" "$_prostor_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_prostor_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
