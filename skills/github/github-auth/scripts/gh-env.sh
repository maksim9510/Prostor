#!/usr/bin/env bash
# GitHub environment detection helper for Prostor Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _prostor_env="${PROSTOR_HOME:-$HOME/.prostor}/.env"; [ -f "$_prostor_env" ] && grep -q "^GITHUB_TOKEN=" "$_prostor_env" 2>/dev/null; then
