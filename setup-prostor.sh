...
# 5. Symlinks the 'prostor' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Prostor Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/prostor-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink prostor into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up prostor command..."

PROSTOR_BIN="$SCRIPT_DIR/venv/bin/prostor"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$PROSTOR_BIN" "$COMMAND_LINK_DIR/prostor"
echo -e "${GREEN}✓${NC} Symlinked prostor → $COMMAND_LINK_DISPLAY_DIR/prostor"
...
# ============================================================================
# Seed bundled skills into ~/.prostor/skills/
# ============================================================================

PROSTOR_SKILLS_DIR="${PROSTOR_HOME:-$HOME/.prostor}/skills"
mkdir -p "$PROSTOR_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.prostor/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$PROSTOR_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  prostor status        # Check configuration"
if is_termux; then
    echo "  prostor gateway       # Run gateway in foreground"
else
    echo "  prostor gateway install # Install gateway service (messaging + cron)"
fi
echo "  prostor cron list     # View scheduled jobs"
echo "  prostor doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m prostor_cli.main setup
fi
