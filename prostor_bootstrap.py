"""Windows UTF-8 bootstrap for Prostor entry points.
...
should be imported at the very top of every Prostor entry point
(``prostor``, ``prostor-agent``, ``prostor-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import prostor_bootstrap``
# (or ``from prostor_bootstrap import apply_windows_utf8_bootstrap``) at
